...
`TASK-20260908-03 | apps/cooperative-live 모둠 미리보기·재섞기 | implementation agent | 상태: DOING`
...
- **P2-COLLAB-08 — 모둠 구성 회피 장치 (계획 승인 완료, 구현 전)**: 2026-09-08 관찰([`EXP-007`](./EXPERIMENTS.md))에서 1학기가 지나 학생들이 서로를 잘 아는 상태이므로 모둠 구성이 학습 분위기를 좌우한다는 것이 확인됐다. 특히 친한 남학생끼리 묶이면 분위기를 저해하는 행동이 나온다. 정적 협동 MUD는 교사가 모둠·번호를 직접 부여하므로 교실에서 바로 피할 수 있지만, `apps/cooperative-live`는 활동마다 무작위로 모둠을 배정한다(D-023) — 이 요구와 정면으로 충돌한다. 계획: [`implementation_plan_cooperative_group_preview.md`](./docs/plans/implementation_plan_cooperative_group_preview.md) — 교사가 활성화 전에 무작위 배정 결과를 미리 보고 다시 섞을 수 있게 `lobby → preview → active` 상태를 추가한다(영구 식별자·제외 목록 없이). 사용자가 2026-09-08 진행을 승인했다(미리보기 중 학생에게 모둠 번호 비공개, 재섞기 횟수 무제한).
