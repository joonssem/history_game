...
- `index.html`의 「확장 역사 활동」 `<section>`(현재 69~104번 줄)

**조건부**
- `index.html` 스크립트 태그 — **자기 파일의 캐시버스터 줄만**. 현재 270번 줄 `js/miniGames.js?v=20260909-detective1`. 다른 줄은 건드리지 않는다.
...
이 지시서 초안 작성 시점에는 `feat/cooperative-group-preview`에 확장 역사 활동 작업 2개가 main에 반영되지 않은 채 남아 있었다. **2026-09-10에 `b18d658`(merge: feat/cooperative-group-preview 확장 활동 정리 병합)로 main에 병합되어 해결됐다.**

따라서 아래는 이제 `origin/main`에 들어 있다. 다시 cherry-pick 하지 않는다.

| 커밋 | 내용 | 상태 |
|---|---|---|
| `76f7cde` | 미니게임 버튼–결과 위치 불일치 수정 + 아이패드 가로모드 세로 스크롤 완화 | main 반영됨 |
| `2108f0f` | **확장 역사 활동 토글 스위치** 도입 + Deep-dive 유물 설명 오류 수정 | main 반영됨 |
| `9be09eb` | 유물비교 시대 매칭 3차 수정 | main 반영됨 |

**이미 정착된 것 (그 위에서 작업한다):**
- `index.html:77-79` — `.activity-toggle` 탭 전환 UI. 타임머신 스토리 / 역사 미니게임을 한 번에 하나만 전체 너비로 보여 준다.
- `index.html:82`, `:89` — `#activity-panel-story` / `#activity-panel-minigame`
- `js/app.js:99-100` — `switchExpandedActivityTab()`
- `css/style.css:585-608` — `.activity-toggle`, `.activity-toggle-btn`, `.activity-toggle-btn.active`
- 각 게임의 시작 버튼과 결과 컨테이너가 같은 위치에 붙어 있다(`76f7cde`가 고친 문제)

이 구조를 **없애거나 되돌리지 않는다.** 새 레이아웃 작업은 이 위에 얹는다.
...
- 토글 스위치는 이미 main에 정착했다(§1). 그 위에서 나머지를 맞춘다.
...
- `76f7cde`·`2108f0f`·`9be09eb`를 cherry-pick. 셋 다 이미 main에 있다(`b18d658`).
