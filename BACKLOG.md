...
3. **확인받지 못한 채 배포된 것 1건** — 조작대의 map-evidence 캔버스 옵션 A(`699cc53`). 내용은 보수적이나 승인 기록이 없다.
...
6. **T4 `claude-deep-prehistoric` 선별 반영 판단** — 7커밋 중 2개는 patch-equivalent이며 5개가 고유하다. `6fee7fc`는 직접 cherry-pick 시 현행 `stage 4`의 `hotspot-choice`와 충돌하지만, 이를 제외한 7단계 14문구는 임시 시험에서 전체 검사에 통과했다. 문서는 핵심 결정·역할이 이미 main에 반영됐고 과거 상태가 섞여 있어 그대로 병합하지 않는다. **권고: 직접 cherry-pick 금지, 문구 수동 이식 + 끊어진 문서 링크 정리 + 완료 후 archive 태그/브랜치 정리를 한 묶음으로 승인.** 상세: [`claude_deep_prehistoric_branch_review.md`](./docs/audits/claude_deep_prehistoric_branch_review.md).
...
13. **T3 `art_12` 시대 표시 결정** — 현행 `조선 후기`는 연표상 세종→명량 순서를 보존하지만 교과서의 명시적 “조선 후기 사회 변화” 소단원보다 앞선 사건에 붙어 있다. `조선 전기`로 라벨만 바꾸면 1597년이 현재 범위 밖이 되고 세종 유물과의 순서 강제가 사라진다. 현행 유지 후 표시 라벨과 계산 범위를 분리해 `조선 시대(임진왜란)`/`[1592, 1598]`로 다루는 별도 계획을 권고한다([T3 조사](./docs/audits/source_crosscheck_followup_20260914.md#t3-art_12-판옥선-시대-표시와-연표-영향-조사)).

**2026-09-15 갱신**: T2 공식 원문 대조와 T3 판옥선 시대 표시 조사를 완료했다([보고서](./docs/audits/source_crosscheck_followup_20260914.md)). T2에서는 벽란도 순서의 근거를 우리역사넷 원문으로 교체하고 명백한 유물 원문 불일치 3건을 정정했다. T3에서는 데이터를 수정하지 않고, 현행 `조선 후기`가 연표 순서를 보존하지만 교과서 단원 표시와 어긋나며 `조선 전기`로 라벨만 바꾸면 세종과의 순서 판정이 사라짐을 확인했다. `art_12`의 최종 표시 방식 등 1·2·3·4·6번의 판단 대기는 유지한다. 수업 관찰 준비물은 갖춰졌다(`docs/plans/classroom_observation_inquiry_pilot.md`) — 다음 개발 라운드보다 관찰을 먼저 권한다. 전체 현황은 `docs/handoff/claude_four_track_session_close_20260911.md` 2026-09-14 갱신 절. 새로 남은 기록 사항: 유적 출처 정책이 `kind: "site"` 대상의 확인 날짜·사진 유무를 저장할 필드를 정의하지 않았다(기능 영향 없음, 다음 유적 페어 추가 때 정한다). 근대 개항 1관문은 정답 문장의 "개항의 계기" 부분을 받치는 자료가 없다(감사 RT-02 부기, 수업 관찰 뒤 판단).
...
- **2026-09-11 라운드 2 「대조실」 — 깊게 파기(신규 페어 동결, 기존 페어 유적 연결 시도, 다시보기 판단)**: `docs/handoff/claude_four_track_round2_instruction.md` §2를 받아 진행. 이번 라운드는 **런타임 코드(`js/artifactComparison.js`·`js/encyclopedia.js`)를 전혀 건드리지 않고 데이터·문서만** 바꿨다 — `sourceNote`는 어느 렌더 함수에서도 읽지 않는 개발자용 메타데이터임을 `grep`으로 재확인했으므로, 캐시버스터 갱신·1180×820 확인·다른 트랙 교차 확인이 이번 변경엔 해당하지 않는다(§0-1/0-3 요건은 런타임을 고쳤을 때만 발동).
  - **2-1 신규 페어 동결**: 지시대로 추가 페어를 만들지 않았다(11개 유지). 2026-09-08 "5개로 마무리, 피드백 먼저" 합의를 이미 넘어선 상태라는 지시서의 지적에 동의.
  - **2-2 기존 6개 페어 유적 연결 — 표**: `docs/plans/artifact_site_source_policy.md` §4의 확인된 7곳 표를 근거로 검토했고, museum.go.kr 소장품 페이지를 WebFetch로 다시 열어 출토지 서술을 직접 확인했다. **결과: 6개 페어 중 붙인 것 0개, 전부 "확인 후 보류"**(추측 금지 원칙 준수).

    | 페어 | 대상 | 판단 | 근거 |
    |---|---|---|---|
    | `cmp_ceramics_1`/`cmp_ceramics_2` | 청자·백자·분청사기 매병 | 보류 | 셋 다 전세품 — 애초에 출토지가 없음(기존 `location` 필드에도 명시돼 있었음) |
    | `cmp_folk_paintings_1` | 김홍도·신윤복 그림 | 보류 | 화첩 소장품, 발굴 유적이 존재하지 않는 유형 |
    | `cmp_science_1` | 앙부일구·자격루 | 보류 | 궁중 기물, 보관·전승 경로만 있고 출토지 개념 자체가 없음 |
    | `cmp_paleo_neo_1`.B | 바퀴날도끼(본관6555) | 보류 | museum.go.kr에 "북한지역에서 발견"만 명시, 구체적 유적명 없음(2026-09-11 WebFetch 재확인) |
    | `cmp_metal_tech_1`.B | 용·봉황 장식 고리자루 큰 칼(신수1319) | 보류 | museum.go.kr이 "비슷한 유물이 나온 곳"(가야 지산동·옥전동, 신라 천마총, 백제 무령왕릉)만 예시로 들 뿐, 이 소장품 자체의 출토지는 명시 안 함 — 그중 하나로 추측해 붙이지 않음(2026-09-11 WebFetch 재확인) |

    각 판단과 근거는 해당 페어의 `sourceNote`에도 그대로 남겼다(2026-09-11 표시). 검증: `python scripts/01_validate_game_data.py` 통과(11개 페어 그대로).
...
### 2026-09-11 「연결 공방」(activity-loop) Track 3 Round 2 — §3-1 P2 구현: 나의 연표(옵션 B)

- **구현**: "📜 역사 연표 순서 맞추기" 안에 「⏳ 나의 연표」/「📖 기본 연표」 하위 토글 추가. 기본값은 "나의 연표"이며, 해금 유물 3개 미만이면 자동으로 "기본 연표"로 대체(안내 문구 포함). 진입점을 `startTimelineGame()` → `openTimelineGame()`으로 바꾸고(index.html 반영), 기존 고정 2세트 로직은 `#timeline-mode-content` 하위로만 다시 그리도록 최소 수정(고정 세트 자체는 손대지 않음).
- **정렬 알고리즘**: `data/artifacts.json`의 `hint`에서 `(단원, 차시)`를 정규식으로 뽑아 정렬 키로 쓴다. 설계안 §5-3의 5개 예외(타임루프 스토리 보상 `art_29`, Deep-dive 보상 `art_deep_1~4`)는 "N단원 심화" 표기가 있으면 그 단원 마지막 순번(lesson=99)으로, 그마저 없으면(`art_29`) era→단원 역추정 표로 처리 — 조용히 버리거나 임의 위치에 끼우지 않는다는 원칙을 지켰다.
- **해금 소량 시 동작**: 최소 3개 기준(P1 카드 짝맞추기와 동일 근거 — 2개 이하는 "순서"라는 개념이 성립하지 않음). 미만이면 "기본 연표"로 자동 대체.
- **재플레이 가치**: 해금이 많아지면 매번 무작위로 최대 6개만 뽑아 제시(선택된 것들끼리는 실제 역사적 순서를 정답으로 유지), "🔀 다른 유물로 다시 뽑기" 버튼 제공.
- **비허구 원칙**: 정답 공개 시 연도 대신 era를 보여주고, "정확한 연도가 아니라 시대 순서" 및 "같은 시대 안의 세부 순서는 실제와 다를 수 있다"는 문구를 명시.
- **`js/encyclopedia.js`는 읽기만** 했다(`unlockedArtifacts` 배열을 읽기만 하고 쓰지 않음, 새 필드도 추가하지 않음). 기존 배지(`badge_timeline_master`)를 재사용하고 새 배지는 추가하지 않았다(encyclopedia.js의 `badgesList`는 트랙 2 소유).
- **검증**: `node --check`, `01_validate_game_data.py`, `06_validate_static_assets.py`, `07_audit_activity_duration.py` 통과. `activity-loop`(8803)에서 실제 클릭으로 검증 —
  - 정렬 결과를 직접 출력해 (단원, 차시) 기준 순서가 맞는지 확인(예: 구석기→고조선→고려→[삼국시대/선사시대 예외, 같은 순위라 원래 배열 순서 유지]→조선 후기→현대)
  - 정답 판정 성공 흐름, 하위 토글 양방향 전환, 해금 2개일 때 자동으로 "기본 연표"로 대체되는 것, **다른 단원 탭(1→3)으로 이동해도 동일하게 동작**하는 것(§0-1 교차 확인 — 미니게임은 단원과 무관하게 전역 상태로 동작해 원래도 이 성질을 만족했다)까지 확인. 콘솔 error/warning 0건.
  - 아이패드 세로(820×1180) 887.8px — 여유 있음. **가로(1180×820) 838.8px로 뷰포트보다 약 19px 초과** — 하위 토글 한 줄(약 40px)이 늘어난 만큼이며, "한 화면을 완전히 채우고 아주 살짝 스크롤" 수준으로 지시서가 우려한 "과도한" 스크롤은 아니라고 판단했으나, 완전한 무초과는 아님을 정직하게 기록한다.
  - **캐시버스터**: `index.html`의 `js/miniGames.js?v=...`를 `20260911-personaltimeline1`로 갱신함(§0-1).
- **판단이 갈린 지점**: 정렬 동률 처리(같은 unit·lesson=99인 예외 항목들끼리의 순서)는 원래 `artifacts.json` 배열 순서를 그대로 따른다 — 진짜 역사적 선후관계(예: 선사시대가 삼국시대보다 앞섬)를 완벽히 지키지 못할 수 있음을 설계안에서 이미 밝힌 그대로이며, 이번 구현도 그 한계를 그대로 안고 간다(추가 근거 없이 임의로 더 정교화하지 않음).

### 2026-09-11 「연결 공방」(activity-loop) Track 3 Round 2 — §3-2 P3 완료: 스토리-MUD 관계 정의안 (문서만)

- `docs/plans/story_mud_relationship.md` 작성. 코드 변경 없음.
- **핵심 발견 1**: `git log -p --follow`로 `js/storyEngine.js` 전체 이력을 확인한 결과, `data/stories_chasi2.json`(구석기 편)은 생성 이후 단 한 번도 로드된 적이 없는 죽은 데이터 — "타임머신 스토리" 탭에는 실제로 3편(고조선·삼국·조선)만 뜬다.
...
### 2026-09-14 「연결 공방」(activity-loop) Track 3 — 라운드 3: 나의 연표 인수 검토 + 원인과 결과·기본 연표 사실 검증

**§3-1 나의 연표 판정 코드 인수 검토** (`js/miniGames.js`의 `getArtifactTimelineRange`/`isChronologicallyValidOrder`, 기획 세션이 RT-01 대응으로 변경)

- 시대 기간표(`eraRanges`, 16개 구간)를 36개 유물 전체와 대조해 검토. 겹치는 구간(예: 청동기시대×고조선, 통일신라×남북국시대, 삼국·남북국/조선 시대 등 광역 버킷)은 모두 실제로 시기가 겹치거나 시기 특정이 어려운 유물(art_deep_1~4처럼 여러 시대를 아우르는 심화 보상)에 의도적으로 쓰인 것으로 확인 — "판단 보류(겹치면 순서 강요 안 함)"라는 D-030 규칙 4에 맞게 설계되어 있다. 전기/후기 등 인접 구간의 경계(예: 고려~조선 1391/1392, 조선후기~개항기 1875/1876, 일제강점기~현대 1944/1945)는 모두 표준 시대 구분과 일치한다. **기간표 자체는 적절하다고 판단.**
- **`data/artifacts.json`의 `era` 라벨 오류 1건 발견 — 읽기만 하고 수정하지 않음(트랙 밖):**

| 유물 | 현재 era | 실제 시기 | 판정 |
|---|---|---|---|
| `art_12` 조선 수군 판옥선 & 천자총통(명량해전 MUD) | 조선 후기 | 1592~1598 임진왜란(정유재란 명량대첩은 1597) | **오류로 판단.** 이 앱의 다른 "조선 후기" 유물(art_11 김홍도/하회탈, art_13 상평통보, art_14 홍대용 지구의)은 전부 17세기 후반~18세기(대동법·실학·서민문화기)이고, "조선 전기" 유물(art_22, art_26, art_sejong)은 전부 임진왜란 이전이다. 임진왜란(1592~1598)은 이 앱 자신의 분류 관행상 "조선 전기"에 속해야 앞뒤가 맞는다. `조선 시대`(광역, art_19가 씀)로 완화하거나 `조선 전기`로 정정하는 것을 대조실/기획 세션에 제안한다. |

기타 35개 유물의 `era` 라벨은 모두 적절함을 확인(예: art_25 발해=남북국 시대, art_29 백제 금동대향로=삼국 시대 등).

**§3-2 원인과 결과 4세트·기본 연표 2세트 사실·의미 검증 (주 작업)**

`data/causeEffectChains.json`(4세트) + `data/timeline.json`(2세트), 총 40개 사건 카드를 전부 검토했다.

| 세트 | 검토 결과 |
|---|---|
| 원인과결과 stage1(고려) | **오류 발견·정정.** 아래 참고 |
| 원인과결과 stage2(조선) | 확인 — 5개 사건 모두 날짜로 확인되는 사건이고 인과 서술이 사실과 부합(조선 건국 1392 → 한양 천도 1394 → 훈민정음 창제 1443 → 임진왜란·명량대첩 1592/1597 → 병자호란 1636). 수정 없음. |
| 원인과결과 stage3(국권피탈) | 확인 — 을사늑약(1905)→을사의병→국권피탈(1910)→3·1운동(1919)→임시정부수립(1919), 전부 사실과 순서 일치. 수정 없음. |
| 원인과결과 stage4(광복~분단) | 확인 — 8·15광복(1945)→5·10총선거/정부수립(1948)→6·25전쟁(1950)→인천상륙작전(1950.9)→정전협정(1953), 전부 사실과 순서 일치. 수정 없음. |
| 기본 연표 stage1(선사~삼국) | 확인 — 고조선 건국(전승, "(전승)" 표기로 신화성을 이미 명시)→삼국 성립(BC1~AD1세기)→근초고왕(4세기)→광개토대왕·장수왕(5세기)→진흥왕(6세기). 순서·연대 모두 정확. 수정 없음. |
| 기본 연표 stage2(고려~조선) | 확인 — 왕건 고려건국(918)→직지 인쇄(1377)→조선건국(1392)→훈민정음(1443)→임진왜란(1592). 순서·연대 모두 정확. 수정 없음. |

**정정 상세 — 원인과결과 stage1(고려)의 "벽란도" 위치 오류**

- **문제**: 기존 순서가 "…강감찬 귀주대첩 → 몽골 침입·팔만대장경 조판 → 벽란도 국제 교류"였다. 즉 벽란도의 전성기가 몽골 침입 **이후**의 성과인 것처럼 배치되어 있었다.
- **대조**: 우리역사넷 「활발한 대외 교류」(2026-09-15 확인, `https://contents.history.go.kr/mobile/ta/view.do?levelId=ta_m71_0050_0010_0030_0040`)는 거란과의 전쟁이 끝나 대외 관계가 안정된 뒤 벽란도가 국제 무역항으로 크게 번성했다고 설명한다. 우리역사넷 「몽골의 고려 침입」(`https://contents.history.go.kr/mobile/kc/view.do?levelId=kc_i200800`)은 1231년 1차 침입 시작을 명시한다. 따라서 벽란도 교류는 몽골 침입 **이전**이며, 기존 순서와 정반대다.
- **조치**: 이벤트 순서를 "…강감찬 귀주대첩 → 벽란도 국제 교류 → 몽골 침입·팔만대장경 조판"으로 정정. `ce15`(벽란도) 힌트도 "거듭된 전란 속에서도"(몽골 침입까지 포함한 것으로 오독될 수 있는 표현) → "거란의 침입을 모두 물리치고 안정을 되찾은 고려는…"으로, `ce14`(몽골) 힌트도 "거란을 이겨낸 고려였지만" → "번영을 누리던 고려였지만"으로 자연스럽게 연결. `id`는 유지해 기존 배지·저장 데이터와 호환된다.
- **검증**: 로컬(activity-loop:8803)에서 정정된 순서로 정답 판정 실제 실행, "🎉 완벽합니다!" 및 사건 체인 텍스트가 새 순서로 정상 출력됨을 확인. `01_validate_game_data.py` 통과, `node --check js/miniGames.js` 통과(코드는 수정하지 않았으므로 회귀 없음), `git diff --check` 통과. 콘솔 에러 0건. 데이터 파일(`data/causeEffectChains.json`)만 바뀌었고 `<script>` 태그로 로드되지 않으므로 캐시버스터 갱신 대상이 아니다(app.js가 `fetch()`로 매번 새로 읽음).

**판단이 갈린 지점**: `ce13`(강감찬 귀주대첩)은 실제로는 거란의 3차 침입(993/1010/1019 중 세 번째)에서 있었던 승리인데, 카드는 "재침입"으로 2차 침입까지만 암시해 2차 침입(1010, 개경 함락)을 건너뛴다. 5학년 수준에서 "재침입"이라는 표현이 사실을 왜곡하지는 않는다고 판단해 수정하지 않았다(성립 요소인 "서희 담판 이후에도 거란이 다시 쳐들어왔고 강감찬이 물리쳤다"는 명제 자체는 참이다). 더 정밀하게 하려면 "여러 차례의 재침입" 정도로 완화할 수 있으나, 이번 라운드의 "사실관계·인과 서술" 검증 기준(명백한 오류만 정정)에는 해당하지 않는다고 보아 보류.

### 2026-09-14 라운드 3 통합 판단 (Opus 5)

- **대조실이 브랜치를 거치지 않고 main에 직접 올렸다(`56f8013`).** 지시서는 worktree 브랜치에서 작업하도록 했다. 내용(9개 페어 연대 표기 정밀화, `pottery_1` sourceNote 모순 정정, 유적 링크 `checkedAt`·`hasPhoto` 필드와 `01_validate_game_data.py` 검사)은 통합 검증에서 문제가 없었으므로 되돌리지 않는다. 다음 라운드부터 "main 직접 push 금지, 통합은 기획 세션이 한다"를 지시서 공통 절에 명시한다.
...
### 2026-09-15 「연결 공방」(activity-loop) Track 3 — 라운드 5: 원인과 결과 인과 서술 강도 대조

`data/causeEffectChains.json` 4세트(20개 카드)를 대상으로, 이번엔 연도·순서가 아니라 **"A 때문에 B"라는 인과 연결 자체가 공식 출처로 뒷받침되는지**를 봤다. 상세: [`docs/audits/cause_effect_fact_check.md`](docs/audits/cause_effect_fact_check.md).

- **정정 1건**: `ce15`(벽란도, 1단원) — "거란의 침입을 모두 물리치고 안정을 되찾은 고려는"이라는 정치적 안정→무역번성 인과를, 우리역사넷 「벽란도」가 지리적 이점만 원인으로 들 뿐 지지하지 않아 순수 시간 표지("그 무렵 고려는")로 완화. 참고로 이 카드는 라운드3에서 위키백과를 근거로 순서를 고쳤던 카드인데, 이번에 공식 출처로 재확인한 결과 **순서 판단은 유지되고 인과 서술만 추가로 낮췄다**(별개 축의 문제였음).
- **수정 없음(강한 인과가 오히려 원문으로 확인됨)**: `ce14`(팔만대장경) — 우리역사넷이 "부처의 힘으로 몽골군의 침입을 물리치고자 하는 염원에서 비롯되었다"고 명시해, 기존 서술이 과장이 아니라 정확한 인과였음을 확인.
- 2단원 5개 카드(조선)는 모두 인과 서술이 적절 — 특히 `ce18`(훈민정음)은 세종 서문 원문이 직접 뒷받침. 수정 없음.
- **3단원(을사늑약~정전협정) 10개 카드는 규칙대로 수정하지 않고 확인만** — 표준적 인과 설명이며 과장 표현을 찾지 못함(원문 미대조로 "확인 불가" 표시).
...
### 2026-09-16 「관문 설계실」(gate-grammar) Round 7 §1 — 1단계: 공인 카드를 품팔이 카드로 교체

- 지시서: `docs/handoff/claude_four_track_round7_instruction.md` §0·§1, 결정 근거 `DECISIONS.md` D-034. **`docs/handoff/claude_joseon_late_role_cards.md` 한 파일만** 고쳤다. `apps/**`·`cooperative-mud/**`·`data/mud/*.json`·`js/*.js`·남의 파일은 건드리지 않았다. main 체크아웃 미접촉, 이 브랜치에만 커밋 — push 없음.
- **5인 변형의 `gongin`(공인) 카드를 `laborer`(품팔이) 카드로 교체**했다. 공인은 보부상과 같은 상인 관점이라 겹친다는 D-034 판단을 그대로 따랐다. 품팔이 카드 내용: 이앙법 확산으로 광작이 늘며 땅을 빌리지 못하게 된 일부 농민이 품삯으로 생활 — "모든 농민이 땅을 잃었다"처럼 쓰지 않고 privateInfo 2·5번에서 "일부"·"모든 마을에 똑같이 있는 것은 아니"라고 범위를 명시했다(지시서 주의 사항 직접 반영).
- 이해관계 충돌 문장 추가: 농민(넓은 땅을 지어 이익) ↔ 품팔이(일거리는 생기지만 제 땅이 없어 불안정) — 같은 변화(광작)가 두 처지에 다르게 나타남을 명시.
- 역할 id·`evidenceId`를 D-034 확정값 표와 맞췄다: `evidenceOptions`의 `ev-gongin`을 `ev-laborer`(roleId `laborer`)로 교체, §9의 5인 구성 안내를 `["farmer","bobusang","craftsman","recordkeeper","laborer"]`로 수정, §10 확인 불가 목록에서 대동법·공인 관련 항목을 뺐다(더 이상 해당 없음).
- 문서 첫머리에 "§4~§8(최초 판단·공동 해석·질문 틀·교사 개입·sticky-wall)은 연결 공방의 참고 자료이며 확정본은 패킷 JSON"이라고 명시했다(지시서 소유 경계 반영).
- 대조실 요청 목록을 6곳→5곳으로 정리(대동법·공인 성립 범위 항목 제거).
- 검증: `04`·`05`·`13`·`14`·`15`(조작대가 이번 라운드에 만든 `scripts/15_validate_cooperative_packet.js`) 전부 통과 — `15`는 연결 공방의 `joseon_late_packet.draft.json`에 "전국" 과잉 단정어 경고 3건을 냈으나 그 파일은 이번 라운드 내 연결 공방 소관이라 손대지 않았다. 부수 갱신된 보고서 2개(내 소유 아님)는 `git checkout --`으로 되돌렸다.
- 커밋 직전 `git status --short`로 대상 문서 1개만 바뀌었는지 확인했다.
- **확인 불가**: 광작-품팔이/임노동 인과의 정확한 시기·지역 범위(대조실이 라운드 7 §2에서 우선 정리하기로 함), 전황 시기·빈도, 관영→민간 수공업 전환 폭.
- **2단계에 넘길 것**: 대조실의 수정 요청(2단계에서 반영 예정, 이번 1단계에서는 반영하지 않음).

### 2026-09-16 「관문 설계실」(gate-grammar) Round 7 §1 — 2단계: 역할 카드 확정

- 지시서: `docs/handoff/claude_four_track_round7_stage2_instruction.md` §1. `docs/handoff/claude_joseon_late_role_cards.md` 한 파일만 고쳤다. main 체크아웃 미접촉, 이 브랜치에만 커밋 — push 없음.
- 대조실이 `docs/audits/joseon_late_source_review.md`의 `ROLE_CARD_REVIEW`에서 낸 문장별 판정을 전부 반영했다.
  - **범위 초과 정정**: 농민 #2 "혼자 힘으로" → "예전보다 적은 일손으로"(광작농도 농번기엔 고용 노동을 함께 썼다는 원문 근거).
  - **확인 불가 → 빼서 §11 "보류 문장"에 보존**: 농민 이모작 문장, 보부상 "곡식·물건 값 시기별 변동" 문장. 개념 자체는 삭제하지 않고 각 역할의 `interest` 필드에 남겨 카드 설계(이해관계 충돌)는 그대로 유지했다.
  - **일반화(낮추기)**: 수공업자 #3의 "보부상이 다른 지역에서 가져온 물건과 경쟁"이라는 특정 인과를 "다른 지역 물건과 겨루어야 할 때도 있다"로 일반화(원문 미확인 인과만 제거, 개념은 유지). 원문은 §11에 보존.
  - **원문 용어 교체**: 기록관 #2 "품을 팔러 떠돌아다니는 사람" → "유리민"(대조실이 확인한 우리역사넷 「인구의 증감」, 정조 12년(1788) 호적 미등재 약 30% 추정 수치도 근거로 확인됨).
  - 기록관 #3(장시 기록의 고을별 차이·미조사 범위)은 대조실이 "확인 불가"로 남겼으나, 역할이 스스로 밝히는 업무 범위 한계 서술이라 새 역사적 단정이 아니라고 판단해 그대로 두고 그 판단을 문서에 명시했다.
- **품팔이(`laborer`) 카드를 대조실 근거로 전면 재작성**: "몰락 농민 중 일부"로 범위를 확정(우리역사넷 「광작과 광작농」). "모든 품팔이가 도시로 갔다"처럼 단일 경로로 쓰지 않고, 농촌 잔류(고공)·도성 이동(세곡 하역) 두 경로를 우리역사넷 「임노동자」 원문 그대로 함께 제시했다.
- **글자 수 자체 점검**(조작대의 `scripts/15 --cards` 옵션이 아직 없어 수작업으로 확인): `privateInfo` 문장 중 60자를 넘긴 2곳(농민 #4, 기록관 #2)을 40자 안팎으로 압축했다. 나머지 문장은 40~55자 구간으로 확인, 별도 수정하지 않았다.
- 문서 첫머리 상태를 "확정(2단계)"으로 바꾸고, 새 §11 "보류 문장" 절을 추가해 빠진 문장·판정·되살릴 조건을 표로 남겼다.
- 검증: `04`·`05`·`13`·`14`·`15` 전부 통과(데이터 불변이라 04/05/13/14 회귀 없음). `15`는 자리표시자 15곳 경고(연결 공방 2-나 몫, 1단계 규칙대로 경고 취급)만 냈다 — `--cards` 옵션은 아직 없어 실행하지 못했고, 조작대가 올리면 다시 돌려 볼 예정이다. 부수 갱신된 보고서 2개(내 소유 아님)는 `git checkout --`으로 되돌렸다.
- 커밋 직전 `git status --short`로 대상 문서 1개만 바뀌었는지 확인했다.
