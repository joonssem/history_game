...
1. `preview` 동안 학생 화면에 "잠시만 기다려 주세요"만 보여줄지, 아니면 자기가 배정된 모둠 번호까지는 미리 보여줄지 — 후자는 재섞기 중간 결과를 학생이 보게 되어 혼란을 줄 수 있어 전자를 기본값으로 제안한다.
2. 재섞기 버튼에 횟수 제한을 둘지(예: 교실에서 무한정 반복하며 시간을 끌 가능성) — 일단 제한 없이 시작하고 실제 수업에서 문제되면 조정하는 방향을 제안한다.
