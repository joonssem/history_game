...
- **P2-COLLAB-08 — 모둠 구성 회피 장치 (미착수)**: 2026-09-08 관찰([`EXP-007`](./EXPERIMENTS.md))에서 1학기가 지나 학생들이 서로를 잘 아는 상태이므로 모둠 구성이 학습 분위기를 좌우한다는 것이 확인됐다. 특히 친한 남학생끼리 묶이면 분위기를 저해하는 행동이 나온다. 정적 협동 MUD는 교사가 모둠·번호를 직접 부여하므로 교실에서 바로 피할 수 있지만, `apps/cooperative-live`는 활동마다 무작위로 모둠을 배정한다(D-023) — 이 요구와 정면으로 충돌한다. 실시간 앱에 "이 조합은 피한다" 같은 교사 지정 제외 목록이 필요한지, 아니면 정적 버전처럼 교사가 모둠을 직접 배정하는 방식을 유지할지 결정한다. 학생 이름 없이(모둠·번호 기반) 제외 규칙을 표현할 방법도 함께 검토한다.
