...
교실 앞 화면: QR 1개 (세션 입장 주소 + 수업 코드. 개인 비밀 없음)
  ...
  → 새 호·모둠·역할 배정      = 활동마다 새 무작위 값, 이전 활동과 연결하지 않음
...
학생을 화면에서 구분하는 이름은 `S07` 같은 시스템 코드 대신 **활동마다 새로 무작위 선택되는 호(號)** 로 한다. 학생은 섞인 공용 목록에서 하나를 고르거나 무작위 선택을 요청할 수 있고, 미선택자는 시작 시 서버가 남은 호에서 배정한다. 목적은 실제 이름 없이 학생이 그 시대 안에 있다고 느끼게 하는 것이다.

**왜 호인가.** 호는 조선 시대에 이름 대신 쓰던 별칭이다. `퇴계`(물러난 시내), `율곡`(밤나무 골), `다산`(차 나는 산)이 자연 풍경과 연결되고, 이황·이이·정약용은 5학년 교과 내용이라 학습으로 연결된다. 앱은 "옛사람들은 이름 말고 스스로 지은 호로 불렸어요" 한 줄로 안내한다. 앱의 호는 실제 학생의 지속 별명이 아니라 한 활동에서만 쓰는 임시 활동명이다.

**호는 역할과 충돌하지 않는다.** 호는 직업이나 신분을 주장하지 않으므로, 서버가 배정한 역할과 나란히 쓸 수 있다. `다산 정약용`처럼 호와 역할을 병기한다. 따라서 이름 선택과 역할 배정의 시점을 나눌 필요가 없다.

```text
대기 화면   섞인 목록에서 새 호 선택/무작위   "새벽 안개"
교사 시작   서버가 모둠·역할을 무작위 배정    + 농부
표시        새벽 안개 · 농부
```

**대기 시간을 그대로 쓴다.** 21명이 QR로 들어오는 동안이 곧 선택 시간이다. 교사가 시작 버튼을 누르는 순간 **미선택자에게는 서버가 남은 호를 무작위로 배정**하고, 전체 대기 학생을 해당 활동의 모둠·역할에 무작위 배치한다. 늦게 들어온 학생 때문에 수업이 지연되지 않는다.
...
- **세션 안에서만 유일성을 보장한다.** 이미 선택된 호는 목록에서 비활성으로 표시하고 다음 활동에서는 목록을 다시 섞는다.
...
| 수업 소유 교사 | 자기 세션의 모둠 구성·진행 상태·집계 확인, 힌트·심화 개입·종료·삭제. 개인 선택 내용 조회 금지 |
...
### 2-3. 개인 응답 비저장 원칙 (2026-09-06 결정)

**학생의 판단 내용은 서버에 저장하지 않는다.** 앱은 이름을 수집하지 않지만, 교사가 `모둠·번호 = 실제 학생` 대응표를 갖고 있으므로 서버의 개인 응답과 결합하면 특정 학생의 판단이 식별된다. 정적 버전에는 없던 위험이다(기기 안에만 있었다).

교사 대시보드가 실제로 필요로 하는 것은 학생의 **답**이 아니라 **진행 상태**다. 2026-09-04 수업에서 교사가 꼽은 제약은 "모둠 진행 상황을 볼 수 없음"이었지 "학생이 무엇을 골랐는지 볼 수 없음"이 아니었다. 빠른 모둠에 추가 상황을, 느린 모둠에 힌트를 주려면 속도만 알면 된다.

| 서버에 저장 | 학생 기기의 세션 한정 메모리/`sessionStorage` |
|---|---|
| 세션·모둠 구성, 역할 배정, 호 | **최초 판단의 선택지와 이유** |
| 모둠 단계와 단계 시작 시각 | **수정 판단의 선택지와 이유** |
| 각 학생의 **제출 여부와 시각** (내용 없음) | **역사 비교 응답** |
| 도움 요청 플래그 | |
| 모둠 법 초안·확정 (모둠 산출물) | |
| 법 확인 응답 `agree`/`discuss` (합의에 필수, 내용 아님) | |

교사 화면 예: `2모둠 · 4명 중 3명 제출 · 새 증거 단계 · 6분 경과 · 도움 요청 1`

이 원칙이 포기하는 것과 대체 수단:

- 교사가 화면에서 특정 학생의 선택을 열어 볼 수 없다 → **걸어가서 학생 화면을 본다.** 2026-09-04에 성과를 만든 대면 관찰 방식에 가깝다.
- 판단 변경을 서버에서 집계할 수 없다 → 학생 기기의 요약 화면을 띄우고 모둠 대표가 발표한다. 2026-09-04 수업도 모둠 1번 학생 발표로 진행했다.

함께 지키는 두 가지:

- **보관 기간 0일.** `수업 종료`가 곧 삭제다. 세션 하위 rooms/players/actions/events와 입장·복구 자격을 함께 지운다. 남기고 싶은 관찰은 교사가 앱 밖에 옮겨 적는다.
- **활동 간 연결 금지.** 다음 협동 MUD는 새 참여 ID·호·모둠·역할로 시작한다. 기기의 개인 응답도 종료 시 지우고 이전 세션 식별자를 재사용하지 않는다.
- **대응표를 앱에 넣지 않는다.** `2모둠 3번 = ○○○` 표는 교사가 앱 밖에서만 관리하고 수업 후 폐기한다. 학생 명단 업로드 기능을 만들지 않는다 — 편의를 이유로 추가하는 순간 위 두 층이 무의미해진다.

남는 조건: 제출 여부와 모둠 산출물은 수업 중 잠시 서버에 존재한다. Convex Cloud는 미국 동부·아일랜드 리전만 제공하므로 **개인정보 처리 근거와 Convex·Vercel 처리위탁·국외 처리 조건을 착수 전에 확인한다**(`BACKLOG.md` P1-COLLAB-PRIVACY). 국외 처리가 불가하면 [D-019](../../DECISIONS.md)를 다시 연다. 이 §2-3 설계는 백엔드와 무관하게 그대로 쓸 수 있다.
...
players {
  ...
  // §2-3 개인 응답 비저장 원칙: 선택지·이유의 내용은 서버에 두지 않는다.
  // 제출 여부와 시각만 저장해 교사 대시보드의 진행 상태를 만든다.
  firstDecisionSubmittedAt?: Timestamp;    // 내용 없음. 학생 기기에만 저장
  revisedDecisionSubmittedAt?: Timestamp;  // 내용 없음. 학생 기기에만 저장
  historyReflectionSubmittedAt?: Timestamp;

  // 모둠 합의에 필요한 최소 응답. 내용이 아니라 동의 여부다.
...
연결이 끊기면 전송 대기와 서버 저장 완료를 구분한다. 최종 공동 결정·다음 단계는 서버 응답 전 완료 표시하지 않는다. 오래된 오프라인 제출은 버전 검사로 거부하고 현재 단계로 복구한다. `sessionStorage`는 필요하면 미제출 초안 캐시로만 사용하며 공동 상태를 덮어쓰는 기준으로 삼지 않는다. 종료 시 이 캐시를 지운다.
...
**보관 기간은 0일이다**(§2-3). `수업 종료`가 곧 삭제이며, 교사가 남기고 싶은 관찰은 앱 밖에 옮겨 적는다. `deleteAfter`는 종료 버튼을 누르지 못한 세션의 안전망으로만 쓰고 짧게 잡는다. 삭제 작업은 세션 하위 rooms/players/actions/events, 힌트·심화 이력, 입장·복구 자격을 함께 제거하고 세션 전용 익명 인증 잔여 데이터와 기기 `sessionStorage`도 정리한다. 자동 삭제 실패를 교사에게 알리고 수동 강제삭제를 제공한다. 인증·호스팅 제공자의 운영 로그·백업 보관은 별도 확인한다. 후속 연구용 내보내기·학생 장기 추적·다른 교사 초대는 MVP 밖이다.
