# Walkthrough

## 2026-09-10 — 고려 문화 파일럿 GitHub Pages 운영 배포

`TASK-20260910-07 | main 병합·GitHub Pages 배포 | integration agent(Codex) | 상태: DONE`

- `audit/interaction-diversity`를 최신 `main`과 격리된 통합 worktree에서 fast-forward한 뒤 `1b1fc89d24625fa5ffd1f9f6744c160de5c126d4`로 `main`에 푸시했다.
- GitHub Pages build `1205796181`은 해당 커밋 기준 `built` 상태이며 운영 URL은 `https://joonssem.github.io/history_game/`이다.
- 운영 화면에서 inquiry-task 패널을 확인했고, Canvas 3회 탭과 첫 선택지 단독 선택만으로는 진행되지 않았다. 두 근거 자료와 최종 판단 제출 후에만 첫 관문이 해제되었다.
- 운영 브라우저 콘솔 error/warning은 0건이었다.

## 2026-09-10 — 고려 문화 `inquiry-task` 수직 파일럿

`TASK-20260910-06 | regular_goryeo_culture inquiry-task 수직 파일럿 구현 | 실행·코딩 에이전트(Codex) | 상태: DONE`

- `regular_goryeo_culture`의 반복 핫스팟 4단계를 첫 판단·수정, 인쇄 절차 배열, 지도 근거–자료 한계, 주장–근거–한계 구성 활동으로 바꿨다. 단계 1~3에서 획득한 근거 6장을 최종 관문에서 다시 사용한다.
- 공용 `inquiry-task`와 네 하위 task 판정기를 `js/mudInquiry.js`로 분리했다. 조작 횟수가 아니라 제출 결과가 타당한 `validated-state`일 때만 기존 MUD 선택지 게이트가 열린다.
- 카드 선택과 순서 이동은 실제 버튼으로 제공하고 선택 상태를 `aria-pressed`, 피드백을 `aria-live`로 노출했다. 같은 오개념을 다시 제출하면 질문형 힌트를 붙이고 오류 요약으로 키보드 초점을 옮긴다. 지도 단계는 Canvas 지점과 동일한 DOM 버튼을 함께 제공하며 어느 쪽을 눌러도 같은 의미 ID가 선택된다.
- 계약·무결성·런타임 검사에 새 interaction과 task별 필수 필드·참조·완료 전략을 추가했다. 반복 탭 감사기는 `validated-state` 단계를 숫자 target만 보고 후보로 잡지 않도록 갱신했다.
- 데스크톱 Chrome에서 오답→수정→완료 네 단계, 캔버스 지점 선택, 키보드 Enter, 최종 유물·성찰 화면, 기존 구석기·신석기 핫스팟 게이트를 확인했고 콘솔 오류·경고는 0건이었다. 390×844·820×1180 지정 뷰포트와 실제 학생 수업 관찰은 다음 검증 게이트다.
- 검증: `scripts/01·03·04·06·08·09_validate_*`, `scripts/07_audit_activity_duration.py`, `scripts/09_audit_tap_resistance.py`, `scripts/05_test_simulator_runtime.js`, `scratch/audit_interaction_diversity.py`, JS 문법 검사, `git diff --check` 통과.

## 2026-09-08 — 실시간 협동 MUD 모둠 미리보기·재섞기

`TASK-20260908-03 | apps/cooperative-live 모둠 미리보기·재섞기 | implementation agent | DONE`

- 수업 시작을 `lobby → preview → active`로 분리해 교사가 학생에게 공개하기 전에 모둠 구성을 확인하고 무제한으로 다시 섞을 수 있게 했다.
- 미리보기 취소 시 모둠·역할 배정을 지우고 입장 대기로 돌아가며, 확정할 때만 학생 단계를 역할 확인으로 바꾸고 모둠 방을 생성한다.
- 미리보기 중 새 입장과 호 변경을 막고, 학생 화면에는 모둠 번호와 역할 대신 대기 안내만 표시한다. 새 영구 식별자나 제외 규칙은 추가하지 않았다.
- 데모 브라우저에서 활동 생성, 가상 학생 8명 입장, 미리보기, 재섞기, 취소, 재미리보기, 확정을 확인했다.
- Convex 개발 배포에서도 합성 학생 8명으로 미리보기 시 `lobby` 단계 유지·방 0개, 재섞기 결과 변경, 취소 후 재시작, 확정 후 2개 방 생성을 확인했다. 종료 뒤 `sessions`, `players`, `rooms`, `interventions`는 모두 0건이다.
- 검증: `npm run lint`, `npm run typecheck`, `npm test`(6건), `npm run build`, Convex 함수 배포 및 합성 데이터 통합 점검 통과.

## 2026-09-08 - Cooperative live cloud preview integration

`TASK-20260908-01 | privacy record, join security plan, preview integration | integration agent | DONE`

- Limited the Vercel GitHub App to `joonssem/history_game`. Deployed `main` commit `d9ee325` as a Next.js project rooted at `apps/cooperative-live`.
- Added only four public client variables to Vercel. Kept the teacher Auth0 `sub` allowlist only in the Convex deployment environment.
- Kept Auth0 Google-only and allowed the localhost and stable Vercel callback, logout, and web-origin URLs. The non-verifiable callback confirmation safeguard remains enabled.
- Verified the authenticated teacher dashboard, eight synthetic students, two groups of four, unique role assignment, and session start against the US-East Convex development deployment.
- After cleanup, confirmed that the Convex `sessions`, `players`, `rooms`, and `interventions` tables were empty.
- Validation passed: lint, TypeScript, five unit tests, production build, Convex function deployment, and Vercel deployment. The Convex TypeScript blocker was fixed with `allowImportingTsExtensions`.
- The stable Vercel alias is a synthetic-data technical preview, not approval for real-student use. QR token hardening is planned but not implemented, and the privacy and foreign-processing gate remains open.

## 2026-08-31 — 반복 탭 감사 도구 오탐 정리

- 고유 단서·순서 진행을 이미 강제하는 구석기 6단계와 적 함선 표적을 요구하는 명량 포격 단계를 단순 반복 탭 후보에서 제외했다.
- JSON 핫스팟이 없는 레거시 렌더러의 실제 런타임 계약을 감사 도구에 명시해, 구조적 후보 수를 실제 위험에 맞게 집계하도록 했다.
- 앱 런타임 동작은 변경하지 않았으며, 감사 결과와 전체 검증을 다시 실행했다.

## 2026-08-31 — 청동기 거석 운반 반복 탭 내성 보강

- `regular_bronze_age` 3단계의 반복 슬라이더 활동을 흙 경사로·통나무 굴림대·공동 노동 핫스팟 활동으로 전환했다.
- `dolmen-step3`에서도 핫스팟 입력이 우선 처리되도록 보완하고, 캔버스·대체 버튼의 고유 단서 기록을 동기화했다.
- 청동기 거석 운반 장면을 추가해 세 단서의 위치와 역사 맥락을 화면에 연결했다.
- 820×1180 Chrome에서 동일 단서 반복 방지, 세 단서 완료 후 선택지 활성화, 콘솔 오류 0건을 확인했다.
- 구조적 반복 탭 후보는 51개에서 50개로 감소했다.

## 2026-08-31 — 브라우저 회귀 점검 복구 및 대표 수직 슬라이스 완료

- 브라우저 플러그인 갱신 후 820×1180 세로형 태블릿 뷰포트에서 실제 화면 점검을 수행했다.
- 포털 초기 화면과 2단원 7차시·3단원 12차시의 primary MUD 연결이 의도대로 동작하는지 확인했다.
- `regular_paleolithic`에서 필수 단서 잠금, 순서 오류 피드백, 오답 IF 분기, 정상 단계 이동을 확인했다.
- 성찰 단서 3개를 모두 확인하기 전 완료 버튼이 잠긴 상태를 확인하고, 완료 후 성찰 UI·도감 언급·콘솔 오류 0건을 확인했다.
- 상세 결과는 `BROWSER_REGRESSION_CHECKLIST.md`에 기록했다.

## 2026-08-31 — P1-02 1차 반영분 브라우저 확인

- 820×1180 세로형 태블릿 뷰포트에서 `regular_independence`, `regular_modern_open`, `regular_post_war`, `regular_japanese_rule_1`의 첫 단계를 확인했다.
- 네 MUD 모두 자료 단서용 대체 버튼이 표시되고, 단서 확인 전 선택지가 잠기며, 콘솔 오류가 없었다.
- 1~3단계 전체 흐름과 실제 학생 소요 시간 측정은 후속 검증으로 남겼다.

## 2026-08-31 — 시뮬레이터 상태 접근성 보강

- 캔버스가 안내문·피드백 영역을 `aria-describedby`로 참조하도록 연결했다.
- 시뮬레이터 안내문을 보조기기용 `note`로 지정하고 대체 조작 영역을 `group`으로 명시했다.
- 필수 활동 전후 선택지의 `aria-disabled` 상태를 실제 버튼 잠금 상태와 동기화했다.
- 검증: 전체 데이터·MUD·계약·런타임 검사, JS 문법 검사, `git diff --check` 통과.

## 2026-08-31 — 선택지 식별 표시 개선

- MUD 선택지 오른쪽의 반복되는 `선택 n →` 배지를 A/B/C 식별자로 변경했다.
- 정답 여부는 노출하지 않으면서 선택지별 색상과 모서리 형태를 다르게 적용해 모바일 화면에서도 버튼을 구분하기 쉽게 했다.
- 기존 선택지 섞기, 잠금 상태, 접근성용 `aria-label`은 유지했다.
- 검증: `node --check js/mudEngine.js`, `git diff --check` 통과.

## 2026-08-31 — Regular 반복 탭 내성 1차 묶음

- `regular_independence` 1~3단계의 단순 게이지 활동을 `hotspot-discovery`로 전환했다.
- 3·1 운동의 독립 의지·여러 계층 참여·비폭력 시위, 아우내 장터의 학생·주민 참여, 임시정부의 임시헌장·국민 주권·민주공화제 단서를 각각 선언했다.
- `independence-evidence` 장면과 공통 대체 버튼을 연결해 캔버스와 버튼이 같은 단서 진행 상태를 사용하도록 했다.
- 구조적 반복 탭 후보는 66개에서 63개로 감소했다. 이 감사는 실제 학생 소요 시간 측정이 아닌 구조 선별이다.
- 검증: 게임 데이터 JSON, MUD 무결성, MUD 계약, 시뮬레이터 런타임, JavaScript 문법, 반복 탭 감사, `git diff --check` 통과.

## 2026-08-31 — Regular 반복 탭 내성 2차 묶음

- `regular_modern_open` 1~3단계의 단순 게이지 활동을 `hotspot-discovery`로 전환했다.
- 강화도 조약의 무력 위협·치외법권·해안 측량권, 근대 시설의 병원·우편·이용 한계, 전차·전등·도시 생활 변화 단서를 선언했다.
- 기존 4단계의 `modern-open-evidence` 장면과 공통 대체 버튼을 재사용했다.
- 구조적 반복 탭 후보는 63개에서 60개로 감소했다. 실제 학생 시간 측정은 별도 검토 대상이다.
- 검증: 게임 데이터 JSON, MUD 무결성, MUD 계약, 시뮬레이터 런타임, JavaScript 문법, 반복 탭 감사, `git diff --check` 통과.

## 2026-08-31 — Regular 반복 탭 내성 3차 묶음

- `regular_post_war` 1~3단계의 단순 게이지 활동을 `hotspot-discovery`로 전환했다.
- 전쟁 피해·서로 돕기·재건 조건, 천막 교실·배움 이어가기·지역별 조건, 이산가족 기록·DMZ·대화·교류 단서를 선언했다.
- 기존 4단계의 `post-war-evidence` 장면과 공통 대체 버튼을 재사용했다.
- 구조적 반복 탭 후보는 60개에서 57개로 감소했다. 실제 학생 시간 측정은 별도 검토 대상이다.
- 검증: 게임 데이터 JSON, MUD 무결성, MUD 계약, 시뮬레이터 런타임, JavaScript 문법, 반복 탭 감사, `git diff --check` 통과.

## 2026-08-31 — Regular 반복 탭 내성 4차 묶음

- `regular_japanese_rule_1` 1~3단계의 단순 게이지 활동을 `hotspot-discovery`로 전환했다.
- 헌병 경찰의 통치·즉결 처벌·일상 통제, 토지 조사·소유권 변화·농민 부담, 비밀 결사·서약·저항의 위험 단서를 선언했다.
- 기존 4단계의 `colonial-1910s-evidence` 장면과 공통 대체 버튼을 재사용했다.
- 민감한 역사 주제의 기존 서술을 대량 수정하지 않고, 자료 기반 관찰 단서만 추가했다.
- 구조적 반복 탭 후보는 57개에서 54개로 감소했다. 실제 학생 시간 측정은 별도 검토 대상이다.
- 검증: 게임 데이터 JSON, MUD 무결성, MUD 계약, 시뮬레이터 런타임, JavaScript 문법, 반복 탭 감사, `git diff --check` 통과.

## 2026-08-31 — 민감한 역사 주제 자료 중심 상호작용

- `regular_japanese_rule_2` 1~3단계의 단순 게이지 활동을 `hotspot-discovery`로 전환했다.
- 황국신민화 정책은 정책·이름·학교생활 자료, 강제 동원 단계는 동원 기록·피해자 증언·기억 활동 자료, 조선어학회 단계는 사전 원고·맞춤법·언어생활 자료를 확인하도록 구성했다.
- 피해 상황의 직접 연기·재현이나 공포·죄책감 기반의 정답 유도를 추가하지 않았다.
- 기존 4단계의 `colonial-1930s-evidence` 장면과 공통 대체 버튼을 재사용했다.
- 구조적 반복 탭 후보는 54개에서 51개로 감소했다. 실제 학생 시간과 교사 관점의 최종 검토는 별도 대기 항목이다.
- 검증: 게임 데이터 JSON, MUD 무결성, MUD 계약, 시뮬레이터 런타임, JavaScript 문법, 반복 탭 감사, `git diff --check` 통과.

## 2026-08-31 — 누적 변경 후 감사 기준 갱신

- 5개 Regular MUD의 상호작용 변경을 반영해 활동 시간 감사 결과를 다시 생성했다.
- Regular 28종 중 보완 우선 대상은 16종, 기준 충족 추정은 12종으로 집계되었다. 이 수치는 실제 학생 측정이 아닌 설계 지표다.
- IF 스테이지 검토 신호는 94개, 유물 카드 표현 검토 신호는 6개로 확인되었다.
- 검증: `scripts/07_audit_activity_duration.py`, `scripts/09_validate_mud_sources.py`, `scripts/10_audit_if_stages.py`, `scripts/11_audit_artifacts.py` 실행.

## 2026-08-31 — IF 스테이지 품질 1차 대상 선별

- IF 스테이지 94개 검토 신호를 설명 길이와 자료·비교 단서 수로 재선별했다.
- `regular_korean_war` 3-1, `regular_joseon_diplomacy` 1-1·2-1·3-1, `regular_independence_army` 3-1, `regular_goryeo_culture` 1-1을 1차 검토 대상으로 정했다.
- 문장 보강 전 교육적 기준과 변경 범위를 [`implementation_plan_if_stage_quality_batch.md`](./docs/plans/implementation_plan_if_stage_quality_batch.md)에 기록했다.
- 이번 단계에서는 코드와 콘텐츠를 수정하지 않았다.

## 2026-08-31 — IF 스테이지 품질 1차 문장 보강

- `regular_korean_war` 3-1, `regular_joseon_diplomacy` 1-1·2-1·3-1, `regular_independence_army` 3-1, `regular_goryeo_culture` 1-1의 narrative를 자료·기록·비교 단서와 연결했다.
- 고려 문화 IF 단계의 부적절한 복귀 선택지를 목재 가공·보관 환경 자료 비교 행동으로 교체했다.
- 가상 결과와 확인 가능한 역사 사실을 구분하고, 공포나 죄책감으로 정답을 유도하지 않는 기준을 유지했다.
- IF 감사 신호는 94개에서 92개로 감소했다. 남은 신호는 교사 관점의 문장 난이도와 최소 길이 기준을 추가 검토한다.
- 검증: 게임 데이터 JSON, MUD 무결성, MUD 계약, 출처 메타데이터, IF 감사, 시뮬레이터 런타임, JavaScript 문법, `git diff --check` 통과.

## 2026-08-31 — 접근성·상태 표현 1차

- 시뮬레이터 캔버스에 `role="img"`와 대체 조작 안내를 추가했다.
- 게이지 진행률에 `role="status"`와 `aria-live`를 추가했다.
- 퀴즈·도감 모달에 `role="dialog"`, `aria-modal`, 제목 연결을 추가했다.
- 모달 닫기 버튼에 목적을 설명하는 `aria-label`을 추가했다.
- 실제 키보드 포커스·보조기기 점검은 브라우저 확인 대기 항목으로 남겼다.

## 2026-08-31 — 유물·보상 설명 표현 1차

- `art_1`, `art_3`, `art_4`, `art_15`, `art_16`, `art_18`의 과장·단정 표현을 자료의 의미와 역사적 범위에 맞게 완화했다.
- 유물명·시대·등급·보상 ID 연결은 변경하지 않았다.
- 자동 유물 감사 신호는 6종에서 0종으로 감소했다. 교사 관점의 최종 사실·문장 난이도 검토는 남겨 두었다.
- 검증: 게임 데이터 JSON, MUD 무결성, 출처 메타데이터, 유물 감사, 시뮬레이터 런타임, JavaScript 문법, `git diff --check` 통과.

## 2026-08-31 — 설계 정합성 대표 수직 슬라이스 1차 반영

- `WIREFRAMES.md`에서 실제 화면에 없는 `[자료 보기]` 요소를 제거했다.
- `regular_paleolithic` 6단계 성찰 시뮬레이터를 `required: true`로 변경해 성찰 단서 확인 후 엔딩으로 진행하도록 설계 기준을 반영했다.
- 구현 계획은 정적 검증 완료·브라우저 시각 점검 대기 상태로 유지한다. 브라우저 연결 플러그인 버전 오류로 태블릿 화면 검증은 수행하지 못했다.
- 검증: 게임 데이터 JSON, MUD 무결성, MUD 계약, 시뮬레이터 런타임, `mudEngine.js`·`mudSimulators.js` 문법, `git diff --check` 통과.

## 2026-08-28 — P1 Regular 시간·IF 구조 감사

- Regular MUD 28종의 활동 시간 구조를 다시 생성해, 10분 이내·설계 목표 약 9분 기준과 반복 탭 조기 종료 위험을 분리해 확인했다.
- 반복 탭 구조 감사는 65개 후보를 확인했으며, 장면이 있는 단계는 Claude 소유 범위이므로 좌표·핫스팟·상호작용·완료조건을 변경하지 않았다.
- Regular 재시도(IF) 단계 94개를 `docs/audits/if_stage_audit.md`로 선별했다. 설명 길이, 자료·비교 단서, 원 단계 복귀 선택지를 기준으로 검토 신호를 기록했으며, 문장 보강은 교사 검토 후 진행할 미완료 항목으로 남겼다.
- 검증: `scripts/07_audit_activity_duration.py`, `scripts/09_audit_tap_resistance.py`, `scripts/10_audit_if_stages.py` 실행 완료. 이번 단계에서는 JSON 장면 필드를 수정하지 않았다.

## 2026-08-28 — P2 MUD 등록 차시 기준 정합화

- 커리큘럼 화면의 `lessonDisplay`가 단원 내부 번호를 사용한다는 점을 확인하고, 전역 번호로 기록된 세 Regular 항목을 화면 표시 기준으로 정렬했다.
- `app.js`의 인덱스 우선 조회가 세종·3·1 운동·6·25 MUD를 해당 화면 차시에 직접 연결하도록 했고, 비등록 차시용 fallback은 유지했다.
- 장기 기준을 `DECISIONS.md`의 D-008에 기록했다. Claude 담당 시뮬레이터 장면 필드는 변경하지 않았다.

## 2026-08-28 — P2 유물·보상 설명 1차 사실 범위 검수

- MUD 보상 35개 연결을 전수 확인해 누락 ID나 보상명 불일치가 없음을 확인했다.
- `art_9`, `art_10`, `art_13`의 시대 표기를 각각 고려 시대, 고려 시대, 조선 후기로 바로잡았다.
- 직지 설명의 단정적 표현을 `현존하는 세계에서 가장 오래된 금속활자본으로 알려진`으로 정밀화했다. UNESCO와 국가유산포털의 현존·기록 범위를 반영한 표현이다.
- 36종 전체의 출처·문장 난이도 검수는 아직 완료하지 않았으며 BACKLOG에 남겼다.

## 2026-08-28 — P2 유물·보상 카드 구조 감사

- 유물 36종의 필수 필드, 과장·단정 표현, 설명 길이를 자동 선별하는 `scripts/11_audit_artifacts.py`를 추가했다.
- 검토 신호는 `docs/audits/artifact_audit.md`에 기록하고, 보상 ID 연결 검증과 교육적 사실 검토를 분리했다.
- 이번 단계에서는 유물명을 대량 변경하지 않았으며, 실제 역사 사실·문장 난이도 판정은 교사 검토 항목으로 유지했다.

## 2026-08-28 — 문서 기반 운영 체계 정비

- `PRD.md`, `BACKLOG.md`, `DECISIONS.md`를 최소 범위로 추가해 요구사항·미완료 검토·장기 설계 결정을 분리했다.
- `agents.md`, `README.md`, `project_context.md`에 문서별 역할과 링크를 연결하고, Regular MUD(5~10분 복습)와 Deep-dive MUD(확장 탐구)의 교육 목적을 명시했다.
- 이미 완료된 JSON 기반 MUD, Vanilla JS, GitHub Pages 구조와 교육과정 정합성 검토는 BACKLOG가 아니라 `DECISIONS.md`에 기록했다.
- `AI_Architecture_Reflection.md`는 저장소에 존재하지 않아 실행 지시로 해석하거나 내용을 보충하지 않았다. 실제 파일명은 `project_context.md`(소문자)이며 링크도 이 이름으로 유지했다.
- 검증: 핵심 문서 존재·링크 대상·작업 트리 상태를 확인하고, 문서 간 역할 및 완료/미완료 항목의 중복을 점검했다.

## 2026-08-28 — Regular MUD 시간 기준 보정

- 학생이 탭 가능한 요소를 의미 없이 빠르게 눌러 기존 5분 안팎의 활동을 1~2분 안에 끝내는 관찰을 반영했다.
- Regular MUD의 운영 기준을 10분 이내, 설계 목표 약 9분으로 수정하고, 자료 읽기·단서 확인·해석 없이 완료되는 경로를 별도 검증 대상으로 등록했다.
- Deep-dive MUD는 기존처럼 Regular의 대체가 아닌 확장 탐구 활동으로 유지하며 시간 기준을 혼합하지 않았다.

## 2026-08-28 — Claude/Codex 분업 후 P1 기준선 감사

- Claude 장면 영역과 충돌하지 않도록 28종 Regular MUD의 단계·시뮬레이터·완료 조건을 전수 확인했다.
- 기존 무결성·계약·카탈로그·시뮬레이터 런타임 검증은 모두 통과했다. 다만 다중 선택지 단계의 정답 위치가 1번에 치우쳤다는 기존 경고가 유지된다.
- 여러 게이지형 단계가 `target: 100`과 2~4회의 동일 탭으로 완료되므로, `required: true`만으로는 무작위 탭에 의한 1~2분 종료를 막지 못한다는 구조적 후보를 확인했다.
- 장면이 있는 단계의 `scene`, `hotspots`, `interaction`, `completion.target`, 단계 구조는 변경하지 않았다. 후속 수정은 장면이 없는 단계부터 우선 검토한다.

## 2026-08-28 — 무작위 탭 위험 자동 선별기 추가

- `scripts/09_audit_tap_resistance.py`를 추가해 Regular의 필수·비핫스팟 시뮬레이터 중 4회 이하의 반복 동작으로 완료되는 구조를 선별한다.
- 이 선별기는 실제 학생 소요 시간을 단정하지 않으며, 장면이 있는 단계에는 좌표 검토 필요 표시만 하고 JSON을 수정하지 않는다.
- 버튼형 시뮬레이터의 고유 `actions`는 반복 캔버스 탭과 구분해 선별 대상에서 제외한다.

## 2026-08-28 — 장면 비보유 Regular 메타데이터 기준 갱신

- `regular_joseon_economy.json`과 `regular_joseon_silhak.json`의 `playTime`을 기존 `5-10min`에서 현재 운영 기준인 `10min`으로 갱신했다.
- 두 파일 모두 `simulator.scene`, `hotspots`, `interaction`, `completion.target`, 단계 구조는 변경하지 않았다.

## 2026-08-28 — 실학 MUD 3단계 서사 정합성 보정

- `regular_joseon_silhak.json` 3단계와 3-1 실패 분기의 narrative가 동학·인내천 장면 및 infoText와 어긋나던 문제를 동학의 평등·사람의 존엄을 설명하는 내용으로 보정했다.
- `scene`, `hotspots`, `interaction`, `completion`, 단계 구조는 변경하지 않았다. 해당 JSON은 Claude의 Phase 37 장면 작업 파일이므로 narrative 변경 사실을 공유해야 한다.

## 2026-08-28 — Bronze 반복 탭 완료 조건 보강

- `mudEngine.js`에 선택형 시뮬레이터 액션의 고유 ID 집계와 `completion.uniqueActions` 판정을 추가했다.
- `regular_bronze_age.json` 2단계는 거석 운반의 경사로·굴림대·공동 노동 단서 3개를 각각 확인해야 선택지를 해제하도록 보강했다.
- 대상 단계에는 `scene`·`hotspots`가 없으며, Claude 담당 렌더러와 장면 계약은 변경하지 않았다.

## 2026-08-26 — 구석기 자연환경 탐색 개선

- 구석기 1단계에 `paleo-environment` 전용 시뮬레이터를 추가했다.
- 바위 그늘, 강가, 트인 평야를 서로 다른 탐색 지점으로 렌더링한다.
- 같은 지점을 반복해서 눌러도 완료되지 않으며, 3개의 서로 다른 환경 단서를 찾아야 선택지가 활성화된다.
- 탐색 지점별 역사적 피드백과 `환경 단서 찾기: n/3` 진행 표시를 제공한다.
- 불 피우기는 마른 풀 → 나뭇가지 → 부싯돌 순서를 판정한다.
- 주먹도끼 제작은 가장자리 → 돌의 면 → 뾰족한 끝의 서로 다른 작업 지점을 판정한다.
- 사냥은 발자국·바람·바위 길목, 공동체는 식량·불씨·도구·가죽을 각각 확인해야 한다.
- 마지막 성찰은 환경 관찰·도구와 불·공동체 협력의 세 단서를 확인해야 완료된다.
- 구석기 실패 분기 시뮬레이터도 해당 본 단계의 전용 활동을 재사용하도록 정리했다.
- 구형 `paleo-fire`·`paleo-stone` 렌더러가 전용 렌더러를 가로채던 분기 충돌을 제거했다.
- 불 피우기·주먹도끼의 단계 진행률을 화면 게이지와 연결하고, 안내 문구를 실제 상호작용 순서와 일치시켰다.
- 활동 판정 유형을 `interaction` 필드로 분리해 구석기 시뮬레이터가 공통 처리기(`hotspot-discovery`, `ordered-hotspot`, `resource-allocation`, `reflection`)를 사용하도록 전환했다.
- 시뮬레이터 활동 유형 문서를 `simulator_schema.md`에 정리하고 무결성 검증기에 유형 검사를 추가했다.
- MUD 무결성 검증기에 새 시뮬레이터 모드를 등록했다.
- 검증: `node --check` 2개 파일, 게임 데이터 검증, MUD 무결성 검증, `git diff --check`, 브라우저 반복 클릭·3개 단서 탐색 흐름.

## 반복 문제 점검 결과

- `paleo-fire`, `paleo-stone`, 일부 `culture-touch`·`text-reading` 활동은 여전히 좌표·선택·순서를 검증하지 않고 클릭 횟수로만 완료된다.
- 필수 시뮬레이터의 완료 계약이 모드별로 일관되지 않아 새 모드 추가 시 실행 코드·JSON·검증기를 함께 수정해야 한다.

## 2026-08-26 — 신석기 시뮬레이터 공통 계약 적용

- 신석기 1단계 환경 탐색을 `hotspot-discovery`로 전환했다.
- 신석기 2단계 빗살무늬 토기 제작을 `ordered-hotspot`으로 전환해 점토 → 뾰족한 바닥 → 빗살무늬 순서를 판정한다.
- 신석기 3단계 가락바퀴 옷감 짜기를 `resource-allocation`으로 전환해 가락바퀴·뼈바늘·실 재료를 각각 확인한다.
- 신석기 4단계 농경과 정착을 `reflection`으로 전환해 정착·농경·토기의 역사적 연결 단서를 확인한다.
- 위 활동의 핫스팟 좌표, 라벨, 피드백, 완료 조건을 `regular_neolithic.json`에 선언하고 공통 엔진 상태를 사용하도록 연결했다.
- 검증기에서 지원되는 `interaction` 유형을 검사하도록 보완했다.
- 브라우저 화면 검증은 로컬 브라우저가 구형 캐시 화면을 표시하고 런타임 전역 객체를 격리하는 문제로 정적 검증과 분리했다. 저장소 기준 검증은 계속 진행한다.

## 2026-08-26 — 선택지 버튼 구분성 개선

- 모든 선택지에 화면 표시 순번(`선택 1`, `선택 2` 등)을 부여해 무작위 섞기 이후에도 버튼을 식별할 수 있게 했다.
- 정답 여부를 색상이나 문구로 노출하지 않아 탐구형 선택의 비결정성은 유지했다.
- 각 버튼에 전체 선택지 내용을 포함한 `aria-label`을 추가해 보조기기 사용성을 보완했다.

## 2026-08-26 — 광복 MUD 필수 활동 연결

- 광복 1단계 태극기 시뮬레이터를 필수 활동으로 지정했다.
- 태극 문양과 건·곤·감·리 4괘를 모두 완성해야 다음 선택지가 열린다.
- 광복 3단계 5·10 총선거 시뮬레이터를 필수 활동으로 지정했다.
- 기표 도장 찍기 → 투표함 넣기의 두 행동을 완료해야 다음 선택지가 열린다.
- 중복 클릭은 행동 수를 중복 집계하지 않으며, 공통 `simulatorProgress` 완료 판정을 사용한다.
- 검증: JavaScript 문법 검사, 게임 데이터 검증, MUD 무결성 검사, `git diff --check`.

## 2026-08-26 — 삼국 MUD 한강 전략 탐색 연결

- 삼국 1단계에 한강 유역·서해 교역로·백제 영토 탐색 활동을 추가했다.
- 삼국 2단계에 평양성·한강 유역·국내성 탐색 활동을 추가했다.
- 삼국 3단계에 북한산 순수비·한강 유역·당항성 탐색 활동을 추가했다.
- 각 단계에서 3개의 역사적 지점을 확인해야 다음 결단이 열리도록 `hotspot-discovery`와 공통 완료 계약을 적용했다.
- 검증: JavaScript 문법 검사, 게임 데이터 검증, MUD 무결성 검사, `git diff --check`.

## 2026-08-26 — 인터랙션 데이터 계약 검증 강화

- 핫스팟이 선언된 시뮬레이터의 ID 중복, 좌표 범위, 라벨·피드백 누락을 무결성 검증에서 확인하도록 했다.
- `ordered-hotspot`의 순서 목록이 실제 핫스팟 ID를 참조하는지 검사하도록 했다.
- 필수 핫스팟 활동의 완료 목표가 실제 핫스팟 수를 초과하지 않는지 검사하도록 했다.

## 2026-08-26 — 청동기 고인돌 축조 활동 연결

- 고인돌 1단계 받침돌 세우기와 4단계 완성 행동을 필수 활동으로 연결했다.
- 2단계 흙 경사로는 진행도 100%까지 쌓아야 다음 결단이 열리도록 했다.
- 3단계 거석 견인은 동원 인원 슬라이더와 3회 이상의 이동 행동을 함께 요구하도록 했다.
- 기존 고인돌 전용 조작·렌더러를 유지하면서 공통 `simulatorProgress` 완료 판정을 연결했다.
- 검증: JavaScript 문법 검사, 게임 데이터 검증, MUD 무결성 검사, `git diff --check`.

## 2026-08-26 — 고조선 핵심 자료 탐색 연결

- 고조선 1단계에 요동·한반도 북부·아사달 건국 영역 탐색을 추가했다.
- 고조선 2단계에 8조법의 살인·상해·절도 조항 탐색을 추가했다.
- 고조선 3단계에 비파형 동검·고인돌·청동기 문화권 탐색을 추가했다.
- 각 단계에서 3개 역사 단서를 확인해야 다음 결단이 열리도록 공통 `hotspot-discovery` 완료 계약을 적용했다.
- 검증: JavaScript 문법 검사, 게임 데이터 검증, MUD 무결성 검사, `git diff --check`.

## 2026-08-26 — 통일신라 문화·통치 자료 탐색 연결

- 통일신라 1단계에 9주·5소경·왕권 강화 단서를 추가했다.
- 2단계에 석가탑·다보탑·청운교·백운교 탐색을 추가했다.
- 3단계에 석굴암 돔 쐐기돌·습도 조절·본존불 탐색을 추가했다.
- 각 단계에서 3개 역사 단서를 확인해야 다음 결단 또는 엔딩으로 진행하도록 공통 `hotspot-discovery` 완료 계약을 적용했다.
- 검증: JavaScript 문법 검사, 게임 데이터 검증, MUD 무결성 검사, `git diff --check`.

## 2026-08-26 — 구조 안정화 Phase 1 기준선 고정

- `main`과 `origin/main`이 `27124cd`로 일치함을 확인하고 MUD 32종·전체 242단계를 집계했다.
- 기존 JSON·MUD 무결성 검사와 핵심 JavaScript 문법 검사는 통과했으나, 원본 복수 선택지 115단계 모두 정답이 첫 위치임을 확인했다.
- GitHub Pages의 `_index.json`이 HTML로 응답하여 라이브 앱이 하드코딩 fallback으로 실행되는 배포 결함을 확인했다.
- iPad 10세대 세로 기준에서 2열 MUD와 피드백의 핫스폿 가림, 캔버스 대체 조작 부재를 재현했다.
- 상세 증거와 Phase 2 입력 결함은 `baseline_report_structural_stabilization.md`에 기록했다.

## 2026-08-26 — 구조 안정화 Phase 2 P0 활동·배포 봉쇄

- 불 피우기 legacy 상태에 `step`을 초기화하고 유한값 기반 진행률 계산을 적용해 오답 후 `NaN`이 발생하지 않도록 했다.
- 필수 hotspot 활동에 캔버스와 같은 행동을 수행하는 버튼 대체 조작을 추가했다.
- 피드백을 캔버스 밖으로 옮겨 iPad에서 남은 hotspot을 가리던 문제를 제거하고, 상태 알림을 추가했다.
- 앱 푸터에 배포 확인용 버전을 표시하고 `.nojekyll` 마커를 추가해 `_index.json`의 Pages 정적 제공을 준비했다.
- 검증: JS 문법, 게임/MUD 무결성, 로컬 iPad 820×1180에서 구석기 탐색·불 피우기 오답 후 재시도·완료, 콘솔 오류 0건, Pages `_index.json` HTTP 200 / `application/json`, 배포 버전 `2026.08.26-p2`.

## 2026-08-26 — Phase 3 데이터 계약·교육과정 매핑 기준선

- 현재 MUD 차시 번호가 2015 개정 교육과정 기준임을 `_index.json`에 명시하고, 목표 교육과정은 2022 개정으로 분리했다.
- `data/curriculum_mapping.json`에 32개 MUD의 기존 차시 연결을 보존하면서 2022 매핑은 `unmapped`·`pending-content-review`로 기록했다.
- `simulator_contract.json`과 `simulator_schema.md`에 지원 interaction과 필수 완료 필드의 기준을 문서화했다.
- `scripts/04_validate_mud_contract.py`를 추가해 인덱스·파일·매핑 수와 MUD ID, 시작 단계, simulator 계약을 자동 검증한다.
- README와 인수인계 문서의 교육과정·대시보드 범위를 실제 결정과 일치시켰다.
- 검증: 게임 데이터, MUD 무결성, 새 계약 검증, JS 문법 검사, `git diff --check` 통과. 기존 정답 1번 편중 경고는 후속 단계로 유지.

## 2026-08-26 — Phase 4 시뮬레이터 입력 경계 안정화

- `MudSimulators.interactionHandlers`와 dispatcher를 추가해 hotspot interaction별 처리기를 한 곳에서 선택하도록 했다.
- 캔버스 바깥 클릭, 이미 확인한 단서, 순서를 틀린 선택은 `simActionCount`에 반영하지 않도록 입력 판정과 행동 기록을 분리했다.
- 고인돌·투표·태극기 활동도 실제 상태 변화가 발생한 경우에만 행동을 기록하도록 정리했다.
- `resizeCanvas()`에서 Canvas transform을 초기화한 뒤 배율을 적용해 화면 재진입·회전 시 배경과 핫스팟이 누적 변형되지 않게 했다.
- 검증: `node --check` 2개 파일, MUD 무결성, simulator contract, `git diff --check` 통과.

## 2026-08-26 — Phase 4 runtime 상태·버튼 action 분리

- `MudEngine`에 simulator state·progress 접근자를 추가하고 JSON의 `progressKey`와 공통 진행률을 동기화했다.
- 화면상 진행과 완료 잠금이 다른 키를 읽던 구석기 6개 활동의 구조 결함을 수정하고 상태 단위 테스트를 추가했다.
- MUD JSON의 `buttonsHtml` 6개와 인라인 JavaScript를 구조화 action 14개로 전환했다.
- 광복 태극기의 전체 완성 단축 버튼을 제거하고 여섯 요소를 각각 수행해야 완료되도록 조정했다.
- iPad 820×1180 브라우저 검증에서 구석기 3개 단서 후 잠금 해제, 광복 5개 후 잠금 유지·6개 후 해제, 콘솔 오류 0건을 확인했다.

## 2026-08-26 — Phase 5 정적 품질 게이트

- `.github/workflows/quality-gate.yml`을 추가해 push·pull request에서 JSON, MUD 계약, runtime 단위 테스트, 정적 asset, JS 문법, whitespace를 검사한다.
- `scripts/06_validate_static_assets.py`로 index.html의 로컬 참조 파일과 `.nojekyll` 존재를 확인한다.
- README의 검증 불가능한 100%·완벽 표현을 현재 기준선과 지원 범위 표현으로 조정했다.
- 로컬 품질 게이트 전체 검증 통과. 기존 선택지 정답 1번 편중 경고는 별도 콘텐츠/랜덤화 과제로 남아 있다.

## 2026-08-26 — Phase 6 선사·고조선 2022 파일럿 매핑

- 구석기·신석기·청동기·고조선 MUD 4종을 2022 개정 성취기준 `[6사04-01]`에 직접 매핑했다.
- 각 MUD에 교육과정 메타데이터와 국립특수교육원·국사편찬위원회 출처를 기록했다.
- 전곡리 연대 혼동, 신석기 환경 단정, 청동기 현대 지리 용어, 고조선 위치·건국 연대의 확정적 표현을 교정했다.
- Deep-dive·스토리·연표에 반복되던 기원전 2333년 확정 표현도 건국 전승으로 통일했다.
- 출판사별 단원명·차시 번호는 교과서 정보가 확인되기 전까지 비워 두고, 성취기준 매핑만 완료 상태로 관리한다.

## 2026-08-26 — Phase 7 성취기준 전수 분류

- `data/curriculum_standards_2022.json`에 고대·고려·조선·식민지기/광복·전쟁·민주화/산업화의 2022 개정 성취기준을 추가했다.
- 32개 MUD 중 30개를 `[6사04-01]`~`[6사07-02]`에 직접 매핑하고, 해당 MUD JSON과 매핑표의 성취기준이 일치하도록 계약 검증을 확장했다.
- `regular_myeongnyang`, `regular_joseon_diplomacy`는 현행 성취기준과의 직접 정합성이 약하므로 보조 역사 맥락으로 표시했다. 삭제하거나 억지로 재매핑하지 않는다.
- 출판사별 단원·차시 번호와 MUD별 역사 서술의 근거 출처는 다음 단계에서 검증한다.

## 2026-08-26 — Phase 8 역사 서술 1차 감사

- `content_claim_audit.md`에 반복 문제와 보정 원칙을 기록했다.
- 훈민정음·학익진·석굴암·칠정산·앙부일구·고려 금속활자·전곡리 주먹도끼·대한민국 임시정부·명량대첩의 과도한 절대 표현을 자료 중심 표현으로 바꿨다.
- 이번 단계는 서술 위험을 낮추는 1차 보정이며, 모든 개별 주장에 대한 공식 출처 대조는 다음 단계로 남겼다.

## 2026-08-26 — Phase 9 주제별 역사 출처 메타데이터 보강

- 직접 매핑된 MUD에 2022 개정 교육과정 출처와 국사편찬위원회 주제 자료 출처를 함께 기록했다.
- 계약 검증은 직접 매핑 MUD에 교육과정 출처와 역사 출처가 모두 있는지 확인하도록 강화했다.
- 주제 자료는 개별 문장 자동 승인이 아니라 후속 세부 사실 검증을 위한 기준점으로 명시했다.

## 2026-08-27 — Phase 10 활동 시간·상호작용 전수 감사

- `scripts/07_audit_activity_duration.py`와 `activity_duration_audit.md`를 추가해 Regular MUD의 핵심 단계·능동 활동·필수 활동·예상 시간을 전수 계산한다.
- Regular 28종 중 24종이 보완 대상으로 확인되었고, 가장 반복적인 구조 결함은 시뮬레이터 완료 없이 선택지를 통과할 수 있다는 점이다. 구석기는 자동 추정상 10분을 조금 넘을 가능성이 있어 실제 학생 시간 측정 대상으로 표시했다.
- Regular MUD 28종의 선언 시간을 승인된 개인 활동 목표인 `5-10min`으로 통일했다.
- iPad 10세대 가로 크기(1180×820)에서 구석기 화면의 레이아웃, 필수 활동 전 선택지 잠금, 대체 터치 버튼 상태를 확인했다.
- 품질 게이트에서 감사 보고서가 데이터와 동기화되어 있는지 검사한다. 실제 5~10분 판정은 학생 3명 이상의 사용 시간 측정으로 확정한다.

## 2026-08-27 — Phase 11 레거시 터치 활동 잠금 전환

- `culture-touch`, `text-reading`, `battle-gauge`, `economy-*` 66개 단계에 두 번의 조작과 100% 진행률을 요구하는 공통 완료 조건을 적용했다.
- 이 모드들은 이미 공통 입력 처리에서 진행률을 올리고 있어 별도 렌더러를 추가하지 않고 선택지 잠금을 적용할 수 있었다.
- 발해 MUD의 실제 화면에서 조작 전 선택지 비활성화, 두 번 터치 후 선택지 활성화, 콘솔 오류 없음까지 확인했다.
- 활동 감사 기준상 보완 대상은 24종에서 19종으로 줄었다. 전용 `mn-*` 모드와 핵심 단계가 3개인 MUD는 별도 설계 단계로 남긴다.

## 2026-08-27 — Phase 12 명량 전용 상호작용 완료 조건

- 명량 본선 1·2단계는 전력·지형 단서 세 가지를 각각 확인해야 결단할 수 있도록 구조화 `observe` 행동을 추가했다.
- 3단계는 실제 왜선 네 척 명중과 네 번 이상의 조작을, 4단계는 조류 슬라이더 80 이상 조작을 완료 조건으로 연결했다.
- 포탄 충돌 시 완료 판정을 즉시 갱신하고, 슬라이더의 사용자 입력만 행동으로 세도록 엔진 경계를 보강했다.
- 자동 runtime 테스트로 중복 관찰 무효화와 슬라이더 임계값을 고정했으며, 실제 화면에서 세 단서 확인 전 잠금·확인 후 해제와 콘솔 오류 0건을 확인했다.
- 활동 감사 기준상 보완 대상은 19종에서 18종으로 줄었다. 다음 단계는 핵심 단계가 3개인 MUD의 시대별 내용·분기 보강이다.

## 2026-08-27 — Phase 13 고대사 3단계형 MUD 확장 파일럿

- 통일신라와 발해를 3개에서 4개 핵심 단계로 늘리고, 마지막에 유적·문서·제도·주민 자료를 비교·종합하도록 했다.
- 통일신라의 석굴암 습도 조절 단정과 발해의 단일 민족·단일 문화 서사를 자료가 뒷받침하는 범위로 교정했다.
- 터무니없는 오답 대신 한 자료만으로 시대 전체를 단정하는 실제적인 오개념을 실패 분기로 만들었다.
- 국사편찬위원회 자료를 다시 대조해 출처 범위를 기록했고, 두 MUD의 예상 활동 시간은 7.5분·7.9분이 되었다.
- iPad 크기 실제 화면에서 두 MUD의 새 4단계 완주와 콘솔 오류 0건을 확인했으며, 보완 대상은 18종에서 16종으로 줄었다.

## 2026-08-27 — Phase 14 일제 강점기 3단계형 MUD 확장 파일럿

- 3·1 운동·임시정부와 1910년대 무단 통치 MUD에 선언·시위·헌장, 통치·수탈·저항 자료를 종합하는 4단계를 추가했다.
- 독립운동을 단일 영웅의 서사로 좁히거나 토지 조사 사업의 결과를 근거 없는 숫자로 단정하던 표현을 자료 중심으로 교정했다.
- 모든 실패 분기는 한 자료로 시대 전체를 판단하는 오개념을 다시 비교하도록 바꿨다.
- 국사편찬위원회 헌병 경찰·토지 조사 사업·대한민국 임시 헌장 자료의 적용 범위를 MUD 메타데이터에 기록했다.
- 실제 화면 완주과 콘솔 오류 0건을 확인했고, 활동 감사의 보완 대상은 16종에서 14종으로 줄었다.

## 2026-08-27 — Phase 15 고려 3단계형 MUD 확장 파일럿

- 거란 침입과 고려 문화·교류 MUD에 외교·방어·재건, 제작·인쇄·교류 자료를 종합하는 4단계를 추가했다.
- 서희 담판의 조건과 이후 축성 과정, 귀주대첩 뒤 재건을 반영하고 전쟁의 인명 피해를 승리 놀이처럼 다루던 표현을 교정했다.
- 팔만대장경·직지·벽란도는 각 자료가 알려 주는 범위와 한계를 구분하도록 바꿨다.
- 국사편찬위원회 강동 6주·귀주대첩 자료를 추가 기록했고, 공통 interaction·계약·무결성 검증을 통과했다.
- 활동 감사의 보완 대상은 14종에서 12종으로 줄었다. 다음 묶음은 조선 3단계형 MUD이다.

## 2026-08-27 — Phase 16 조선 3단계형 MUD 확장 파일럿

- 조선 후기 서민 문화, 조선 신분제, 세종 시기 과학 MUD에 그림·공연·문학, 법제·직업·호적, 문자·관측·기구 자료를 종합하는 4단계를 추가했다.
- 조선 신분을 고정된 네 계층으로 단순화하던 서술을 법제상 양천제와 실제 계층·시기별 변화로 보정했다.
- 세종 한 사람의 영웅 서사는 정책·학자·기술자의 활동과 자료의 범위로, 서민 문화는 단일한 저항 서사가 아닌 다양한 공연·문학·생활 자료로 바꿨다.
- 국사편찬위원회 초등·교과서 자료의 적용 범위를 기록했고, 활동 감사의 보완 대상은 12종에서 9종으로 줄었다.

## 2026-08-27 — Phase 17 근현대 3단계형 MUD 확장 파일럿

- 개항기, 일제 말기, 전후 사회 MUD에 조약·시설·도시 생활, 정책·피해 기록·언어 운동, 피해·재건·분단 자료를 종합하는 4단계를 추가했다.
- 근대 문물을 무조건적 발전으로, 전후 재건을 개인의 노력만으로, 평화를 개인의 다짐만으로 설명하던 단선을 자료 비교로 보정했다.
- 강제 동원과 일본군 위안부 피해는 선정적 재현 대신 기록·증언의 의미와 피해자 존엄을 존중하는 학습 흐름으로 수정했다.
- 국사편찬위원회 강화도 조약·민족 말살 정책·전후 사회 자료의 적용 범위를 기록했고, 활동 감사의 보완 대상은 9종에서 6종으로 줄었다.

## 2026-08-27 — Phase 18 민감한 역사 활동의 정서 안전성 재설계

- 한국전쟁 MUD의 전투 수행·학도병 영웅화와 의병/안중근 MUD의 자해·총격 재현을 제거하고 두 활동을 각각 4단계 자료 탐구로 재설계했다.
- 한국전쟁은 전쟁 발발·피난, 전선 지도와 학생 일기, 정전 협정, 피해·분단·평화 종합으로 구성해 승패보다 사람들의 경험과 평화 과제를 살피게 했다.
- 의병/안중근은 의병의 다양한 참여, 단지동맹 태극기 관찰, 하얼빈 의거와 재판 진술 구분, 동양평화론 종합으로 구성했다.
- 위험한 역사적 사실은 숨기지 않되 학생이 모방하지 않도록 명시하고, 모든 단계에서 세 자료 단서를 직접 찾아야 선택할 수 있게 했다.
- 국사편찬위원회 자료를 출처 메타데이터에 추가했으며, 활동 감사의 보완 대상은 6종에서 4종으로 줄었다.

## 2026-08-27 — Phase 19 남은 수동 활동 구조 보완

- 고조선과 조선 건국 MUD의 실제 단서 탐색 화면이 감사에서 수동 정보 화면으로 잘못 집계되던 문제를 터치 기반 모드로 정리했다.
- 두 MUD의 마지막 단계도 건국 전승·법·유물, 한양의 입지·도성·교육 기관을 연결하는 세 단서 탐색으로 바꿨다.
- 조선 외교 MUD는 국경 정책, 병자호란의 성 안팎 기록, 전란 뒤 외교·회복을 자료로 읽는 네 단계 흐름으로 확장했다.
- 승패·성벽 방어 중심의 선택지를 자료의 범위와 백성의 생활을 함께 해석하는 선택지로 교체했다.
- 무결성·계약·런타임 검증을 통과했고, 활동 감사의 보완 대상은 4종에서 구석기 1종으로 줄었다.

## 2026-08-27 — Phase 20 구석기 활동 시간 최적화

- 구석기 핵심 생존 판단 5개는 필수 단서 탐색으로 유지하고, 마지막 성찰은 선택형 회고로 전환했다.
- 첫 보금자리와 사냥 단계의 중복 설명을 압축해 학습 개념과 단서 피드백은 유지하면서 읽기 부담을 줄였다.
- 구석기 활동을 5~10분 목표에 맞추고 전체 Regular MUD 활동 감사의 보완 신호를 제거했다.

## 2026-08-27 — Phase 21 iPad 화면·터치 흐름 점검

- iPad 10세대와 같은 820×1180 화면에서 구석기 환경 탐색의 선택지 잠금·세 단서 확인 뒤 해제·콘솔 오류 0건을 확인했다.
- 6·25 전쟁 MUD에서도 잠금과 단서 확인 뒤 해제가 정상 동작했으나, 처음부터 완료 문구가 보이는 데이터 표현 문제를 발견했다.
- 완료 문구를 초기 안내 문구와 함께 재사용하던 14개 단계를 전수 탐지해, 시작 시에는 탐색 안내가 보이고 완료 뒤에만 성공 문구가 보이도록 교정했다.
- MUD JSON과 진도표의 응답을 캐시 없이 읽도록 해 배포 뒤 예전 시나리오가 보일 위험을 줄였고, 진도표·README의 변경된 세 시나리오 제목도 실제 JSON과 맞췄다.

## 2026-08-27 — Phase 22 MUD 카탈로그 자동 정합성 검증

- `_index.json`의 제목·부제·차시·테마·스토리 정보가 각 MUD JSON과 같은지 검사하는 전용 검증을 추가했다.
- 새 검증이 선사 심화 MUD 부제와 3·1 운동 MUD 제목의 실제 불일치를 찾아 즉시 교정했다.
- README에 32개 MUD의 기계 검증되는 전체 목록을 추가해, 진도표와 문서의 기준 정보를 한 곳에서 확인할 수 있게 했다.

## 2026-08-27 — Phase 23 2022 개정 성취기준 정합성 검증

- 국립특수교육원 성취기준·성취수준 원문과 32개 MUD 매핑을 다시 대조해 직접 매핑 30종과 보조 맥락 2종을 확정했다.
- 파일럿으로 남아 있던 선사·고조선 4종도 정식 성취기준 매핑 상태로 전환했다.
- 성취기준 검증 완료와 출판사·교과서 판본별 차시 확인 대기를 별도 상태로 분리해 과도한 확정을 방지했다.
- 성취기준별 적용 범위와 보조 활동의 활용 원칙을 별도 보고서에 정리했다.

## 2026-08-27 — Phase 24 역사 출처 메타데이터 품질 게이트

- 32개 MUD의 출처 필수 필드, URL 형식, MUD 내부 중복, 교육과정·국사편찬위원회 출처 구성을 자동 검사하는 품질 게이트를 추가했다.
- 반복 실행 과정에서 누적된 안중근 MUD 출처 2건과 한국전쟁 MUD 출처 4건을 제거했다.
- 민감 역사 MUD 생성 스크립트도 URL 기준으로 출처를 한 번만 추가하도록 바꿔 같은 문제가 재발하지 않게 했다.
- 출처가 없던 명량·조선 외교 보조 MUD에는 교육과정상 보조 활동이라는 한계와 국사편찬위원회 역사 근거를 함께 기록했다.

## 2026-08-27 — Phase 25 역사 서술 품질 1차 감사

- 과장 표현, 반사실적 단정, 단일 원인, 집단 비하·폭력 재현을 A~D 등급으로 구분하는 감사 기준을 세웠다.
- 일제 말기 MUD는 원고 하나의 유실이 한국어·국가 소멸로 이어진다는 단정을 자료의 의미와 한계를 비교하는 문장으로 바꿨다.
- 전후 재건은 개인의 끈기만이 아니라 정책·산업·국제 원조 등 복합 조건을 살피게 했고, 세종의 칠정산·측우기 설명은 확인 가능한 범위로 제한했다.
- 명량의 가상 패망과 극적인 궤멸 서술을 해상 보급·선박·화포·물길 자료를 연결하는 판단 활동으로 조정했다.

## 2026-08-27 — Phase 26 Deep-dive 폭력·집단 서사 재설계

- Deep-dive 4종의 몰살·섬멸·국가 소멸·집단 비하형 문장을 전수 분류하고 D급 핵심 표현을 제거했다.
- 선사 활동에서 학생이 포로 노비화와 가혹한 처벌을 명령하던 역할을 자료에서 계층과 법의 성격을 해석하는 조사자 역할로 전환했다.
- 삼국·조선 전쟁은 상대 집단을 공격하는 체험 대신 지형·행정 기구·선박·화포·보급 자료를 연결하도록 바꿨다.
- 근현대사는 한국광복군·낙동강 전선 하나로 이후 역사를 확정하지 않고 광복의 복합 원인과 전쟁 속 개인 기록을 함께 읽도록 조정했다.

## 2026-08-27 — Phase 27 Regular 실패 분기 정규화

- 25개 Regular MUD에서 실패 화면의 `역사의 IF` 표기를 `자료 해석`으로 일괄 전환했다.
- 일제 피해 증언, 중인 전문 기술, 상민·노비 노동, 개항기 교통은 한 자료나 한 집단이 사회 전체를 결정한다는 단정을 자료의 범위와 복합 조건을 살피는 피드백으로 바꿨다.
- 도구·지형·유물 선택의 조건을 설명하는 피드백은 유지해, 게임의 즉각적인 학습 안내와 역사적 가상 결말을 구분했다.

## 2026-08-27 — Phase 28 `최초`·수치 주장 사실 범위 감사

- 국사편찬위원회 자료와 대조해 매소성 20만은 『삼국사기』 기록과 과장 가능성을 함께 제시하고, 명량은 전체 함대·교전선·격파 수를 구분했다.
- 명량 제목도 `13척으로 133척을 막아선 기적`에서 `13척과 울돌목의 물길`로 바꿔 수치 경쟁보다 지형·선박 자료에 초점을 맞췄다.
- 모내기 노동력 1/5, 전차 아시아 최초, 현재 세계 경제 순위처럼 근거 범위가 불명확하거나 변동하는 표현을 제거했다.
- 상정고금예문은 기록상 금속활자 사용과 실물 미전승을 구분하고, 고조선 50만 전은 화폐의 보편 사용을 확정하지 않도록 조정했다.

## 2026-08-27 — Phase 29 집단 호칭·가치 편향 감사

- 학생 안내 문장의 `왜군`, `까막눈`, `미신`을 각각 일본군, 문해 환경, 자료 근거를 설명하는 중립적 표현으로 정규화했다.
- 성리학과 실학을 낡음/진보로 단순 대립시키거나 독립운동을 `민족의 혼`으로만 설명하던 문장을 다양한 자료와 관점을 비교하도록 교정했다.
- 모욕적인 오답 선택지와 `구국의 명장`, `위대한 민족 예술` 같은 정답 유도 표현을 직책·사료·관찰 가능한 특징 중심으로 바꿨다.
- Deep-dive 4종 엔딩을 진/노멀/배드의 국가 가치 평가에서 자료 종합·부분 해석·재검토의 학습 성취 표현으로 전환했다.

## 2026-08-27 — Phase 30 iPad 시뮬레이터 UX 회귀 점검

- iPad 10세대 세로 화면(820×1180)에서 구석기·고조선 MUD의 선택 잠금, 단서 탐색, 선택 해제와 콘솔 오류 0건을 실제 브라우저로 확인했다.
- 구석기 첫 활동은 세 장소를 모두 확인해야 완료되는 조건과 안내 문구가 달랐으므로, 세 단서를 명시하도록 바로잡았다.
- 모든 핫스팟 활동이 공유하는 대체 조작 버튼에 최소 44px 높이를 적용해 iPad 터치 목표 크기를 충족시켰다.
- 불 피우기에서 순서를 틀려도 재료 버튼이 잠기지 않고 올바른 첫 재료부터 다시 진행되는 것을 확인했다.

## 2026-08-27 — Phase 31 구석기 시뮬레이터 장면 맥락화

- 구석기 첫 활동의 한탄강 지형 장면을 기준으로, 불·주먹도끼·사냥·공동체·성찰 활동에도 내용과 맞는 캔버스 배경을 추가했다.
- 동굴의 모닥불과 재료, 자갈밭과 석기, 평원·동물 흔적·바위 길목, 야영지와 나눔, 해 질 녘 성찰 장면이 각각의 단서 위치와 함께 보인다.
- 외부 이미지 대신 코드 기반 벡터 장면을 사용해 저작권 확인, 네트워크 의존, iPad 로딩 부담 없이 일관된 화면을 유지한다.
- iPad 10세대 세로 기준에서 시각 점검과 단서 완료 흐름, 44px 대체 조작, 콘솔 오류 0건을 다시 확인했다.

## 2026-08-27 — Phase 32 신석기·고조선 장면 구조 확장

- 공통 핫스팟 렌더러가 `culture-touch` 모드만 보고 단색 장면을 그리던 문제를 해결하기 위해, 각 시뮬레이터 JSON의 `scene` 키를 읽도록 확장했다.
- 신석기 4단계에는 한강변 움집 마을·토기 제작·옷감 제작·정착 성찰 장면을, 고조선 4단계에는 영역 지도·8조법 기록·동검과 고인돌·자료 종합 장면을 연결했다.
- 각 장면은 실제 단서 좌표를 가리지 않고, 핫스팟 원형과 대체 조작을 그대로 공유하므로 상호작용 계약은 변경하지 않았다.
- iPad 10세대 세로 화면에서 신석기와 고조선의 장면·44px 터치 대상·선택 잠금·콘솔 오류 0건을 점검했다.

## 2026-08-27 — Phase 33 삼국·통일신라·발해 장면 맥락화

- `scene` 키를 필수 핫스팟 활동 8곳에 추가했다: 삼국 백제(한강·서해 교역로·영토), 고구려(평양성·국내성·남진 경로), 신라(북한산 순수비·한강·당항성), 통일신라 9주 5소경 행정 기록·불국사(석가탑·다보탑·청운교백운교)·석굴암(돔·불상 배치·본존불)·통치문화교류 자료 종합, 발해 유적·문서·주민 구성 자료 종합.
- `required: true`이지만 `hotspots`가 없는 게이지형 활동(삼국 4단계, 발해 1~3단계)은 공통 핫스팟 렌더러 경로를 타지 않으므로 이번 범위에서 제외했다.
- 삼국 세 장면은 정밀 지도가 아닌 모식도로 그려 방위·축척을 표시하지 않았고, 영토는 실선이 아닌 파선으로 표현해 확정된 국경으로 오독되지 않게 했다. 발해 주민 구성 장면은 집 크기와 높이를 동일하게 배치해 집단 간 우열로 읽히지 않도록 했다.
- 핫스팟 좌표·완료 조건·피드백 문구는 변경하지 않았고, 캔버스 렌더러 스모크 테스트로 8개 장면이 예외 없이 그려지고 `setLineDash` 상태가 항상 복원되는지 확인했다. 실제 브라우저 시각 점검은 Codex가 수행한다.

## 2026-08-27 — Phase 34-0 deep-dive mode 오배정 버그 수정

- `deep_joseon`(2·7), `deep_modern`(1·3·5), `deep_three_kingdoms`(1·7), `deep_prehistoric`(1a) 8개 단계가 훈민정음 창제·수원 화성 축조·만세 운동·낙동강 방어선 등 실제 내용과 무관하게 `mode: "paleo-fire"`/`"paleo-stone"`을 재사용하고 있었다. `drawSim()`과 `handleCanvasTouch()` 어느 분기에도 걸리지 않아 캔버스가 비고 게이지가 0%에서 멈추는 상태였다.
- 각 단계의 `mode`만 내용에 맞는 `culture-touch` 또는 `battle-gauge`로 교체했다. 핫스팟·완료 조건·안내 문구·렌더러 코드는 손대지 않았다.
- Node 스텁으로 `handleCanvasTouch()`를 직접 호출해 8개 모두 `gaugeProgress`가 실제로 증가하는지 확인했다. 전부 `required: false`(선택 활동)이므로 진행을 막지는 않았지만 화면이 반응하지 않는 상태였다.
- 이 수정은 화면을 "고장 → 제네릭"으로 되돌릴 뿐 시대 맥락 장면을 주지는 않는다. 장면 추가는 후속 Phase의 범위다.

## 2026-08-27 — Phase 34A 자료 종합형 11종 장면 확장

- 고려 문화·항쟁, 독립운동, 일제강점기 1·2, 조선 서민 문화·건국·신분제, 근대 개항, 광복 이후, 세종 등 11개 MUD의 "세 자료를 비교하세요" 4단계에 `scene` 키를 추가했다. 전부 서로 다른 시대인데도 동일한 초록 원 placeholder로 렌더링되던 문제를 해결했다.
- `unified-silla-summary`·`balhae-evidence`에서 쓰던 자료 카드 3장 레이아웃을 그대로 계승해, 새 화풍을 만들지 않고 시각적 일관성을 유지했다. 공통 카드 그리기를 `drawConfiguredSceneBackground` 내부의 `card()` 헬퍼로 뽑아 11개 장면이 공유한다.
- 카드 안 도형은 팔만대장경판·직지·벽란도 교역선(고려 문화)부터 훈민정음 해례본의 천지인 기하 기호·칠정산 궤도·앙부일구(세종)까지, 각 MUD의 실제 핫스팟 자료와 1:1로 대응한다.
- 민감 시대(일제강점기 1·2)는 봉인·문서·장부 등 중립적 기호로만 표현했고, 사람·가해 집단·폭력 묘사를 넣지 않았다.
- 캔버스 렌더러 스모크 테스트로 11개 장면 전부 예외 없이 그려지고 `setLineDash`가 복원되는지, JSON-JS `scene` 문자열이 27개 전부 일치하는지 확인했다. 실제 브라우저 점검은 Codex가 수행한다.

## 2026-08-27 — Phase 34B 서사형 3종 장면 확장(민감 시대 표현 원칙 적용)

- `regular_independence_army`(1~4), `regular_joseon_diplomacy`(1~4), `regular_korean_war`(1~4) 12개 단계에 `scene`을 추가했다. 다단계 서사 구조라 34A의 자료 카드 패턴 대신 단계별 전용 장면을 새로 설계했다.
- 단지동맹 태극기(2단계)는 신체 훼손을 연상시키지 않도록 작업대·조명·확대경·보호 장갑으로 구성한 유물 보존실 관찰 장면으로 그렸다. 신체·혈흔 묘사는 전혀 넣지 않았다.
- 6·25전쟁 지도(1·3단계)는 남북을 동일한 회색 반도 실루엣으로 그리고 38선·군사분계선을 중립적인 회색 파선으로만 표현했다. 정전 협상 탁자는 좌우 대칭 구성으로 그려 승패를 암시하지 않았고, 깃발·비둘기 등 상징을 넣지 않았다.
- 남한산성 전란(2단계)은 성곽 윤곽·눈 결정·빈 항아리로 겨울 농성의 어려움을 표현했고, 전투·병사·무기는 그리지 않았다. 이산가족·강제 동원과 마찬가지로 사람의 고통을 직접 묘사하지 않는 원칙을 유지했다.
- 캔버스 렌더러 스모크 테스트로 12개 장면 전부 예외 없이 그려지고 `setLineDash`가 복원되는지, JSON-JS `scene` 문자열이 39개 전부 일치하는지 확인했다. 실제 브라우저 점검은 Codex가 수행한다.

## 2026-08-27 — Phase 35 렌더러 확장과 고대 잔여 8종

- `drawSim()`에 3줄을 추가해, 핫스팟이 없는 게이지형 활동도 JSON에 `scene`을 선언하면 시대 배경을 그리도록 확장했다. `scene`을 선언하지 않은 기존 활동의 분기 순서는 전혀 바꾸지 않았다.
- `regular_three_kingdoms`(4), `regular_three_kingdoms_life`(1~4), `regular_balhae`(1~3) 8개 필수 게이지형 활동에 `scene`을 추가했다. 이 8개는 핫스팟이 없어 기존에는 `scene`을 선언해도 무효였던 활동이다.
- 가야 철제 판갑옷(3단계)은 mode 이름이 `battle-gauge`이고 안내 문구가 "탐구"라 쓰였지만 실제 내용은 제철·갑옷 제작 기술이므로 전투 없이 공방(가마·판갑옷·덩이쇠)으로 그렸다. 발해 건국(1단계)도 "결전" 문구 대신 산세·깃발·이동 경로만으로 표현해 병사·무기 묘사를 피했다.
- **회귀 검증이 이번 Phase의 핵심이다.** 실제 `drawSim()`을 호출하는 Node 테스트로, `scene`을 선언하지 않은 5개 대표 레거시 mode(`gwangbok-flag`, `economy-farm`, `text-reading`, `battle-gauge`, `culture-touch`)가 여전히 원래 placeholder 문구를 그대로 그리고 `drawConfiguredSceneBackground`를 전혀 호출하지 않는지 확인했다. 동시에 `scene`을 선언한 게이지 활동은 새 분기로 정상 라우팅되는지도 확인했다.
- 캔버스 렌더러 스모크 테스트로 8개 장면 전부 예외 없이 그려지고, JSON-JS `scene` 문자열이 47개 전부 일치하는지 확인했다. 실제 브라우저 점검은 Codex가 수행한다.

## 2026-08-27 — Phase 36 고려 4종 14개 장면 확장

- 고려 건국·사회·항쟁·문화 MUD의 필수 활동 14개에 scene 키와 캔버스 장면을 연결했다.
- 후삼국 모식도·발해 유민 수용·훈요 10조·과거·균등 상속·배흘림기둥·외교 담판·경판·금속활자·벽란도 항구 등 자료와 활동을 연결했다.
- 흥화진·귀주는 전투 승리 대신 지형·방어 거점·재건 단서로, 벽란도는 세계지도 대신 한 항구의 배·창고·화물로 표현했다.
- JSON·분기 계약·런타임 검증을 통과했으며, 실제 iPad 육안 점검은 후속 검증 대상으로 남긴다.
## 2026-08-31 — P2-03 MUD 등록 경로 구조 감사

- `_index.json` 32개 등록 항목과 커리큘럼 48차시의 매칭 상태를 대조했다.
- Regular MUD 28종 중 포털 매칭 차시는 39개이며, 도입·정리 차시 등 9개는 MUD가 없다.
- `app.js`의 레거시 Regular 조건문이 약 140줄 남아 있음을 확인했다.
- `unitId + lessonNumbers`만으로는 2단원 7차시와 3단원 12차시가 각각 두 MUD와 중복되어 첫 배열 항목을 선택하는 문제가 있다.
- `docs/plans/implementation_plan_registration_single_source.md`를 작성하고, `primary`/`supplementary` 등록 계약과 검증 선행 원칙을 기록했다.
- 이번 묶음에서는 코드와 인덱스 데이터를 변경하지 않았다. 주·보조 MUD의 교육적 선정이 먼저 필요하다.

## 2026-08-31 — P2-03 인덱스 주·보조 계약 1차 반영

- `regular_myeongnyang`과 `regular_korean_war`를 `supplementary`로 표시해 중복 차시의 기본 포털 버튼에서 제외했다.
- `app.js`의 인덱스 매칭이 `supplementary` 항목을 건너뛰도록 수정했다.
- `scripts/03_validate_mud_integrity.py`에 placement 유효성, MUD ID 중복, primary 차시 중복 검사를 추가했다.
- 검증 통과: MUD 무결성, MUD 계약, 정적 자산, 시뮬레이터 런타임, `node --check js/app.js`, `git diff --check`.
- `app.js`의 기존 Regular 레거시 조건문은 아직 남아 있어, 다음 묶음에서 인덱스 실패 시에만 제한적으로 유지하거나 제거한다.

## 2026-08-31 — P2-03 포털 Regular 매칭 단일 경로 1차 완료

- `app.js`의 기존 Regular 차시 조건문을 `_index.json` 로드 실패 시에만 실행되는 비상 호환 경로로 격리했다.
- 인덱스가 정상 로드되면 primary MUD는 인덱스 매칭만 사용하고, 인덱스에 없는 차시는 퀴즈·탐구·준비 중 분기로 처리한다.
- 새 Regular MUD는 JSON 파일과 `_index.json`만 갱신하면 포털 매칭 대상이 되는 구조를 확인했다.
- 검증 통과: MUD 무결성, MUD 계약, 정적 자산, 시뮬레이터 런타임, `node --check js/app.js`, `git diff --check`.
- 남은 확인: 보조 MUD 별도 노출, 실제 브라우저·태블릿에서 2단원 7차시와 3단원 12차시 버튼 확인.

## 2026-08-31 — P3 브라우저·기기 회귀 점검 시도

- 브라우저 연결을 시도했으나 현재 클라이언트가 설치되지 않은 `26.825.31414` 서비스 파일을 참조해 초기화에 실패했다.
- 브라우저 도구 문제로 실제 화면·태블릿 육안 검증은 수행하지 못했다.
- 코드 기반 검증은 계속 통과했으며, 브라우저 도구가 정상화되면 2단원 7차시·3단원 12차시의 primary 버튼과 대표 MUD 흐름을 우선 확인한다.

## 2026-08-31 — P3 브라우저 회귀 체크리스트 문서화

- 브라우저 연결 복구 후 바로 실행할 수 있도록 `BROWSER_REGRESSION_CHECKLIST.md`를 추가했다.
- 포털 초기 화면, 중복 차시 매칭, 대표 MUD 흐름, 터치·접근성, 기록 양식을 한 문서에 고정했다.
- 핵심 흐름 차단·잘못된 MUD 연결·터치 실패의 우선순위 판정 규칙도 기록했다.

## 2026-09-01 — P1-02 반복 탭 내성 브라우저 회귀 1차

- 로컬 Chrome에서 `regular_independence` 1단계를 열어 대체 조작 버튼을 확인했다.
- 같은 단서를 다시 누르면 버튼이 즉시 비활성화되고 진행률이 1/3에 머물러, 반복 탭만으로 완료되지 않음을 확인했다.
- 세 단서를 모두 확인하기 전 선택지가 잠기고, 모두 확인한 뒤 선택지가 활성화되는 것을 확인했다.
- 오답 선택 후 `1-1` IF 단계로 이동하고, 재시도 버튼으로 1단계에 복귀하며 단서 상태가 초기화되는 것을 확인했다.
- 이번 점검은 1단계 대표 흐름만 대상으로 했으므로 2~3단계 전체 정상·오답·IF 회귀 확인은 남겨 두었다.

## 2026-09-01 — P1-02 2~3단계 브라우저 회귀 완료

- `regular_independence`에서 1→2→3단계 정상 진입을 확인했다.
- 2단계는 단서 3개 확인 전 선택지 잠금, 완료 후 활성화, 오답→`2-1` IF→재시도→2단계 복귀를 확인했다.
- 3단계는 단서 3개 확인 전 선택지 잠금, 오답→`3-1` IF→재시도→3단계 복귀를 확인했다.
- P1-02 계획의 1~3단계 정상·오답·IF 회귀 기준을 충족했다. 실제 학생 활동 시간 측정은 별도 검토로 남겼다.

## 2026-09-01 — P1-04 민감 역사 주제 자료 흐름 브라우저 확인

- `regular_japanese_rule_2` 1단계에서 정책 문서·이름 통제·학교생활 단서를, 2단계에서 동원 기록·피해자 증언·기억 활동 단서를 확인했다.
- 2단계의 안내가 피해 재현이 아닌 기록·증언·기억 활동 중심으로 표시되고, 기존 전투 그래픽 문구가 나타나지 않는 것을 확인했다.
- 2단계의 정상 선택 후 3단계로 진입했으며, 3단계에서 사전 원고·맞춤법·언어생활 자료 단서가 표시되는 것을 확인했다.
- 이는 현재 공통 evidence 장면의 자료 흐름 확인이며, Claude의 Phase 38 고유 장면 교체 후 최종 시각 검수는 별도로 남겼다.

## 2026-09-01 — P2-01 모달 접근성 브라우저 확인

- 로컬 Chrome에서 퀴즈와 도감 모달이 `role="dialog"`와 제목 연결(`aria-labelledby`)을 제공하는 것을 확인했다.
- 퀴즈 닫기 버튼(`골든벨 퀴즈 닫기`)과 도감 닫기 버튼(`역사 도감 닫기`)에 명시적 접근성 이름이 있고, 닫은 뒤 포털로 복귀했다.
- 점검 탭의 콘솔 오류는 0건이었다. 실제 키보드 포커스 순서와 보조기기 낭독 확인은 남겨 두었다.

## 2026-09-01 — Phase 38 Codex 승인 확인

- Claude의 `4a3c577` 구현 커밋과 `88ed08c` 핸드오프 문서를 검토했다.
- Phase 38 대상 JSON 5개에서 scene 값 15개만 교체되었고, `hotspots`·`interaction`·`completion.target`·단계 구조·학습 문구의 보호 필드 차이는 0건이었다.
- JSON의 Phase 38 scene 15개가 모두 `drawConfiguredSceneBackground`에 존재하며, 전체 고유 renderer scene 키 97개와 일치하는 것을 독립 검사했다.
- 표준 데이터·MUD 무결성·계약·정적 자산·카탈로그·출처·탭 감사·런타임 검증과 JS 문법 검사 3종, 커밋 diff 공백 검사를 모두 통과했다. 활동 시간 감사는 임시 복사본에서 `32개 MUD / 28개 Regular / 14개 보완 신호`로 확인했다.
- 캐시 갱신 URL로 실제 브라우저에서 `memorial-candlelight`를 확인했다. 촛불·꽃·기록 문서 중심 장면이며 legacy `호국 결전` 문구가 없고 콘솔 오류도 0건이었다.
- 구현 커밋과 핸드오프 문서는 내용상 승인한다. 다만 현재 Codex 문서 변경분과 untracked 핸드오프 지시서가 작업 트리에 있으므로, 이번 승인에서는 push하지 않고 사용자 승인 후 원격 반영한다.

## 2026-09-01 — P2-03 중복 차시 primary MUD 브라우저 확인

- 2단원 7차시 포털에서 `regular_joseon_diplomacy`만 표시되고 보조 `regular_myeongnyang`은 기본 목록에서 제외되는 것을 확인했다.
- 3단원 12차시 포털에서 `regular_post_war`만 표시되고 보조 `regular_korean_war`는 기본 목록에서 제외되는 것을 확인했다.
- 두 primary 버튼을 실제로 눌러 각 MUD 화면에 진입했으며, 점검 탭 콘솔 오류는 0건이었다.
- 별도 보조 MUD 노출과 태블릿 육안 검증은 후속 확인 대상으로 남겼다.

## 2026-09-01 — P2-01 키보드 모달 접근성 보강

- 퀴즈·도감 모달을 연 뒤 닫기 버튼으로 포커스를 이동시키고, Tab 키가 모달 내부 요소만 순환하도록 `app.js`에 공통 키보드 처리를 추가했다.
- Escape 키로 열린 모달을 닫고, 모달을 연 원래 버튼으로 포커스를 복귀하도록 했다.
- `index.html`의 `app.js`에도 캐시 버전 쿼리를 추가해 브라우저가 이전 접근성 코드를 계속 사용하는 문제를 방지했다.
- 로컬 Chrome에서 퀴즈 모달의 닫기 버튼 포커스, Tab 내부 이동, Escape 닫기, 트리거 복귀를 확인했으며 콘솔 오류는 0건이었다.
- 실제 보조기기 낭독과 색상 없이 상태를 구분하는 검증은 후속 확인 대상으로 남겼다.

## 2026-09-01 — P2-02 유물·보상 설명 2차 대조

- 감사 대상 `art_1`, `art_3`, `art_4`, `art_15`, `art_16`, `art_18`의 현재 설명을 계획서의 수정 방향과 대조했다.
- 과장·단정 표현은 완화된 상태이며, 유물 ID·이름·등급·보상 연결은 변경하지 않았다.
- 유물·보상 무결성, MUD 계약, 출처 검증을 재실행해 모두 통과했다.
- 추가 코드·데이터 수정은 하지 않았으며, 역사 사실에 대한 교사 최종 검토 대기 상태를 유지한다.
## 2026-09-01 — 사용자 1차 콘텐츠 승인 및 후속 검토 항목

- Phase 38 근현대 15개 장면은 역사 상황 표현이 대체로 교과서와 유사하고, 전체 그래픽 구성 방향이 적절하다는 사용자 검토를 받았다.
- 선택지는 정상 작동하는 것으로 확인했다. 모바일 화면 검토는 추후 진행한다.
- 민감한 역사 내용은 대체로 적절하다는 검토를 받았다.
- 그래픽은 학습 내용 확정 후 단계별로 미세 조정할 수 있도록 남겼다.
- 유물·보상 관련 이미지는 설명과 완전히 다른 사례가 많아 별도 정합성 검토 항목으로 등록했다.
## 2026-09-01 — 민감 역사 glossary 표현 완화

- 사용자 승인에 따라 `regular_japanese_rule_2`의 glossary 2건만 수정했다.
- 일본군 ‘위안부’ 정의는 사실을 유지하면서 “성적 착취를 한 중대한 인권 범죄”로 표현을 완화했다.
- 조선어학회 사건 정의는 사실을 유지하면서 “가두고 심하게 고문해 … 목숨을 잃은 사건”으로 표현을 완화했다.
- `instruction`·`infoText`·`feedback`·`narrative`·`completion`·`simulator`와 장면 렌더러는 변경하지 않았다.
## 2026-09-01 — IF 스테이지 6개 1차 내용 검토

- `regular_korean_war:3-1`, `regular_joseon_diplomacy:1-1/2-1/3-1`, `regular_independence_army:3-1`, `regular_goryeo_culture:1-1`의 현재 `narrative`와 재시도 선택지를 검토했다.
- 6개 모두 자료 재확인·비교로 복귀하며, 가상 실패 결과를 역사적 사실처럼 단정하거나 폭력·감정을 정답으로 강요하는 표현은 발견하지 못했다.
- 단계별 사용자 최종 확인 포인트는 `codex_if_stage_review.md`에 기록했다. 문구 수정은 사용자 승인 전까지 보류한다.
## 2026-09-01 — 유물 아이콘·보상 토스트 정합성 수정 확인

- Claude가 승인된 `art_3`, `art_6`, `art_10`, `art_11`, `art_12`, `art_13`, `art_deep_2`의 아이콘을 유물명에 맞게 교체했다.
- `js/mudEngine.js`의 MUD 완료 보상 토스트가 `artifactId` 대신 사람이 읽는 유물명을 표시하도록 수정했다.
- 커밋 `a2ad6eb`의 변경 범위와 Codex 재검증 결과를 확인했다. 게임 데이터·MUD 무결성·계약·정적 자산·카탈로그·출처·시뮬레이터 런타임·JavaScript 문법 검증이 모두 통과했다.
- `art_19`, `art_22` 등 정확한 한국식 성곽·궁궐 이모지가 없는 낮은 위험도 항목은 변경하지 않았다.

## 2026-09-01 — 접근성 상태 표현 코드 점검

- 선택지 잠금 상태가 네이티브 `disabled`와 `aria-disabled`로 함께 전달되고, 선택지에는 색상 외에 A/B/C 문자 표식이 있음을 확인했다.
- 시뮬레이터 피드백·진행률은 `role="status"`와 `aria-live`로 읽히도록 구성되어 있다.
- 실제 보조기기 낭독과 색상 없이 완료·비활성 상태를 구분하는 기기 검증은 사용자 환경 점검으로 남겼다.

## 2026-09-01 — 테마 피드백 배너 명암 대비 보강

- 어두운 MUD 테마에서 `mn-canvas-feedback`의 어두운 글자가 거의 보이지 않던 공통 대비 문제를 수정했다.
- 테마 배경색과 흰색·어두운 글자색의 대비를 계산해 더 높은 대비의 글자색을 선택하도록 했다.
- 기존 진행 상태·문구·시뮬레이터 구조는 변경하지 않았다.
- 375×812 모바일 Chrome에서 어두운 `regular_japanese_rule_1`과 밝은 `regular_paleolithic`을 확인했으며, 두 화면 모두 배경에 맞는 흰색 글자와 콘솔 오류 0건을 확인했다.

## 2026-09-01 — 반복 탭·활동 시간 감사 수치 구분 정정

- `scripts/09_audit_tap_resistance.py` 재실행 결과 반복 탭 구조 후보는 40개임을 확인했다.
- 활동 시간 감사의 Regular 보완 신호 16종과 반복 탭 구조 후보 40개는 서로 다른 지표이므로 분리해 기록했다.

## 2026-09-01 — 반복 탭 후보 40개 우선순위 분류

- 구조 감사 후보 40개를 scene 보유 36개와 Codex가 별도 확인할 scene 없는 4개로 분리했다.
- 우선 확인 대상은 `regular_gwangbok:3`, `regular_myeongnyang:4`, `regular_bronze_age:1/4`이며, 단일 버튼·특수 슬라이더를 일반 반복 탭 활동과 동일하게 일괄 수정하지 않도록 했다.
- 상세 판단 기준과 다음 조치는 `codex_tap_resistance_priority.md`에 기록했다.

## 2026-09-01 — 중간 점검 문서 동기화 및 후속 대기 항목 정리

- Claude의 IF 6개 최종 검토를 반영해 `regular_joseon_diplomacy:2-1`의 `척화·주화` 용어 glossary 추가 여부를 사용자 확인 대기로 남겼다.
- `regular_sejong`은 2단계와 3단계 simulator에 각각 중복 `instruction`·`feedback`이 있음을 확인했으며, 자료 중심의 앞 문구를 유지하고 뒤 문구를 삭제하는 안을 사용자 승인 대기로 남겼다.
- `BACKLOG.md`, `project_context.md`, IF·등록 경로·Phase 38 계획서의 완료 수치와 상태를 최신 커밋 기준으로 동기화했다.
- IF 6개 사용자 최종 승인, 세종 중복 키 결정, 실제 태블릿·보조기기·학생 활동 시간 측정은 후속 검토 항목으로 유지한다.

## 2026-09-01 — IF 승인 및 세종 simulator 중복 키 정리

- 사용자가 IF 6개를 현행 유지 승인했다. `regular_joseon_diplomacy:2-1`의 “척화·주화” glossary 보강은 별도 보류한다.
- `regular_sejong` 2·3단계 simulator의 뒤쪽 중복 `instruction`·`feedback` 4줄을 삭제하고, hotspot과 직접 연결되는 자료 중심 문구를 유지했다.
- `completion.target`, hotspots, `infoText`, 단계 구조와 선택지 흐름은 변경하지 않았다.

## 2026-09-01 — 학생 실제 활동 시간 관찰 기록

- 수업 종료 10분 전 1단원 2차시 `regular_paleolithic`과 3차시 `regular_neolithic`을 연속 제시했다.
- 학생들이 두 활동을 모두 4분 이내에 완료했으며, 기존 설계 추정치와 구분되는 실제 관찰값으로 `activity_duration_audit.md`에 기록했다.
- 빠른 완료 과정에서 학생들이 오타 또는 문맥 문제를 발견해 교사에게 제시했다.
- Regular를 일괄적으로 늘리기보다 빠른 학생용 선택형 유물 확장 활동을 검토 대상으로 남겼다.
- 표본 수와 단계별 소요 시간이 없어 P1-02 전체 완료로 처리하지 않았다.

## 2026-09-01 — 1단원 3차시 신석기 MUD 문맥 정합성 수정

- 학생 제보를 따라 `regular_neolithic` 2~3단계의 설명·선택지·시뮬레이터 연결을 대조했다.
- 2단계는 토기 제작, 3단계는 가락바퀴·뼈바늘을 사용하는 의생활 활동으로 구조가 정해져 있음을 확인했다.
- 2단계와 2-1의 농기구 관련 narrative를 토기 제작 문맥으로, 3단계와 3-1의 토기 관련 narrative를 의생활 문맥으로 최소 수정했다.
- 선택지의 `next`, simulator, completion, 보상과 단계 구조는 변경하지 않았다.
- JSON 데이터·MUD 무결성·계약·출처·시뮬레이터 런타임·`git diff --check` 검증을 통과했다.

## 2026-09-01 — 선택지 문장 길이·정답 단서 감사

- `scripts/12_audit_choice_bias.py`를 추가해 다중 선택 결정 단계 124개를 감사했다.
- 정답 위치는 124개 모두 원본 배열 1번이었으나, 런타임 표시 무작위화가 이미 적용되어 위치 단서는 실행 중 완화된다.
- 정답 문장이 오답 평균보다 10자 이상 긴 단계 40개, 모든 오답보다 긴 단계 115개를 확인했다.
- 학생의 “긴 문장만 고르면 정답” 반응과 일치하는 정량 신호로 판단하고, 상위 단계부터 교사 검토 후 문장 보정을 진행하기로 했다.
- 이번 작업에서는 JSON 선택지 문구를 일괄 수정하지 않았다.

## 2026-09-01 — 선택지 품질 보정 1차 및 브라우저 회귀

- `regular_gwangbok:1`, `regular_myeongnyang:1`, `regular_goryeo_society:1`의 선택지 문장을 질문과 연결되도록 보정했다.
- `correct`, `next`, `sound`, 시뮬레이터와 보상 구조는 변경하지 않았다.
- 선택지 감사에서 정답이 오답 평균보다 10자 이상 긴 단계가 40개에서 34개로, 모든 오답보다 긴 단계가 115개에서 114개로 감소했다.
- Chrome에서 `regular_gwangbok:1`의 시뮬레이터 완료 후 정상 선택→2단계 진입과 2단계 오답→`2-1` IF 이동을 확인했으며 오류·경고 로그는 없었다.
- 회귀 중 2단계 질문이 5·10 총선거인데 선택지는 건국준비위원회이고, `2-1`은 선거권을 설명하는 별도 문맥 불일치를 발견해 BACKLOG로 분리했다.

## 2026-09-01 — 광복 MUD 2단계 문맥 정합성 수정

- `regular_gwangbok:2`의 건국준비위원회 활동과 어긋나던 총선거 설명을 광복 직후 치안 유지·건국 준비 설명으로 수정했다.
- `regular_gwangbok:2-1`을 불평등 선거권 설명에서 치안 공백과 건국준비위원회 재확인 설명으로 수정했다.
- 2단계 선택지의 문장 길이와 의미 형식을 함께 균형화했으며, 3단계 총선거 흐름은 유지했다.
- Chrome에서 1단계 시뮬레이터 완료→정상 선택→2단계 진입을 확인했고, 수정된 2단계 문구와 선택지 표시를 확인했다.
- 브라우저 오류·경고 0건, JSON·MUD 무결성·계약·출처·시뮬레이터 런타임·선택지 감사·`git diff --check`를 통과했다.

## 2026-09-01 — 고려·조선 선택지 문맥 정합성 수정

- `regular_goryeo_society:3`의 제사 설명을 부석사 무량수전 배흘림기둥 탐구로 정렬하고, `3-1` IF도 배흘림기둥 원리 재확인으로 맞췄다.
- `regular_joseon_founding:2`의 경복궁 이름 설명을 한양 4대문과 인의예지 탐구로 정렬하고, `2-1` IF도 성문 이름의 유교 덕목 재확인으로 맞췄다.
- 선택지의 `correct`, `next`, `sound`, 시뮬레이터, 보상과 단계 구조는 변경하지 않았다.
- JSON·MUD 무결성·계약·출처·선택지 편향·시뮬레이터 런타임·JavaScript 문법·`git diff --check` 검증을 통과했다.

## 2026-09-01 — 학생 피드백 기반 기획 기준선 갱신

- `PRD.md`, `TECH_STACK.md`, `DECISIONS.md`, `BACKLOG.md`, `agents.md`, `README.md`, `project_context.md`에 실제 교실 기반 반복 개선 사이클을 반영했다.
- 4분 이내 완료 관찰을 선택지 난이도·시뮬레이터 완료 조건·읽기 속도·교실 내 정보 공유로 분리해 진단하도록 정리했다.
- 최소 플레이 로그는 개인정보 없이 브라우저 내부에서 검토하고, 유물은 수집 보상에서 관찰·비교·주장·근거 추론 자원으로 확장하는 방향을 기록했다.
- 친구 비교·협력과 역사 타이쿤은 서버 없는 작은 실험 이후로, 백엔드·DB·로그인·실시간 멀티플레이·공개 순위표는 `Future / Not Now`로 경계를 명시했다.
- 선택지 편향 최신 감사 수치(124개 결정 단계, 34개·114개)를 `BACKLOG.md`와 `docs/audits/choice_bias_audit.md`에 동기화했다.
- 검증: `python scripts/12_audit_choice_bias.py`, `git diff --check` 통과.

## 2026-09-01 — 게임성·Vercel·Supabase 점진 도입 기준선 갱신

- `PRD.md`와 `USER_FLOWS.md`에 탐험·발견·자료 추론·유물 활용·근거 공유를 연결하는 장기 플레이 루프를 추가했다.
- 빠른 완료 문제는 필수 경로를 무조건 늘리지 않고 선택 탐험과 추가 역사 단서로 확장하는 방향을 기록했다.
- `TECH_STACK.md`, `DECISIONS.md`, `project_context.md`, `README.md`, `BACKLOG.md`, `agents.md`에서 현재 GitHub Pages 운영과 목표 Vercel 배포를 구분했다.
- Supabase는 `play_events` 익명 로그부터 검토하고, localStorage는 즉시 상태·복원·네트워크 장애 대응으로 유지하도록 정리했다.
- 개인정보 최소화, RLS, secret/service-role 키 비노출, Anonymous Auth·Realtime·역사 타이쿤의 후순위 게이트를 문서화했다.
- 이번 단계에서는 Supabase 프로젝트·테이블·Vercel 설정·Auth·Realtime·플레이 로그 코드를 구현하지 않았다.

## 2026-09-01 — 중앙 INBOX 및 문서 승격 workflow 도입

- 사용자가 아이디어·수업 관찰·기술 검토를 `INBOX.md` 한 곳에 기록하도록 중앙 입력 창구를 만들었다.
- `EXPERIMENTS.md`, `ROADMAP.md`, `ARCHITECTURE.md`를 추가해 수업 실험, 구현 순서, 시스템 연결 구조를 분리했다.
- `agents.md`, `README.md`, `project_context.md`에 INBOX 원문 보존과 역할별 문서 승격 규칙을 연결했다.
- 판단이 어려운 내용은 확정하지 않고 INBOX에 보존하며, 기존 문서와 충돌하면 사용자에게 보고하도록 규칙화했다.
- 현재 저장소의 루트 문서 링크를 보존하기 위해 `docs/`로 파일을 이동하지 않았다.

## 2026-09-01 — 다중 에이전트 작업 규칙 명시

- `agents.md`에 Codex CLI와 Claude Code의 동일 저장소 작업 원칙을 추가했다.
- 동일 파일 동시 수정 금지, 구현 에이전트와 검토 에이전트 분리, 병렬 작업 시 branch/worktree 우선 원칙을 명시했다.

## 2026-09-01 — 문서 상태 대조 1차 정리

- `BACKLOG.md`에서 이미 해결된 `regular_gwangbok` 문맥 정합성 항목을 미완료 후속 목록에서 제거했다.
- IF 스테이지 6개는 사용자 최종 승인 완료 상태이므로 `사용자 승인 대기` 표현을 정정했다.
- 구현 계획서 파일 자체의 삭제·이동·병합은 수행하지 않고, 상태가 불명확한 문서는 보류했다.

## 2026-09-01 — Claude 문서 정리 감사 반영

- Claude의 `document_cleanup_priority_audit.md`를 인수하고, 실제 `origin/main` 이력과 대조했다.
- 문서 재배치 계획서와 재배치·Phase 37·Phase 38 핸드오프의 오래된 push 보류 상태를 현재 원격 반영 상태로 정정했다.
- 반복 탭 내성 계획서의 실제 반영 수치를 5개 MUD로 통일했다.
- 유물 복원 퍼즐 계획은 코드 미구현이므로 활성 작업으로 유지했으며, 문서 삭제·이동·병합은 수행하지 않았다.

## 2026-09-01 — 선택지 무작위화 계획 상태 정정

- `implementation_plan_choice_randomization.md` 상단에 2026-08-26 구현·검증 완료 상태를 명시했다.
- `js/mudEngine.js`의 선택지 복사본 Fisher–Yates 셔플과 기존 분기 보존 결과를 계획서와 대조했다.
- 선택지 길이·구체성 편향은 무작위화와 다른 문제이므로 `implementation_plan_choice_quality_batch.md`와 별도 감사 문서에서 계속 관리한다.

## 2026-09-01 — 과거 플레이타임 확장 계획 상태 정정

- `implementation_plan_mud_depth_expansion.md` 상단에 `superseded` 상태를 명시했다.
- 과거 계획의 전면적 장면 확장·자원값·분기 구조는 현재 실행 기준으로 사용하지 않도록 하고, 탐험·발견의 최소 실험은 `EXPERIMENTS.md`와 `ROADMAP.md`로 연결했다.
- 과거의 정량 진단과 설계 근거는 삭제하지 않고 기록으로 보존했다.

## 2026-09-01 — 삼국 심화 인터랙티브 화면 내용 정합성 개선

- `deep_three_kingdoms`의 황산벌, 매소성, 기벌포, 천문령 장면을 사건별 화면으로 분리했다.
- 매소성은 산성·장창 방진·보급로를, 기벌포는 금강 하구 갯벌·당나라 수군·신라 대장선을 보여 주도록 구성했다.
- 삼국 심화의 문서형 시뮬레이터에는 남북국·삼국 통일 전쟁의 단계별 제목과 단서를 표시하도록 했다.
- 기존 명량대첩 화면과 대한국새 화면이 삼국 심화에 재사용되던 문제를 전용 렌더러 분기로 차단했다.
- `node --check`, MUD 계약 검사, 시뮬레이터 런타임 검사를 통과했고 브라우저에서 황산벌부터 기벌포까지 진행을 확인했다.

## 2026-09-02 — Track A 런타임 안정화

- 시뮬레이터가 없는 IF 단계에서도 이전 활동 상태가 남지 않도록 `renderStage()`와 `setupSimulator()`의 초기화 경계를 정리했다.
- 시뮬레이터 없는 단계에서는 인터랙티브 카드·위젯·대체 조작을 숨기고, 다음 시뮬레이터 단계에서는 카드와 진행 상태를 복원하도록 했다.
- `gwangbok-flag`·`gwangbok-vote` 캔버스 직접 조작도 대체 버튼과 동일하게 `simulatorProgress`와 완료 판정을 갱신하도록 수정했다.
- 스테이지 전환 시 `simActionIds`를 초기화하고, 해당 상태·캔버스 진행·IF 카드 숨김 회귀 테스트를 `scripts/05_test_simulator_runtime.js`에 추가했다.
- 브라우저에서 `regular_balhae` 정상 완료 → 오답 `1-1` IF → 재시도 흐름을 확인했다. 카드가 `display: none`/`block`으로 올바르게 전환되고 재시도 시 게이지가 0%로 초기화되었으며 콘솔 오류·경고는 0건이었다.
- 검증: `node scripts/05_test_simulator_runtime.js`, 전체 JSON·MUD 계약·자산·카탈로그·출처·탭 내성·선택지 감사, `node --check` 4종, `git diff --check` 통과.
## 2026-09-03 — Deep-dive 시간 기준 및 협동 파일럿 승인 게이트 정리

- `0ca0984`~`e74a792` 핸드오프 4개 커밋을 현재 `codex-deep-three-kingdoms` 브랜치에서 대조했다. Deep-dive 4종의 `playTime`은 구조적 추정 범위로 갱신되어 있으며, 실제 수업 관찰 전 확정값으로 취급하지 않는다.
- `PRD.md`의 Deep-dive 정의를 여러 차시(`lessonNumbers`) 연결 중심으로 바꾸고, 특정 시간에 맞추기 위한 문제 수 증가는 하지 않도록 정리했다.
- `DECISIONS.md`에 D-018을 추가해 단일 세션 시간을 강제하지 않고 교실 실측으로 갱신하는 원칙을 명문화했다.
- 협동 MUD 파일럿은 아직 구현 승인하지 않았다. §10에 4인 역할의 물리적 분리, 실제 수업 시간 리허설, `deep_joseon` 한산도 소재 재사용 의도 확인을 승인 게이트로 추가했다.
- 검증: `git diff --check` 통과.
## 2026-09-03 — 최신 브랜치 정적 회귀 검증

- `00a924f`를 포함한 현재 `codex-deep-three-kingdoms` 기준으로 게임 데이터·MUD 무결성·계약·정적 자산·카탈로그·출처·IF/유물 감사·시뮬레이터 런타임·JavaScript 문법 검사를 재실행해 모두 통과했다.
- 선택지 원본의 정답 위치 편향 경고는 기존 런타임 셔플 전제 경고이며, 계약 검사 실패는 아니다.
- 감사 스크립트 2종이 루트에 보고서를 생성하고 기준 링크는 `docs/audits/`를 가리키는 불일치를 발견해 `BACKLOG.md`의 P2 후속 항목으로 등록했다. 경로 변경은 별도 계획·승인 전까지 수행하지 않는다.
- 검증: `python scripts/01_validate_game_data.py`, `03_validate_mud_integrity.py`, `04_validate_mud_contract.py`, `06_validate_static_assets.py`, `08_validate_mud_catalog.py`, `09_validate_mud_sources.py`, `10_audit_if_stages.py`, `11_audit_artifacts.py`; `node scripts/05_test_simulator_runtime.js`; `node --check` 3종; `git diff --check`.

## 2026-09-03 — Regular MUD 활동 게이팅 정합성 개선

- 작업 claim `TASK-REGULAR-MUD-GATING | data/mud/regular_*.json + walkthrough.md | implementation agent`를 `DONE`으로 종료한다.
- 실행 지시서 `docs/handoff/codex_regular_mud_activity_gating_instruction.md`에 따라 Regular MUD 28종의 판단 스테이지를 전수 대조했다.
- 판단 스테이지 101개에서 게이팅 없음 41개를 `hotspot-discovery`로 전환하고, 기존 `hotspot-discovery` 56개에 `ordered-hotspot`과 서술상 원인→상황→결정 순서의 `sequence`를 추가했다. 최종 집계는 `hotspot-discovery 41`, `ordered-hotspot 58`(기존 2개 포함), `resource-allocation 2`, `none 0`이다.
- 우선순위 1 단계에는 narrative에 이미 있는 자료·행동 근거를 3개 hotspot으로 분리하고 `required: true`, `target/minActions: 3`, `progressKey: "simulatorProgress"` 계약을 적용했다. 광복·명량·청동기 단계의 기존 버튼/게이지 우회 경로는 정보 확인형 hotspot으로 교체했다.
- `regular_paleolithic:6`은 판단 선택지가 없는 최종 성찰 단계이므로 필수 조작을 선택 조작으로 낮춰 D-007 추정 시간을 10.2분에서 9.9분으로 줄였다. Regular 최대 추정 시간은 9.9분이며 10분 초과 파일은 0개다. 감사 결과의 잔여 `8/28` 경고는 10분 초과가 아닌 기존 반복 탭 내성 후속 신호다.
- 선택지의 `text`/`next`/`correct`/`sound`, narrative, glossary는 변경하지 않았고 신규 장면 삽화나 엔진 코드는 추가하지 않았다. 순서·ID·완료 계약 정합성 점검용 `scratch/23_audit_regular_gating.py`를 추가했다.
- Chrome 로컬 브라우저에서 `regular_goryeo_founding`의 자료 확인 전 선택지 비활성 상태를 확인했고, `regular_gojoseon`에서 마지막 단서 선입력 거부와 올바른 3단계 순서 통과를 확인했다. 브라우저 콘솔 오류는 0건이었다.
- 배치별로 `01_validate_game_data.py`, `03_validate_mud_integrity.py`, `04_validate_mud_contract.py`, `07_audit_activity_duration.py`, `node scripts/05_test_simulator_runtime.js`를 실행해 통과했으며, 최종 정적 게이팅 감사와 `git diff --check`도 통과했다.

## 2026-09-04 — 고조선 8조법 DB 없는 협동 MUD v0.1 구현

- 작업 claim `TASK-20260904-02 | cooperative-mud/ + 구현 계획 | implementation agent`를 완료했다.
- 프로젝트명은 **고조선 8조법: 우리 마을의 첫 번째 법**으로 확정했다. 기존 개인형 `regular_gojoseon`과 분리된 `cooperative-mud/` 허브 및 `gojoseon-law/` 독립 정적 페이지를 추가했다.
- 19명 수업을 위한 4·4·4·4·3 모둠과 좌석 선택, 네 역할의 비대칭 정보, 실제 말로 공유했음을 표시하는 로컬 버튼을 구현했다. 5모둠은 빠진 역할의 단서를 추가 증거 단계에서 공용으로 공개한다.
- 사건 1 최초 판단 → 추가 증거 → 판단 유지/변경 → 10분형 사건 2 → 모둠 법 조합 → 실제 전해지는 범금 8조 비교 → 개인 기록 요약 흐름을 구현했다. 5분형은 사건 2를 생략한다.
- 로그인·DB·실시간 동기화·온라인 채팅·개인정보 수집은 포함하지 않으며, 최초 판단·수정 판단·변경 여부·법 조합은 기기별 `localStorage`에만 저장한다.
- 메인 페이지에는 협동 MUD 허브로 이동하는 작은 진입 카드만 추가하고, 상세 화면은 별도 경로로 분리했다. 터치 버튼·키보드 포커스·반응형 레이아웃을 적용했다.
- 검증: 로컬 Chrome에서 5모둠 3번 10분형 전체 진행, 1모둠 1번 5분형 사건 2 생략, 브라우저 콘솔 오류·경고 0건 확인. `node --check` 2종, 게임 데이터·MUD 무결성·계약·정적 자산·카탈로그·출처·시뮬레이터 런타임 검사, `git diff --check` 통과.

## 2026-09-04 — 고조선 협동 MUD 가변 모둠 인원·글꼴 정합성

- 작업 claim `TASK-20260904-03 | cooperative-mud/gojoseon-law | implementation agent`를 완료했다.
- 고정된 `4·4·4·4·3` 구조를 제거하고, 교사가 정한 모둠 번호와 별개로 학생이 실제 모둠 인원 `3명·4명·5명`을 선택하도록 변경했다. 선택한 인원수만큼 좌석 버튼을 생성한다.
- 3인 모둠은 가족 사정 단서를 추가 공개하고, 5인 모둠은 다섯 번째 역할 `마을 기록자`를 배정한다.
- 협동 MUD의 본문·제목 글꼴을 메인 페이지와 동일하게 `Pretendard`·`SchoolSafetyNotification`으로 통일했다.
- 검증: 5인 모둠 5번 역할, 3인 모둠 추가 단서, 인원 미선택 입장 제한, 브라우저 계산 글꼴, 콘솔 오류·경고 0건 확인. `node --check` 2종과 `git diff --check` 통과.

## 2026-09-06 — 실시간 협동 MUD 기초 설계 검토안

- `TASK-20260906-01`(기획·통합) 및 `TASK-20260906-02`(독립 읽기 감사)를 DONE으로 종료했다. 완료 범위는 문서 설계·검토이며 실시간 앱 구현은 미착수다.
- 사용자 첨부 원문과 인터뷰 답변을 INBOX에 보존했다. 21명·40분·활동별 무작위 편성·본인 학급 우선을 반영하고, 4·4·4·4·5인 기본 추천과 3인 포함 대안을 비교했다.
- `docs/plans/COLLABORATIVE_MUD_PLAN.md`, `COLLABORATIVE_MUD_ARCHITECTURE.md`, `COLLABORATIVE_MUD_MVP.md`에 기존 코드 근거, 별도 앱/저장소 추천, 정보 배분, 교사·학생 흐름, Convex 스키마·인덱스·권한·전이·복구, 첫 수직 슬라이스와 검증 기준을 작성했다.
- 기존 고조선 정적 v0.1이 이미 존재하고 3/4/5인을 지원하지만 모둠 수 5개·로컬 저장·공유 후 최초 판단이라는 차이를 기록했다. 기존 Supabase 후보·과거 19명 수업 기록·기존 앱 코드는 변경하지 않았다.
- 독립 검토에서 발견한 미완 결과 스키마, pause 중 교사 정리 권한, 사건 2의 MVP/후속 범위 불일치를 보완했다. 후속 구현은 BACKLOG P2-COLLAB-01에서 관리한다.
- 검증: `git diff --check`, PowerShell 문서 상대 링크·코드 펜스 검사, INBOX 첨부 원문 보존 대조. 이번 변경은 문서만이므로 앱 실행·배포·실제 수업 효과를 검증했다고 주장하지 않는다.

## 2026-09-06 — Vercel 통합 운영 재검토·제품 관점 보충

- `TASK-20260906-03`(기획·통합), `TASK-20260906-04`(읽기 감사) DONE. 사용자 후속 원문을 INBOX에 기록하고 기존 계획 §10~11에 답변 근거를 보충했다.
- GitHub 저장소·Vercel 프로젝트·사용자 주소·Convex 백엔드를 구분했다. 추가 운영 맥락에서는 같은 저장소·두 Vercel 프로젝트를 우선 검토하며, 한 프로젝트에서 기존 정적 자산과 Next 화면을 함께 제공하는 대안도 기록했다. 기존 별도 저장소 권고가 기술적 필수가 아님을 세 문서에서 명확히 했다.
- 실제 배포 포털과 고조선 첫 화면을 관찰하고, 오늘 활동 진입·학생용 용어·대표 선택지의 근거 요구·공동 상태 활용에 관한 개발·디자인·교육 의견을 기록했다. 기존 복습 활동의 시간과 40분 협동 수업 예산을 구분했다.
- 검증: 공식 문서 대조, 독립 읽기 감사, 브라우저 대표 화면 관찰, `git diff --check`, 문서 링크·코드 펜스 검사. 앱 코드·배포 설정·저장소 구조는 변경하지 않았다.

## 2026-09-06 — 문서 상태 정정과 실제 수업 관찰 기록

`TASK-20260906-03`(기획·통합). 이번 작업은 문서만 변경했고 앱 코드·MUD JSON은 수정하지 않았다.

- **누락된 실제 수업 기록을 복구했다.** 고조선 협동 MUD v0.1은 2026-09-04에 실제로 수업 운영되었으나 `EXPERIMENTS.md`·`walkthrough.md` 어디에도 결과가 없었다. 교사 구술 관찰을 `EXP-006`으로 기록하고, 원문은 `INBOX.md` 2026-09-06 항목에 보존했다. 확인된 것(대면 대화 발생, 모둠 결과 다양성, 인원 변동 대응)과 검증되지 않은 것(판단 변경 가설)을 구분했다.
- **백엔드를 하나로 정리했다.** `D-019`로 실시간 협동 MUD의 Convex 채택과 별도 앱 분리를 명문화하고, `D-020`으로 `D-017`의 Supabase `play_events`를 보류했다. `TECH_STACK.md`·`ARCHITECTURE.md`·`ROADMAP.md`의 Supabase 서술에 보류 표시를 달았다. 개인정보·키 관리 원칙은 백엔드와 무관하게 유지한다.
- **`BACKLOG.md` P2-COLLAB-01에 실측 근거를 붙였다.** "교사가 모둠 상황을 볼 수 없음"이 실시간 앱의 1순위 요구 근거임을 명시하고, 후속 항목 P2-COLLAB-02(v0.1 화면 순서 수정)·03(시나리오 확장)·04(역사 비교 화면 대응)를 분리했다.
- **`docs/plans/README.md` 인덱스를 신설했다.** 협동 MUD 계열 8개 문서의 관계와 현재 실행 기준을 한 표로 정리하고, 상태별로 전체 계획서를 분류했다. 명명 규칙(`cooperative`/`collaborative` 혼용 사유)도 기록했다.
- **stale 상태를 정정했다.** 한산도 파일럿은 `draft/승인 대기` → `보류(2026-09-03)`, 선사시대 파일럿은 `in-progress` → `준비 완료·미실행`, 고조선 v0.1은 `COMPLETED` → `COMPLETED·실제 수업 운영됨`으로 바꿨다. `implementation_plan_mud_depth_expansion.md`(superseded)를 `docs/archive/`로 옮겼다.
- **설계 문서의 전제를 정정했다.** `COLLABORATIVE_MUD_PLAN.md` §1에 실측 근거 표를 추가해, 세 전제(비대칭 정보→대화, 교사 대시보드 필요, 예정/참석 인원 분리)가 관찰로 뒷받침됨과 판단 변경 가설이 아직 미검증임을 구분했다.
- 미확보 입력: 사용자가 Claude와 별도로 만들던 "다른 활동 아이디어" 문서는 이 저장소와 발행 아티팩트에 없다. `BACKLOG.md`에 미확보 항목으로 남겼다.
- 검증: `git diff --check`, 신규·수정 문서의 상대 링크 존재 확인, `docs/archive/` 이동 후 참조 경로 확인.

## 2026-09-06 — QR 입장·학생 활동명 설계 반영

`TASK-20260906-04`(기획·통합). 문서만 변경했다.

- 학생 입장을 **QR 우선**으로 바꿨다. 기존 설계의 "학생 21명에게 1회 입장 코드 배부"를 폐지하고, 교실 앞 QR 1개 → 대기 명단 익명 슬롯 → 교사 편성 확정 시 `entryLocked` 잠금 흐름으로 대체했다. 학생 인증 주체를 만들지 않게 되어 **Convex Auth(beta) 의존이 첫 수직 슬라이스의 차단 요인에서 빠졌다.**
- QR과 기존 "토큰·코드를 URL에 넣지 않는다" 원칙의 충돌을 정리했다. QR에는 세션 입장 주소와 수업 코드까지만 담고, 개인 슬롯·역할·단서는 입장 후 서버가 부여한다. 기기 교체용 교사 발급 복구 코드는 그대로 유지한다.
- **학생 활동명(형용사 + 인물)** 을 §2-2로 신설했다. 표시 이름을 `S07` 같은 시스템 코드가 아니라 당시 인물 표현으로 바꿔 몰입과 상호 호명을 얻는다.
- 활동명과 역할을 **분리**하도록 설계했다. v0.1의 역할 이름이 이미 인물 형태라, 학생이 이름으로 역할을 고르면 비대칭 정보 배분이 무너진다. 역할은 서버가 먼저 배정하고 학생은 그 역할 안에서 형용사만 고른다.
- 안전 규칙을 명시했다: 자유 입력 금지·목록 선택만, 상황·장소 중심 형용사만(성격·외모·능력 배제), 세션 내 중복 제외, 30초 내 선택, 교사 화면에 모둠·번호 병기, 세션 한정 폐기.
- `players` 스키마에 `displayNameId`를 추가하고 `joinCodeHash`를 선택 필드로 낮췄다(QR 입장에서는 미사용, 복구 코드에만 사용).
- 학교 Wi-Fi가 기존 활동에서 문제 없었음을 확인해 MVP §11의 기술 위험 하나를 낮췄다. 21대 동시 접속·실시간 구독은 여전히 미실측이다.
- 콘텐츠 확인 항목: 고조선에 정기 `시장`이 있었다고 단정하기 어렵다. 활동명 예시 `시장에 가던 사람`은 구현 전 자료로 확인한다.
- 검증: `git diff --check`, 수정 문서의 상대 링크 확인. 실시간 앱은 여전히 미구현이다.

## 2026-09-06 — 학생 활동명을 호(號) 방식으로 확정

`TASK-20260906-05`(기획·통합). 문서만 변경했다.

- `COLLABORATIVE_MUD_ARCHITECTURE.md` §2-2를 **호(號) 방식**으로 전면 개정했다. 학생 표시 이름을 `S07` 같은 코드가 아니라 학생이 스스로 고른 호로 한다. 근거: 호는 살던 곳·자연물에서 따 본인이 직접 짓는 별칭이며(`퇴계`·`율곡`·`다산`), 이황·이이·정약용은 5학년 교과 내용이라 학습으로 연결된다.
- **시대 무관 공용 호 24개**를 확정했다(자연물형). 장소·직업·농경 전제 표현은 전부 폐기했다 — 한산도 배 위나 궁궐 장면에서 깨지기 때문이다. 호는 앱 공통 상수, 역할 명사는 시나리오 콘텐츠로 경계를 나눴다.
- **이름 선택과 역할 배정의 시점 분할안을 폐기했다.** 호는 직업·신분을 주장하지 않아 서버 배정 역할과 병기할 수 있으므로(`새벽 안개 · 농부`), 앞서 검토했던 "형용사는 대기 중, 명사는 시작 시" 분할이 불필요해졌다.
- **40분 예산 우려를 해소했다.** 선택 시간은 추가 비용이 아니라 21명이 QR로 입장하는 대기 시간이다. 미선택자는 교사 시작 시 서버가 무작위 배정하므로 늦은 학생이 수업을 지연시키지 않는다(Blooket 대기 화면 방식).
- 안전 규칙을 명시했다: 자유 입력 금지, 자연물·풍경만, 능력·외모·순위 표현 배제, 선착순 유일성, 역할은 서버만 배정, 교사 화면에 모둠·번호 병기, 세션 한정 폐기.
- 순위·지각 암시 표현(`가장 늦게 온`)을 뺀 이유를 기록했다. 실제로 늦게 입장한 학생에게 무작위 배정될 때 놀림이 된다.
- `players` 스키마에 `displayNameId`, `displayNameAssignedBy: 'student' | 'auto'`를 반영했다.
- 검증: `git diff --check`, 수정 문서의 상대 링크 확인. 실시간 앱은 여전히 미구현이다.

## 2026-09-06 — 제작 파이프라인 확정과 종이 단계 폐기

`TASK-20260906-06`(기획·통합). 문서만 변경했다.

- **원소스 멀티유즈 파이프라인을 확정했다.** `COLLABORATIVE_MUD_PLAN.md` §12를 신설해 `아이디어 확장·고도화 → 정적 협동 MUD 제작·수업 검증 → Vercel + Convex로 발전` 순서를 명문화했다. 정적과 실시간은 별개 제품이 아니라 하나의 콘텐츠를 두 방식으로 활용하는 것이다.
- **범위를 2단으로 분리했다.** 실시간 버전은 사용자 본인 학급 전용이고, 다른 교사에게는 정적 버전을 제공한다. 교사 초대 UI·다중 테넌시를 만들지 않으므로 실시간 앱의 인증·권한 범위가 줄어든다.
- **종이 단계를 전부 폐기했다.** 라이브 협동 MUD(`cooperative-mud/`)를 확인한 결과 1인 1기기에서 학생마다 다른 정보를 표시하므로 **기기 화면이 곧 역할 카드**다. 종이 역할 카드가 할 일이 없다.
  - 선사시대 파일럿을 `종이 리허설` → `정적 협동 MUD 제작`으로 전환하고 문서 상태를 `콘텐츠 원본`으로 바꿨다. 3막 카드 문안은 정적 웹의 콘텐츠 원본으로 재사용한다.
  - 네트워크 장애 대비 종이 폴백을 **정적 버전 폴백**으로 교체했다(아키텍처 §5·§8, MVP Phase 5·위험표). 전환이 빠르고 준비물이 없다.
  - `cooperative_prehistory_paper_rehearsal_execution.md`를 `docs/archive/`로 옮겼다.
  - 남는 조건을 명시했다: 정적 버전도 GitHub Pages에서 받으므로 학교 인터넷 전체 장애에서는 폴백이 되지 않는다.
- **타이머 페이싱을 설계했다.** 5분형/10분형 선택이 곧 시간 예산이 되고, 진행 바를 예산과 비교해 느리면 힌트·빠르면 추가 활동을 제시한다. 서버 없이 작동하므로 정적 버전에서 먼저 구현한다. 규칙: 페이싱 값은 콘텐츠 소유, 판정은 관대하게, 힌트는 답이 아니라 질문, 팝업으로 대화를 끊지 않음, 빠른 모둠에는 깊이(진도 아님), 카운트다운 숫자 비노출.
- 정적은 개인 기기 진행도만 알 수 있고 실시간은 모둠 단위로 정확하다는 차이를 기록했다. 이것이 실시간 버전의 실질 가치 중 하나다.
- `BACKLOG.md`·`project_context.md`·`docs/plans/README.md`의 종이 관련 서술을 함께 갱신했다.
- 검증: `git diff --check`, 전체 문서 상대 링크 검사. 코드는 변경하지 않았다.

## 2026-09-06 — 개인정보 리스크 대응: 학생 응답 비저장 설계

`TASK-20260906-07`(기획·통합). 문서만 변경했다.

- **[D-021] 학생의 판단 내용을 서버에 저장하지 않는 원칙**을 세웠다. 앱이 이름을 수집하지 않아도 교사가 `모둠·번호 = 실제 학생` 대응표를 갖고 있어 서버 응답과 결합하면 식별된다. Convex는 미국 호스팅이라 국외 이전에도 해당할 수 있다.
- 설계의 핵심 근거: **대시보드가 필요로 하는 것은 학생의 답이 아니라 진행 상태다.** 2026-09-04에 교사가 꼽은 제약도 "모둠 진행 상황을 볼 수 없음"이었지 "학생이 무엇을 골랐는지 볼 수 없음"이 아니었다. 따라서 개인 응답을 빼도 원래 요구는 그대로 충족된다.
- 아키텍처 §2-3을 신설해 서버/기기 저장 경계를 표로 정리했다. `players` 스키마에서 `firstDecision`·`revisedDecision`·`historyReflection`의 내용 필드를 제거하고 `*SubmittedAt` 시각만 남겼다. 모둠 법 초안·확정과 `lawReview`(agree/discuss)는 공동 산출물이라 서버에 유지한다.
- 보관 기간을 **7일 → 0일**로 바꿨다. `수업 종료`가 곧 삭제이며 `deleteAfter`는 종료 버튼을 누르지 못한 세션의 안전망으로만 쓴다.
- MVP §5(대시보드), §6(관찰 방법), §7(로그 범위), §11(위험표)을 같은 원칙으로 갱신했다. 판단 변경 관찰은 서버 집계 대신 학생 기기 요약과 모둠 대표 발표로 한다 — 2026-09-04의 실제 운영 방식이다.
- 학생 명단 업로드 기능을 만들지 않는다는 규칙을 명시했다. 편의를 이유로 추가하면 나머지 두 층이 무의미해진다.
- `BACKLOG.md`에 **P2-COLLAB-05**(학교·교육청 지침 확인, 질문 3개 포함, 착수 전 필수)와 **P2-COLLAB-06**(Vercel 첫 배포 직후 네트워크·QR 실측)을 추가했다. Vercel은 2026-09-06 기준 미시작이다.
- 사용자가 별도로 만들던 "다른 활동 아이디어" 문서는 저장소·아티팩트·다른 세션 어디에도 없어 미확보로 기록했다.
- 검증: `git diff --check`, 전체 문서 상대 링크 검사. 코드는 변경하지 않았다.

## 2026-09-06 — 협동 MUD 수업 구조도 발행과 README 연결

- 기존 정적 7단계 위에 QR 입장·학생 호(號)·타이머 페이싱·교사 대시보드·데이터 경계를 얹은 전체 구조를 아티팩트로 정리했다: <https://claude.ai/code/artifact/97e312bf-7cf9-4b4c-be07-6d0a4fd2c9a2>
- 그림 5개로 구성했다. ① 교사·학생 기기·서버 스윔레인 ② 기존 7단계와 03↔04 순서 교정 ③ 호 선택 흐름과 24개 목록 ④ 서버/기기 데이터 경계 ⑤ 정적에서 실시간으로 갈라지는 배포. 실선은 구현됨, 점선은 설계만으로 구분했다.
- `README.md`의 협동 MUD 절에 구조도 링크와 실시간 확장 설계 문서 3종 링크를 추가하고, 2026-09-04 실제 수업 운영 사실과 `EXP-006` 참조를 넣었다.
- 아티팩트는 기본 비공개이므로 README에 그 사실을 함께 표시했다. 학교·집 두 기기를 오갈 때 저장소에서 링크를 찾을 수 있게 하는 것이 1차 목적이다.
- 검증: `git diff --check`, README 로컬 링크 존재 확인, 아티팩트 HTML의 태그 구조·SVG marker 참조·CSS 토큰 정의 검사 통과. 렌더링 결과를 눈으로 확인하지는 못했다(미리보기가 프로젝트 밖 파일을 정적 스냅샷으로만 열고, 앱 내 브라우저는 claude.ai 미로그인).

## 2026-09-06 — 정적 협동 MUD 2편 추가: 삼국·가야 시조 설화

`cooperative-mud/founding-myths/` 신규. 파이프라인([`COLLABORATIVE_MUD_PLAN.md`](./docs/plans/COLLABORATIVE_MUD_PLAN.md) §12)의 `정적 협동 MUD 제작` 단계다.

- **소재 선정**: 고조선이 1단원 6차시였고 다음이 7차시 삼국·가야 성립이라, 후보 문서의 ⭐최우선인 서희의 담판(15차시)은 진도가 멀어 제외하고 시조 설화(7차시)를 골랐다.
- **역할 4 + 1**: 고구려(주몽)·신라(박혁거세)·가야(김수로)·백제(온조), 5인은 `이야기 모음관`. 모음관은 서기가 아니라 "모둠이 문장을 정하기 전에 서로 다른 이야기의 근거를 두 가지 이상 말하게 하는" 강제 시점을 갖는다.
- **활동의 축**: 셋은 알에서 태어나지만 **백제 온조만 난생이 아니다.** 이 예외가 "모두 알에서 태어났다"는 성급한 일반화를 깨고, 10분형의 `공통점 다시 보기` 단계에서 "백제까지 넣어 말하려면 어떻게 바꿔야 하나"를 묻는다.
- **순서 교정 적용**: 이 편은 `역할 확인 → 최초 판단 → 공유 → 새 자료 → 재판단`으로 만들었다. 고조선 v0.1의 공유 선행 문제(P2-COLLAB-02)를 신규 편부터 바로잡았다. 사용자 결정에 따라 고조선 v0.1은 그대로 둔다.
- **추가 자료**: 광개토대왕릉비(414년). 설화가 옛날이야기로만 남은 것이 아니라 왕실이 자기 자리를 설명할 때 실제로 사용했다는 당대 기록이며, 7차시 광개토대왕 학습과도 이어진다.
- **결과 비교 톤**: 정답/오답 대신 "네 생각은 ~였구나"로 시작한다. `정말 그런 일이 있었기 때문에`를 고른 학생에게도 틀렸다고 하지 않고 자료로 확인하기 어렵다는 점만 알린다. 공동 문장 선택지에 `자료만으로는 하나로 단정하기 어렵다`를 정식 선택지로 넣었다.
- **버그 수정**: 첫 구현에서 3인 모둠일 때 `roles[0..2]`가 고구려·백제·신라가 되어 **가야가 통째로 빠지고 백제가 공통 단서와 중복**됐다. 역할 순서를 고구려·신라·가야·백제·모음관으로 바꿔, 3인에서는 난생 셋을 역할로 두고 예외인 백제를 공통 자료로 모두에게 주도록 고쳤다.
- **문장 조립 수정**: `~으로 ... 하려고 그렇게 이야기했다`가 겹쳐 어색해, `옛사람들은 ~을 ~으로 이야기했다. ~는 뜻으로 보인다.` 두 문장으로 나눴다.
- **출처**: 확인된 링크만 사용했다(국사편찬위원회 우리역사넷 교과서 용어 해설). 『삼국사기』·『삼국유사』·광개토대왕릉비는 편찬 연대와 성격을 텍스트로 인용했고, 개별 사료 페이지 딥링크는 확인 전이라 넣지 않았다.
- 허브(`cooperative-mud/index.html`)를 2개 활동으로 갱신하고, 시조 설화는 공유 전에 각자 판단하는 순서임을 교사 안내에 추가했다. README에도 두 활동을 차시와 함께 연결했다.

검증: 브라우저에서 4인 2번(백제)·3인 3번(가야)·5인 5번(모음관) 세 경로를 끝까지 완주했다. 5분형은 `공통점 다시 보기`를 건너뛰고 문장 단계로, 10분형은 거쳐 가는 것을 확인했다. 3인 추가 단서 표시/숨김, 판단 유지·변경 기록, 요약 화면, 역사 비교 문구 분기를 확인했다. 콘솔 오류·경고 0건, 375px 모바일에서 가로 스크롤 없음·버튼 높이 49px. `node --check` 2종과 정적 검증 7종 통과. 실제 수업 운영은 아직이다.

## 2026-09-06 — 타이머 페이싱을 정적 협동 MUD 두 편에 구현

`COLLABORATIVE_MUD_PLAN.md` §12-3에 설계해 두고 시조 설화 제작 때 빠뜨렸던 항목을 채웠다.

- **공용 모듈로 분리**: `cooperative-mud/pacing.js`. 두 에피소드가 함께 쓰고 다음 시나리오도 그대로 재사용한다. 서버 없이 각 기기가 자기 시계와 자기 진행도만 본다.
- **예산은 콘텐츠가 소유**: 각 `scenario.js`에 `pacing.budgets`로 활동 길이별 화면 목표 시간을, `pacing.prompts`로 화면별 느림·빠름 질문을 넣었다. 실시간 버전이 생겨도 같은 값을 쓴다.
  - 시조 설화 10분형 600초 / 5분형 310초, 고조선 10분형 600초 / 5분형 310초.
- **판정 규칙**: 느림은 해당 화면 목표의 1.5배 초과, 빠름은 누적 목표의 60% 미만. 목표 25초 미만 화면은 느림 판정을 하지 않고, 누적 60초 미만에서는 빠름 판정을 하지 않는다. 감도는 `tuning`으로 시나리오가 덮어쓸 수 있다.
- **대화를 끊지 않는다**: 팝업이 아니라 진행 바 아래 작은 배지만 나타나고, 학생이 눌러야 내용이 펼쳐진다. 화면 이동 시 초기화된다.
- **힌트는 답이 아니라 질문**: `"아직 말하지 않은 친구가 있나요?"`처럼 되묻는 형태로만 썼다. 빠른 모둠에는 다음 단계를 열어 주지 않고 심화 질문만 준다(`"우리 문장이 백제 이야기도 설명하나요?"`).
- **카운트다운을 보여 주지 않는다**: 남은 시간 숫자가 뜨면 학생이 대화를 줄이고 버튼을 누른다.

검증: 빠름 경로는 실제 예산으로 확인했다(`share` 진입 시 누적 90초 대비 즉시 도달 → 심화 배지, 접힌 상태). 느림 경로는 `tuning`으로 목표를 줄여 타이머 동작을 확인했다 — 진입 직후 숨김, 목표 전 숨김, 목표×1.5 경과 후 표시, 배지 클릭 시 펼침, 화면 이동 시 초기화. 두 에피소드를 각각 끝까지 완주해 기존 흐름 회귀가 없음을 확인했고 콘솔 오류 0건. `node --check` 5종과 정적 검증 7종 통과. 실제 수업에서의 목표 시간 적정성은 아직 측정하지 않았다.

## 2026-09-06 — 공통 엔진 분리와 세 번째 협동 MUD: 한강 유역 쟁탈

### 공통 엔진 `cooperative-mud/episode.js`

세 번째 에피소드를 그대로 만들면 앱 코드가 3벌이 되므로 먼저 엔진을 뽑았다. 화면 흐름이 같은 에피소드가 이 파일 하나를 함께 쓴다. 시나리오가 바꾸는 것은 전부 `scenario.js`에 있고, 엔진에는 특정 시나리오의 문안이나 역사 내용을 넣지 않는다.

- 시나리오는 `window.CoopScenario`로 자기를 등록하고, `storageKey`·`labels`·`statement`·`compare`·`documents`를 제공한다.
- 모둠 문장 입력란은 `statement.fields`에서 동적으로 만들어지므로, 새 시나리오는 필드 이름과 개수를 자유롭게 정한다. 문장 조립도 `statement.compose`가 맡는다.
- 결과 비교 문구는 `compare`가 소유한다. `unverifiableChoiceId`로 "자료로 확인하기 어려운 선택"을 지정하면 그 답에만 다른 안내가 나간다.
- 시조 설화를 이 엔진으로 옮기고 `founding-myths/app.js`를 삭제했다. 고조선 v0.1은 화면 순서가 달라(공유가 최초 판단보다 앞) 손대지 않았다.

### 한강 유역 쟁탈 — 1단원 8차시

`cooperative-mud/han-river/`. 새 파일은 `scenario.js`와 `index.html` 둘뿐이다.

- **모둠 대항전을 피했다.** 학생이 나라를 하나씩 맡으면 모둠 간 경쟁이 되고 "우리 나라가 이겨야 한다"가 된다. 대신 역할을 조사 분야로 나눴다 — 땅 조사관(넓은 들과 사람), 물길 조사관(배로 오가는 길), 바닷길 조사관(서해 건너 중국), 기록 조사관(주인이 바뀐 순서). 모둠 전체가 같은 질문 하나에 답한다.
- **비대칭 정보가 판단을 가른다.** 자기 자료만 보면 자기 근거 쪽으로 기울게 되어 있고, 모아 보면 "까닭이 하나가 아니다"에 이른다.
- 5인은 `견주는 사람`으로, 모둠이 문장을 정하기 전에 서로 다른 조사의 근거를 두 가지 이상 말하게 하는 강제 시점을 갖는다.
- 3인은 기록 조사관을 공통 자료로 내린다. 연표는 판단의 전제라 모두가 봐야 한다.
- **추가 자료**: 세 나라의 전성기가 저마다 한강 유역을 차지하고 있던 때와 겹친다는 사실. 개별 근거를 넘어서는 패턴이라 "왜?"를 다시 연다.
- **10분형 다시 보기**: 6세기에 백제와 신라가 함께 되찾은 땅을 신라가 혼자 차지해 동맹이 깨진 일. 자기들이 고른 까닭으로 이 일까지 설명되는지 시험한다. 관산성 전투와 성왕의 죽음은 다루지 않고 "동맹이 깨졌다"까지만 쓴다.
- **전투를 재현하지 않는다.** 활동 내내 "왜 이 땅이 중요했는가"만 다룬다. 완료 화면에도 어느 나라가 이겼는지가 아니라 근거로 설명하는 과정을 본다고 명시했다.
- 모둠 문장은 `가장 큰 까닭은 / 그 증거는 / 다만` 세 칸이며, 마지막 칸에 `땅 하나로 나라의 힘을 다 설명할 수는 없다`를 정식 선택지로 두었다.
- 타이머 페이싱 예산과 질문을 처음부터 넣었다(10분형 600초 / 5분형 310초).

검증: 한강 편을 4인 3번(바닷길)으로 끝까지 완주했고, 3인 1번에서 좌석 3개·기록 조사관이 공통 자료로 내려가는 것과 5분형이 다시 보기를 건너뛰는 것을 확인했다. 엔진 이전 뒤 시조 설화를 4인 4번(백제)으로 다시 완주해 회귀가 없음을 확인했다(역할·라벨·필드·문장·비교 문구·요약 라벨 일치). 허브 3개 활동 표시와 교사 안내 갱신 확인. 콘솔 오류 0건, `node --check` 6종과 정적 검증 7종 통과. 실제 수업 운영은 아직이다.

## 2026-09-06 — 인수인계 문서 최신화와 수업 진행 안내 신설

학교에서 `git pull` 후 문서만 보고 이어갈 수 있는지 점검하다가, 인수인계 문서가 5일치 작업만큼 뒤처져 있는 것을 확인해 채웠다.

- **`project_context.md`가 실제로 비어 있던 부분**: 파일 구조 트리에 `cooperative-mud/`가 아예 없었고(0건), 완료된 기능 절에 협동 MUD 언급이 없었으며(0건), §7 다음 작업 후보는 9월 1일 기준 그대로였다. 헤더도 v3.3 · 2026-09-01이었다.
- 파일 구조에 `cooperative-mud/` 하위(허브·`episode.js`·`pacing.js`·에피소드 3폴더)를 추가하고, 완료된 기능에 협동 MUD 3편과 타이머 페이싱을 넣었다. 헤더를 v3.5 · 최종 갱신 2026-09-06으로 바꿨다.
- **§7 다음 작업 후보를 현재 기준으로 재작성했다.** 교실에서만 확인되는 항목을 앞에 뒀다 — ① 7·8차시 실제 수업 운영 ② 학교 개인정보 지침 확인. 그다음이 타이머 예산 조정·네 번째 시나리오·실시간 첫 슬라이스이고, 개인형 앱 잔여 항목은 뒤로 내렸다.
- §8에 협동 MUD `node --check` 6종을 추가했다. 기존에는 개인형 앱 검증 명령만 있었다.
- **`cooperative-mud/TEACHING.md` 신설**: 교실에서 바로 보는 문서다. 지금까지 운영 방법이 허브 안내·계획서·EXPERIMENTS·BACKLOG에 흩어져 있어, 수업 직전에 한 곳에서 볼 수 없었다.
  - 활동 3편의 차시·주소·운영 여부, 수업 전 준비 4단계, 수업 중 유의점, 관찰 체크리스트 7개, 알아 둘 것(6차시 순서 차이·기기별 저장·인터넷 필요), 수업 뒤 기록 절차.
  - 관찰 체크리스트는 2026-09-04에 이미 확인된 것(대화 발생·결과 다양성)을 빼고 **아직 확인되지 않은 것**만 담았다. 판단 변경은 7·8차시에서만 제대로 측정된다는 점을 명시했다.
- 허브·README·`project_context.md`에서 진행 안내로 연결했다.

검증: `scripts/06_validate_static_assets.py` 통과, 기준·계획 문서 전체 상대 링크 0건 깨짐. 코드는 변경하지 않았다.

## 2026-09-07 — Regular MUD 시뮬레이터 이미지 전수조사 및 11곳 배경 누락 수정

`data/mud/regular_*.json` 28종 전체를 대상으로 [`docs/audits/regular_simulator_image_inventory.md`](./docs/audits/regular_simulator_image_inventory.md)를 작성했다 — 씬 97종·레거시 모드 22종의 설명(표1)과 215개 스테이지의 대화·선택지↔이미지 연결(표2).

- `hasHotspots` 102곳 중 실제로 배경 그림이 없는 곳은 11곳뿐임을 재확인(91곳은 `scene`이 함께 선언돼 정상 렌더링).
- 우선순위 1(8곳): `regular_myeongnyang`·`regular_bronze_age`는 기존 scene 팔레트 재사용, `regular_gwangbok` 3곳은 `js/mudSimulators.js`에 신규 scene 3개(`gwangbok-radio-announcement`/`gwangbok-committee-office`/`gwangbok-ballot-booth`) 작성.
- 우선순위 2(3곳): `regular_neolithic` 재시도 스테이지가 이미 존재하는 primary 스테이지의 scene을 그대로 재사용하도록 수정.
- `regular_gwangbok.json` 1→2→3→엔딩 전체 완주로 신규 scene 검증.

검증: `node --check`, JSON 유효성, `scripts/03/04_validate_*` 통과. 브라우저에서 11곳 전부 렌더링 확인, 콘솔 오류 0건. 커밋 `5d80d36`·`61a0758`·`b4457c1`.

## 2026-09-07 — 선택지 길이 편향 24곳 수정, 콘텐츠 불일치 3건 발견·수정

`scripts/12_audit_choice_bias.py` 기준 Regular MUD 24개 스테이지(정답이 오답 평균보다 10자 이상 긴 곳)를 "단서 인용형 재작성"(정답·오답 모두 핫스팟 단서를 구체적으로 인용)으로 고쳤다.

- 수정 과정에서 **서사·핫스팟과 선택지 주제 자체가 어긋난 콘텐츠 버그 3건**을 발견했다 — `regular_gojoseon:3`(서사만 "절도 처벌법", 나머지 전부 "비파형 동검=고조선 문화권 증거"), `regular_joseon_silhak:2`(서사·핫스팟만 "토지 개혁", 나머지 전부 "거중기"), `regular_three_kingdoms_life:2`(서사·핫스팟만 "벽화 인물 크기=신분 표현", badge·scene·선택지는 "불교 수용"). 다수 필드가 가리키는 쪽으로 소수 필드를 맞춰 재작성했다.
- Regular MUD 기준 편향 24→0. 프로젝트 전체 잔여 3건은 Deep-dive 소관(범위 밖).

검증: JSON 유효성, `scripts/03/04/09_validate_*` 통과, `12_audit_choice_bias.py` 재실행 확인. 콘텐츠 버그 3건 브라우저 실제 플레이로 재확인, 콘솔 오류 0건. 커밋 `6afc4a3`.

## 2026-09-07 — Regular 활동 시간·탭 내성 구조 재감사

`scripts/09_audit_tap_resistance.py`·`07_audit_activity_duration.py`를 다시 실행해 BACKLOG의 옛 "40개 후보" 수치를 갱신했다.

- 판단 스테이지(분기 있는 결정 지점) 반복 탭 위험은 **0건**. 남은 후보 7곳은 전부 각 MUD의 단일 선택지 엔딩 스테이지(`next: "end"`)라 재설계 불필요로 판정.
- `07_audit_activity_duration.py`가 `regular_paleolithic`을 오탐으로 계속 잡는 원인(09번 스크립트의 `KNOWN_DISTINCT_INTERACTION_MODES` 제외 목록을 07번이 반영 안 함)을 확인, 낮은 우선순위 정리 과제로 기록.
- 구조 감사로 확인 가능한 부분은 완료. 남은 건 실제 학생 3명 이상 교실 실측뿐(사용자 전담).

커밋 `9663c77`.

## 2026-09-07 — MUD 등록 경로: 도달 불가능했던 보조 MUD 2종 노출

BACKLOG P2-03 점검 중 `placement: "supplementary"`로 등록된 `regular_myeongnyang`(2단원 7차시)·`regular_korean_war`(3단원 10~12차시)가 포털 어디에도 노출 경로가 없어 **완전히 도달 불가능한 상태**였음을 발견했다(3단원 44·45번 차시 카드는 primary MUD도 없어 빈 카드였음).

- `js/app.js`에 `findSupplementaryMud()`를 추가해 primary 버튼 아래 `.btn.secondary` 스타일 "+확장 활동" 버튼을 렌더링하도록 수정.
- `app.js`의 레거시 Regular 조건문(~140줄)은 인덱스 로드 실패 시에만 도는 안전망임을 코드로 재확인, 제거하지 않고 유지 결정.
- 작업 중 `index.html`의 `js/app.js?v=...` 캐시 버스터를 갱신하지 않으면 브라우저가 옛 스크립트를 계속 쓴다는 것을 재확인(버전 문자열 `20260907-supplementary1`로 갱신).

검증: `node --check`, `scripts/08_validate_mud_catalog.py` 통과. 데스크톱·태블릿(768×1024) 뷰포트에서 1~3단원 전체 카드 확인, 보조 버튼 클릭 시 `regular_myeongnyang` 정상 오픈 확인, 콘솔 오류 0건. 커밋 `0d820aa`.

## 2026-09-07 — IF 재시도 스테이지 83곳 교사 검토 우선순위화 및 전수 검토

`if_stage_audit.md`의 잔여 90개 신호를 [`docs/audits/if_stage_teacher_review_priority.md`](./docs/audits/if_stage_teacher_review_priority.md)로 4단계 우선순위화했다(둘 다 미달/한쪽만 미달 × 민감 주제 여부). 이어서 83곳 전부를 `MudEngine.openMUD`+`renderStage`로 브라우저에서 직접 열어 검토했다.

- 1순위 14곳 중 3곳, 2순위 15곳 중 2곳만 실제로 주제와 무관한 범용 문구였다(예: 7개 파일에 걸쳐 완전히 동일한 "한 자료는 과거의 한 측면을 보여 줍니다..." 문구가 일제강점기·세종대왕·전후재건 등 서로 다른 주제에 그대로 복사돼 있었음) — 이 5곳만 "단서 인용형 재작성"으로 고쳤다.
- 3순위 16곳·4순위 38곳은 전수 확인 결과 재작성 없이 유지 — 감사 스크립트의 기계적 기준(120자·단서 2개)이 짧아도 이미 구체적인 서술(예: 손변의 재판 일화, 근초고왕의 평양성 공격, 이순신의 실제 발언 인용)을 걸러내지 못했을 뿐, 내용 자체는 문제가 없었다.
- 83개 신호 전부 검토 완료: 재작성 5곳(+이전 세션의 범용 문구 7곳 = 총 12곳), 나머지 76곳은 이미 적절함을 확인.

검증: 재작성한 각 스테이지 JSON 유효성, `scripts/03/04_validate_*`, `10_audit_if_stages.py` 재실행(90→78) 확인. 83곳 전부 브라우저 렌더링 확인, 콘솔 오류 0건. 커밋 `2033322`·`3204538`·`a93b91e` 등.

## 2026-09-07 — 유물 카드 아이콘 7종 수정 및 캐시 버그 발견

`docs/audits/artifact_audit.md` 기준 36종 유물 카드의 콘텐츠(과장 표현 등)는 이미 문제가 없었으나(0건), 감사 스크립트가 검사하지 않는 **아이콘-내용 불일치**를 수동 검토로 7건 찾아 수정했다 — 예: 백제 칠지도(외교 선물인데 전투 쌍검 이모지 ⚔️→단검 🗡️), 6·25 무공훈장(전투 헬멧 🪖→훈장 🎖️), 남한산성·경복궁(일본식 성 이모지 🏯→각각 🏰·🏛️).

- `js/encyclopedia.js`의 `artifacts.json` fetch에 `{cache: 'no-store'}`가 빠져 있어 브라우저가 예전 유물 데이터를 계속 보여주는 버그를 발견·수정(다른 fetch에는 이미 있던 패턴).
- 작업 중 공유 작업 폴더가 다른 세션에 의해 `main`이 아닌 로컬 전용 브랜치(`feat/cooperative-live-vertical-slice`)로 체크아웃돼 있던 것을 발견 — 커밋이 그 브랜치에 잘못 들어갔던 것을 cherry-pick으로 `main`에 복구·push했다.

검증: JSON 유효성, `scripts/03/08/11_validate_*`(11번은 0건 확인) 통과. 실제 도감 UI에서 7개 카드 전부 아이콘 렌더링 확인, 콘솔 오류 0건. 커밋 `c91c2c3`(main).

## 2026-09-07 — Vercel·Convex 실시간 협동 MUD 첫 수직 슬라이스

기존 GitHub Pages 앱을 건드리지 않고 `apps/cooperative-live/`에 Next.js 독립 앱을 만들었다. 첫 범위는 고조선 8조법, 교사 1명, 가상 학생 8명, 4인 모둠 2개다.

- 가상 데이터 모드에서 세션 생성, 학생 8명 입장, 중복 없는 호(號), 모둠·역할 무작위 배정, 학생 단계 진행, 모둠별 힌트/심화, 종료 삭제를 한 흐름으로 확인할 수 있다.
- 실제 연결을 위해 Convex `sessions`, `players`, `rooms`, `interventions` 스키마와 교사/학생 함수, 2시간 만료 정리 함수를 작성했다. 교사는 Auth0 `sub` 허용목록으로 제한하고 학생은 계정 없이 세션 한정 난수 토큰을 받으며 서버에는 SHA-256 해시만 저장한다.
- 학생의 개인 선택·이유는 서버로 보내지 않고 `sessionStorage`에만 둔다. 종료할 때 다른 앱 데이터를 건드리지 않고 `cooperative-live:` 키만 제거한다. 대시보드에는 호·모둠·역할·진행 단계·제출 시각만 표시한다.
- Production·실제 학생 접속은 `P1-COLLAB-PRIVACY` 해제 전까지 금지했다. 환경변수 예시와 Vercel Root Directory, Auth0/Convex 설정 절차는 앱 README에 기록했다.
- 검증: `npm run lint`, `npm run typecheck`, 단위 테스트 3건, `npm run build` 통과. Chrome에서 전체 가상 흐름과 종료 시 13개 레코드 삭제 표시를 확인했고, 375×812 모바일에서 가로 스크롤 없음·터치 버튼 48px·새 탭 콘솔 오류/경고 0건을 확인했다.
- 제한: Convex 익명 로컬 배포는 개발 PC에서 백엔드 바이너리 다운로드 중 `self-signed certificate in certificate chain` 오류가 발생해 실행하지 못했다. TLS 검증 비활성화는 하지 않았고, 실제 함수 통합·Auth0·Vercel Preview 검증은 신뢰할 수 있는 CA 설정 및 계정 연결 후 진행한다.

## 2026-09-07 — 반복 탭 감사 스크립트 판정 기준 통일

`07_audit_activity_duration.py`가 `regular_paleolithic`을 계속 오탐하던 문제를 근본에서 고쳤다.

- **불일치는 문서에 적힌 한 군데가 아니라 네 군데였다.** 두 스크립트가 각자 판정 로직을 들고 있었다.
  - 레거시 중복거부 모드 제외: 09번만 있음 ← 오탐의 직접 원인
  - `completion.uniqueActions` 제외: 07번만 있음
  - `minActions` 기본값: 09번 `completion.target` / 07번 `99`(사실상 판정 안 함)
  - 검사 대상: 09번 전체 스테이지 / 07번 본 스테이지만
  - 출력 차이는 `regular_paleolithic` 하나뿐이었지만, 나머지 세 가지는 우연히 결과가 같았을 뿐이라 언제든 다시 어긋날 수 있었다.
- **판정을 `scripts/_tap_resistance_rule.py`로 분리했다.** 두 스크립트가 `find_rapid_tap_stages()`를 함께 쓴다. 규칙이 한 곳에만 있으므로 구조적으로 다시 갈릴 수 없다.
- 통일 기준은 **제외 조건의 합집합**이다. 다섯 조건(JSON 핫스팟 / 버튼형 개별 action / `uniqueActions` / 레거시 중복거부 모드 / 조작 수 5 이상)은 각각 "같은 곳을 반복해도 통과되지 않는다"는 서로 다른 근거이므로, 하나라도 해당하면 후보가 아니다. 왜 갈렸는지와 각 조건의 근거는 모듈 docstring에 남겼다.
- 결과: 07번 8종 → **7종**(`regular_paleolithic`이 `기준 충족 추정`으로), 09번 7종 유지, 두 스크립트 판정 **완전 일치**.
- `__pycache__`가 생기므로 `.gitignore`를 새로 만들었다(기존에 없었다).

검증: 두 스크립트 재실행 후 대상 MUD 집합이 정확히 일치함을 프로그램으로 대조 확인. `activity_duration_audit.md` 변경은 `regular_paleolithic` 행과 집계 수 3줄뿐. 정적 검증 9종과 `05_test_simulator_runtime.js` 통과. 07번 재실행 멱등성 확인.

**작업 중 발견한 별건**: `scripts/10_audit_if_stages.py`와 `11_audit_artifacts.py`가 보고서를 저장소 **루트**에 쓰는데, 실제 추적 중인 파일은 `docs/audits/` 아래에 있다(문서 재배치 때 스크립트가 갱신되지 않음). 특히 `docs/audits/if_stage_audit.md`에는 손으로 쓴 실행 결과 절이 붙어 있어, 생성물로 덮어쓰면 사라진다. 이번 범위에 포함하지 않고 기록만 남긴다.

## 2026-09-08 — 감사 보고서 출력 경로 수정과 CI 무력화 발견

`10_audit_if_stages.py`·`11_audit_artifacts.py`가 보고서를 저장소 루트에 쓰는데 추적 파일은 `docs/audits/`에 있던 문제를 고쳤다. 조사 중 더 큰 문제를 찾았다.

- **CI 드리프트 검사가 무력화돼 있었다.** `quality-gate.yml`이 `git diff --exit-code -- if_stage_audit.md`로 검사했는데 루트의 그 파일은 **추적 대상이 아니다.** 추적되지 않는 경로를 `git diff`로 비교하면 항상 통과하므로, 10·11번 감사의 드리프트 검사는 무의미했다. 07번만 정상 작동했다(루트 파일이 추적됨).
- **경로만 바꿀 수 없었다.** `docs/audits/if_stage_audit.md`에는 사람이 쓴 `## 실행 결과 (2026-09-07, Claude)` 절이 있고, 그래서 "표는 최초 감사 시점 스냅숏이며 갱신하지 않음"이라는 우회 문구가 붙어 있었다. 스크립트가 파일 전체를 덮어쓰는 구조라 재생성을 포기한 것이다.
- **마커 방식으로 두 문제를 함께 풀었다.** `scripts/_managed_block.py`를 만들어 START/END 사이만 교체한다. 표는 항상 최신이고 사람이 쓴 절은 남는다. `08_validate_mud_catalog.py`가 README에 이미 쓰던 방식과 같다. 마커가 없으면 조용히 덮어쓰지 않고 **실패**한다.
- **07번에도 같은 보호를 적용했다.** 지금은 사람이 쓴 절이 없지만 같은 함정이 그대로 남아 있었다. 이제 생성 감사 보고서 3종이 모두 같은 규칙을 따른다.
- 결과: `if_stage_audit.md`의 얼려 있던 92행 스냅숏이 현재값 78행으로 살아났고, 수기 절은 그대로다. `artifact_audit.md`는 36종·신호 0종. 루트에 잘못된 파일이 더는 생기지 않는다.
- CI 경로를 `docs/audits/`로 바꿔 드리프트 검사가 실제로 작동하게 했다.

검증: 세 보고서 모두 재실행 멱등성 확인, 사람이 쓴 절 보존 확인, 마커 제거 시 종료코드 1로 실패하는 것 확인, CI 검사 대상 3개가 모두 추적 파일임을 확인. 정적·감사 스크립트 11종과 `05_test_simulator_runtime.js` 통과.

**작업 환경 사고**: 이 작업 도중 공유 폴더의 브랜치가 Codex 세션에 의해 `main` → `feat/cooperative-live-vertical-slice`로 두 번 바뀌었다. 두 번째에는 feature 브랜치 파일 위에 편집이 들어가 앞선 `f78a8df` 작업이 빠진 상태로 작업할 뻔했다. 미커밋 변경을 stash·패치로 보존하고 `main`에서 처음부터 다시 작업했다. 한 폴더를 두 세션이 공유하는 한 재발하므로, 별도 worktree 분리를 권한다.

## 2026-09-08 — 이 PC 세션 마무리

이 PC 세션을 닫은 당시 `main`은 `5b79690`이고 원격과 동기화돼 있었으며 미커밋 변경이 없었다.

### 이 PC에서 한 일 (2026-09-07~08)

| 커밋 | 내용 |
|---|---|
| `f78a8df` | 반복 탭 감사 판정을 `_tap_resistance_rule.py`로 통일 (07·09번) |
| `5b79690` | 감사 보고서 경로 수정 + 마커 방식 도입 + **무력화돼 있던 CI 드리프트 검사 복구** |

그 앞선 세션(같은 PC)에서 정적 협동 MUD 3편·타이머 페이싱·공용 엔진·구조도·`TEACHING.md`를 만들었고, 학교 세션의 Regular MUD 작업이 `main`에 이미 들어와 있다.

### 다음 세션이 이어갈 것

1. **7·8차시 협동 MUD 실제 수업 운영** — 제작·검증 완료, 학생과 해 본 적 없음. 진행은 [`cooperative-mud/TEACHING.md`](./cooperative-mud/TEACHING.md), 관찰은 `EXPERIMENTS.md`에 `EXP-006` 형식으로.
2. **학교·교육청 개인정보 지침 확인** 🔴 — 실시간 확장 착수 게이트. 질문 3개는 `BACKLOG.md` `P1-COLLAB-PRIVACY`에 있다.
3. 타이머 예산 조정 → 네 번째 시나리오 순. 실시간 첫 슬라이스는 로컬 구현됐으며 개인정보 게이트 뒤 외부 서비스 연결 검증을 진행한다.

상세는 [`project_context.md`](./project_context.md) §7.

### 인계 시 알아 둘 것

- 실시간 앱(`apps/cooperative-live/`)과 개인정보 감사 문서는 `TASK-20260908-02`에서 `main`에 통합해, 이전의 브랜치 전용 상태와 끊어진 BACKLOG 링크를 해소했다.
- **감사 보고서를 손으로 고칠 때는 마커 바깥에 쓴다.** 안에 쓰면 다음 스크립트 실행에서 사라진다.
- **작업 폴더를 Codex와 공유하는 한 브랜치 전환 사고가 재발한다.** 규칙은 `agents.md`의 Multi-agent working rules에 구체화해 두었다. 병렬 작업 시 `git worktree`로 폴더를 나눈다.
- 백업 stash(`stash@{0}`)가 남아 있다. `main`에서 재작업했으므로 필요 없으며 `git stash drop`으로 지워도 된다.
### 같은 날 — 작업 브랜치 이어받기와 복구·편성 보강

- GitHub의 `feat/cooperative-live-vertical-slice`를 새 PC에 받아 중단 지점에서 재개했다. `main`은 바꾸지 않고 해당 기능 브랜치에서 작업했다.
- 교사가 화면을 새로고침해도 서버의 현재 소유 세션을 다시 찾고, 활동 만들기를 반복해도 유효한 기존 세션을 재사용하도록 했다.
- 같은 학생 탭에서 QR이나 수업 코드를 다시 열면 저장된 세션으로 돌아가 중복 참여자가 생기지 않도록 했다. 세션 종료를 확인하면 이 복구 연결도 함께 지운다.
- 학생 수를 4명씩 자르던 편성을 고쳐 3~24명 어디서든 각 모둠이 3~5명이 되도록 균형 편성한다. 24명 초과 입장은 서버에서 거부한다.
- 6자리 수업 코드 충돌을 끝까지 피하지 못한 경우 중복 저장 대신 오류를 반환하고, 교사용 세션 응답에는 Auth0 `sub`를 포함하지 않는다.
- 개인정보 감사 기준과 실제 QR 구현을 대조해, QR도 수동 입력과 같은 6자리 코드만 사용하고 반복 시도 제한이 없는 공백을 찾았다. 실제 학생 접속 전 QR 전용 장기 난수 입장키와 수동 코드 제한을 설계하도록 `P1-COLLAB-PRIVACY`에 남겼다.
- 검증: `npm ci`, `npm run check` 통과(lint·TypeScript·단위 테스트 5건·Next.js production build). 개발 서버의 `/`, `/teacher`, `/join`은 모두 HTTP 200을 반환했다. 실제 Convex/Auth0/Vercel 연결과 실제 학생 접속은 기존 개인정보 게이트에 따라 진행하지 않았다.
- 기록 확정(2026-09-08): 위 보강 작업을 `8f27604`(`fix: harden cooperative live session recovery`)로 커밋해 GitHub의 `origin/feat/cooperative-live-vertical-slice`에 푸시했다. 로컬과 원격 브랜치가 같은 커밋을 가리키고 작업 트리가 깨끗한 것을 확인했으며, `main`에는 병합하지 않았다.

## 2026-09-08 — 실시간 협동 앱 `main` 통합과 다음 PC 인수인계

`TASK-20260908-02 | feat/cooperative-live-vertical-slice → main 통합·검증·인수인계 | integration agent | 상태: DONE`

- 기능 브랜치의 세 커밋(`036d855`, `8f27604`, `4218c0e`)을 Claude가 마무리한 최신 `main` 위에 병합했다. 충돌한 `INBOX.md`와 `walkthrough.md`는 양쪽 기록을 모두 보존했다.
- `apps/cooperative-live/` 소스와 개인정보 감사 문서가 `main`에 들어와 BACKLOG의 끊어진 링크를 해소했다. 루트 `.gitignore` 설명도 `main`에 실제 앱 소스가 존재하는 현재 상태에 맞췄다.
- `P2-COLLAB-01`의 옛 "미구현" 상태와 네트워크 실측 순서를 현재 기준으로 고쳤다. 로컬 구현은 완료됐고 외부 서비스 연결·실제 학생 적용은 개인정보 게이트 뒤 진행한다.
- 다른 PC의 시작 절차와 남은 일은 [`codex_cooperative_live_main_handoff_20260908.md`](./docs/handoff/codex_cooperative_live_main_handoff_20260908.md)에 고정했다.
- 검증: 앱 `npm run check` 통과(lint·TypeScript·단위 테스트 5건·Next.js production build). 저장소의 데이터·MUD 계약·카탈로그·출처·시뮬레이터 런타임·정적 자산·JavaScript 문법 검사와 생성 감사 3종(활동 시간 7개 후보, IF 78개 신호, 유물 36종/문구 신호 0개)을 다시 실행해 모두 통과했다. `git diff --check`와 병합 충돌 마커 0건도 확인했다.

## 2026-09-08 — 유물 2개 비교·추론 프로토타입 구현

`TASK-20260908-CMP1 | 유물 2개 기반 역사적 추론 프로토타입 | Claude Sonnet 5 | 상태: DONE`

- `BACKLOG.md` P2 항목과 `EXPERIMENTS.md` EXP-002를 이어받아, MUD 클리어 직후(`js/mudEngine.js`의 `renderFinalReflection()`) 해금 유물이 2개 이상이면 관찰→근거 카드→빈칸 주장 완성→결과 비교 4단계의 선택형 보너스 스테이지를 제안하도록 구현했다.
- 구현 전 museum.go.kr을 다시 조회해 빗살무늬 토기(신수22891)·민무늬 토기 항아리(신수10470) 모두 공공누리 제1유형(출처표시)만 적용됨을 확인했다(브레인스토밍 문서의 "빗살무늬=변경금지" 가정은 오류로 정정). 다만 실제 사진은 이번 프로토타입에 넣지 않고 기존 앱과 동일하게 이모지+텍스트로 표시하기로 사용자와 확인했고, 스킵은 불가가 아니라 버튼으로 제공하기로 했다.
- 결과 화면은 "정답/오답" 표현 없이 학생의 관찰·근거·주장을 먼저 요약한 뒤 학계 관점을 병치하고, 근접도에 따라 칭찬형/안내형으로만 톤을 바꾼다. 완료·건너뛰기 모두 `localStorage`(`encyclopedia.data.seenArtifactComparisons`)에 기록해 같은 콘텐츠가 반복 노출되지 않는다.
- 신규 파일: `data/artifactComparisons.json`, `js/artifactComparison.js`. 변경: `js/encyclopedia.js`, `js/mudEngine.js`, `js/app.js`, `index.html`(캐시 버스터 갱신), `scripts/01_validate_game_data.py`.
- 검증: `python scripts/01_validate_game_data.py`·`06_validate_static_assets.py` 통과. 로컬 정적 서버(`static-server`, 8791)에서 브라우저 콘솔로 두 톤 분기와 건너뛰기 후 재노출 안 됨을 직접 실행해 확인했다. 자동화 테스트는 아직 없다.
- 남은 것: 실제 교실 완주율·피로도 관찰(EXP-002 후속), 문서에 남아 있던 2·3번째 유물 페어(청자 매병/달항아리, 삼국 불상)는 이번 범위 밖으로 남긴다.

## 2026-09-08 — 유물 비교·추론 프로토타입 2번째 페어 추가

`TASK-20260908-CMP2 | 유물 비교 페어 확장(청자 매병 vs 달항아리) | Claude Sonnet 5 | 상태: DONE`

- 교사가 실제로 프로토타입을 써 보고 "이스터에그를 찾는 느낌", "시대를 넘나드는 비교의 재미"라는 긍정적 반응을 준 뒤, `data/artifactComparisons.json`에 2번째 페어를 추가했다: 청자 상감 구름·학무늬 매병(고려, 덕수2182) vs 백자 달항아리(조선, 접수702).
- 구현 전 museum.go.kr을 다시 조회해 두 유물 모두 공공누리 제1유형(출처표시)만 적용되고 변경금지 조건이 없음을 확인했다. 실제 사진은 이번에도 넣지 않고 첫 페어와 동일하게 이모지+텍스트 방식을 유지했다.
- 초점은 첫 페어(기술 변화)와 다르게 잡았다 — 화려한 상감 장식(고려)에서 소박한 여백(조선)으로 바뀐 것을 "기술 퇴보"가 아니라 "시대별 아름다움 기준의 변화"로 해석하도록 학계 관점 문구를 설계해, 학생이 앞선 활동에서 얻은 "무늬가 없어진 건 기술이 나빠진 게 아니다"라는 인상을 다른 맥락(가치관 변화)에도 다르게 적용해 보게 했다.
- `js/artifactComparison.js`는 변경하지 않았다 — 여러 페어를 배열 순서대로 제안하도록 이미 일반화되어 있어 데이터만 추가하면 됐다.
- 문서: `BACKLOG.md`의 해당 항목에 확장 기록 추가, `TEACHING_artifact_comparison.md`를 2페어 기준으로 갱신(페어 표 추가, 콘솔 테스트 명령 갱신).
- 검증: `01_validate_game_data.py`·`06_validate_static_assets.py` 통과. 로컬 정적 서버에서 2번째 페어의 두 톤 분기(칭찬형/안내형)와 첫 페어 완료 후 자동으로 이어서 등장하는 것을 브라우저 콘솔로 확인했다.
- 남은 것: 학생 피드백 5개 질문(EXP-002에 함께 기록 예정) 결과를 본 뒤 3번째 페어(삼국 불상 비교 등) 진행 여부를 결정한다.

## 2026-09-08 — 유물 비교·추론 프로토타입 3번째 페어 추가 및 후속 후보 조사

`TASK-20260908-CMP3 | 유물 비교 페어 확장(청자 vs 분청사기) + 후속 후보 조사 | Claude Sonnet 5 | 상태: DONE`

- 사용자와 함께 새 유물 비교 아이디어 5개(신라 금관 vs 가야 금동관, 무령왕릉 관식 vs 신라 금관, 청자 vs 분청사기, 상평통보 vs 오늘날 지폐, 혼천의 vs 홍대용 지구의)를 브레인스토밍한 뒤, museum.go.kr에서 하나씩 실제 확인했다.
- **구현**: 청자 상감 구름·학무늬 매병(고려, 덕수2182) vs 분청사기 상감 물고기무늬 매병(조선 초기, 덕수253) 페어를 추가했다. 둘 다 공공누리 출처표시만 있고 변경금지 없음을 확인했다. 앞선 두 페어가 "기술 발전"·"가치관 변화"를 다뤘다면, 이번엔 "같은 상감 기법이 왕조 교체기의 사회 혼란 속에서 자유분방한 스타일로 바뀐 이유"로 세 번째 해석 유형을 추가했다.
- **조사했지만 보류**: 신라 금관은 금관총 금관(본관9435, relicId=159727)까지 확인했지만, 가야 금동관(고령 지산동 32호분 출토)과 무령왕릉 금제관식(국립공주박물관)은 museum.go.kr의 소장품 검색 relicId 체계가 전시실 안내 페이지의 relicId와 달라 여러 번 시도했지만 확실한 매칭을 찾지 못했다. 잘못된 ID를 지어내지 않고 보류로 남겼다.
- **범위 제외**: 상평통보 vs 오늘날 지폐는 실물 박물관 유물 페어 형식이 아니라 활동 스키마와 맞지 않아 제외했다. 혼천의(구10035)는 단독 확인했지만 짝이 될 실물 소장품을 찾지 못해 보류했다.
- `js/artifactComparison.js`는 이번에도 변경하지 않았다 — 데이터만 추가.
- 문서: `BACKLOG.md`에 확장 2 기록 추가, `TEACHING_artifact_comparison.md`를 3페어 기준 표로 갱신.
- 검증: `01_validate_game_data.py`·`06_validate_static_assets.py` 통과. 로컬 정적 서버에서 3번째 페어의 근접 톤 분기와 앞선 두 페어 완료 후 자동으로 이어서 등장하는 것을 브라우저 콘솔로 확인했다.
- 다음: 신라 금관/가야 금동관/무령왕릉 관식의 정확한 relicId는 사용자가 museum.go.kr을 직접 검색하거나, 더 나은 검색 방법을 찾은 뒤 이어간다.

## 2026-09-08 — 유물 비교·추론 프로토타입 4번째 페어(신라 금관 vs 가야 금동관)

`TASK-20260908-CMP4 | 유물 비교 페어 확장(신라 금관 vs 가야 금동관) | Claude Sonnet 5 | 상태: DONE`

- 3번째 페어 확장 뒤 남겨둔 "신라 금관 vs 가야 금동관" 후보를 사용자와 함께 이어갔다. museum.go.kr의 소장품 검색 relicId 체계와 상설전시 안내(`M0201030700.do`) relicId 체계가 서로 다른 번호 공간이라는 것을 여러 번의 시행착오(WebFetch·WebSearch·실제 브라우저 검색) 끝에 확인했다.
- 사용자가 직접 museum.go.kr에서 가야 금동관의 전시실 안내 페이지(고령 지산동 32호 무덤 출토, showHallId=760&relicId=27014)를 찾아 링크를 전달했다. 이 페이지는 "가야 관은 신라 관처럼 일정한 형태가 없고 풀꽃 모양 세움장식이 특징"이라는 좋은 1차 대조 자료를 담고 있었다.
- 신라 쪽 짝은 이미 확인해 둔 금관총 금관(본관9435, relicId=159727, 순금·나뭇가지형/사슴뿔형 세움장식)을 사용했다.
- 이 페어는 앞선 세 페어(기술 변화·가치관 변화·사회 격변에 따른 양식 변화)와 다르게, "왕관 형식의 통일 여부"로 신라의 중앙집권 체제와 가야의 연맹체 성격이라는 정치적 차이를 추론하게 했다.
- **한계 기록**: 가야 금동관은 상설전시 안내 페이지 설명만 확보했고, 개별 소장품 검색 페이지(소장품번호·이미지 공공누리 조건)는 여러 검색 경로를 시도했지만 확정하지 못했다. 이 활동은 애초에 실물 사진을 쓰지 않는 설계라 기능상 문제는 없지만, 데이터 파일의 `sourceNote`·`license` 필드에 이 한계를 그대로 남겨 추측을 사실처럼 적지 않았다.
- 최초 claim 문구에서 "신라 금관은 신라 에서" 같은 어색한 조사 붙임 문제를 발견해, 빈칸 뒤에 항상 공백이 삽입되는 엔진 구조(`js/artifactComparison.js`)에 맞춰 "나라에서" 형태로 되돌려 자연스러운 문장이 되도록 고쳤다.
- `js/artifactComparison.js`는 이번에도 변경하지 않았다 — 데이터만 추가.
- 문서: `BACKLOG.md`에 확장 3 기록 추가, `TEACHING_artifact_comparison.md`를 4페어 기준 표로 갱신(가야 금동관 이미지 조건 한계도 명시).
- 검증: `01_validate_game_data.py`·`06_validate_static_assets.py` 통과. 로컬 정적 서버에서 4번째 페어의 톤 분기와 문장 조사 수정 결과를 브라우저 콘솔로 재확인했다.
- 다음: 무령왕릉 금제관식(국립공주박물관)의 정확한 소장품번호는 아직 확인하지 못해 5번째 페어 후보로 보류한다.

## 2026-09-08 — 유물 비교·추론 프로토타입 5번째 페어(무령왕 금제 관식 vs 신라 금관)

`TASK-20260908-CMP5 | 유물 비교 페어 확장(무령왕 금제 관식 vs 신라 금관) | Claude Sonnet 5 | 상태: DONE`

- 사용자가 무령왕 금제 관식 관련 링크 2개(한국민족문화대백과사전 「무령왕 금제 관식」, 국가유산청 국가유산포털 국보 상세)를 찾아 전달했다. 확인 결과: 국보 제154호, 국립공주박물관 소장, 백제, 순금 투각기법, 불꽃 모양+팔메트+연꽃무늬, 무덤 주인이 지석(매지권)으로 확인된 유일한 삼국시대 왕릉 출토품.
- 신라 쪽 짝은 4번째 페어에서 이미 확인한 금관총 금관(본관9435)을 재사용했다. 앞선 페어들과 다르게 "완성된 관 형태 vs 관에 꽂는 장식(관식)", "무덤 주인을 정확히 아는지 여부(무령왕릉 지석 vs 금관총의 무명)"라는 다섯 번째 해석 유형으로 프레이밍했다.
- 국가유산청 페이지에는 공공누리 유형이 명시돼 있지 않아 이번에도 개별 이미지 이용조건은 확인하지 못했다. `sourceNote`·`license` 필드에 이 한계를 그대로 남겼다(이미지 미사용이라 기능상 영향 없음).
- `js/artifactComparison.js`는 이번에도 변경하지 않았다 — 데이터만 추가. 총 5개 페어가 배열 순서대로 이어서 등장한다.
- 문서: `BACKLOG.md`에 확장 4 기록 추가, `TEACHING_artifact_comparison.md`를 5페어 기준 표로 갱신. 사용자와 "5개 페어로 일단 마무리하고 학생 피드백부터 받는다"로 합의했다.
- 검증: `01_validate_game_data.py`·`06_validate_static_assets.py` 통과. 로컬 정적 서버에서 5번째 페어의 톤 분기와 앞선 페어 완료 후 자동으로 이어서 등장하는 것을 브라우저 콘솔로 확인했다.
- 다음: 실제 수업에서 학생 피드백(EXP-002)을 받은 뒤 6번째 페어 진행 여부와 톤·난이도 조정을 결정한다.

## 2026-09-08 — 학생 피드백 구글 설문지 연결

`TASK-20260908-CMP6 | 학생 피드백 채널(구글 설문지) 연결 | Claude Sonnet 5 | 상태: DONE`

- 사용자가 "기존 성찰 일기 복사하기 버튼을 실제로 안 쓰고, 학생 입력이 어디로 남는지 모르겠다"고 지적했다. 이 정적 사이트(서버 없음)에서 목적지가 분명한 자동 수집을 하려면 외부 도구가 필요하다는 점을 설명하고, 구글 설문지 연결(B안)로 방향을 정했다.
- 사용자가 만든 구글 설문지 링크를 받아 `js/app.js`에 `window.FEEDBACK_FORM_URL`로 등록했다. 이 앱은 응답을 저장·수집하지 않고 링크만 새 탭으로 연다.
- "📮 오늘 활동 피드백 남기기" 버튼을 두 곳에 추가했다: MUD 공통 완료 화면(`js/mudEngine.js`의 `renderFinalReflection()`)과 유물 비교 결과 화면(`js/artifactComparison.js`의 `renderResultStep()`).
- 설문 문항(공통 6개 + 유물 비교 전용 7개, "유물 비교 활동"을 선택한 학생만 전용 섹션으로 분기)은 Claude가 초안해 채팅으로 전달했고, 사용자가 직접 구글 설문지에 옮겨 만들었다. 이메일 수집은 끄도록 안내했다(개인정보 미수집 원칙 유지).
- 기존 "성찰 일기 복사하기" 버튼은 제거 요청이 없어 그대로 두었다.
- 검증: `01_validate_game_data.py`·`06_validate_static_assets.py` 통과, 4개 JS 파일 문법 검사 통과. 로컬 정적 서버에서 MUD 완료 화면과 유물 비교 결과 화면 양쪽에 피드백 버튼의 `href`가 정확한 구글 설문지 URL로 렌더링되는 것을 브라우저 콘솔로 확인했다.
- 문서: `BACKLOG.md`에 기록 추가, `TEACHING_artifact_comparison.md`에 "학생 피드백은 구글 설문지로 받는다" 안내 추가.

## 2026-09-08 — 유물 비교 활동 시대 순서 버그 수정

`TASK-20260908-CMP7 | 유물 비교 페어 등장 순서 버그 수정 | Claude Sonnet 5 | 상태: DONE`

- 사용자가 "청자·분청사기 매병 페어(고려·조선)가 '삼국의 전성기 한강 쟁탈전' MUD(삼국시대) 클리어 직후에 등장한다"고 보고했다.
- 원인 확인: `data/artifactComparisons.json`의 모든 페어가 `unlockThreshold: 2`로 동일했다. `js/artifactComparison.js`의 `getEligibleComparison()`은 해금 유물 개수만 보고 배열 순서상 다음 미확인 페어를 그냥 띄우므로, 시대를 전혀 고려하지 않았다. 특히 한강 쟁탈전 MUD는 칠지도(art_5)·순수비(art_6) 2개를 한 번에 줘서 카운트가 급증해, 아직 이르지도 않은 고려·조선 페어가 조건을 만족해버렸다.
- 수정: 페어별 `unlockThreshold`를 시대 구간에 맞게 분리했다.
  - `cmp_pottery_1`(신석기 vs 청동기): 2 → 3 (청동기 MUD 직후)
  - `cmp_crown_1`·`cmp_crown_2`(삼국시대 신라·가야·백제): 2 → 6 (한강 쟁탈전 직후 정확히 도달하도록)
  - `cmp_ceramics_1`·`cmp_ceramics_2`(고려·조선 도자기): 2 → 8 (고려 문화 MUD 이후)
- `js/artifactComparison.js`는 변경하지 않았다 — `unlockThreshold`를 실제로 비교에 쓰고 있었으므로 데이터만 고치면 됐다.
- 검증: 브라우저 콘솔에서 `encyclopedia.data.unlockedArtifacts` 개수를 2/3/4/6/6(이전 페어 확인 후)/8로 바꿔가며 `ArtifactComparisonEngine.getEligibleComparison()`을 직접 호출해, 각 구간에서 같은 시대의 페어만 반환되는지 확인했다(2 → null, 3 → 토기, 6 → 신라·가야 왕관, 8 → 도자기). `01_validate_game_data.py`·`06_validate_static_assets.py` 통과.
- 문서: `BACKLOG.md`에 버그·수정 기록 추가, `TEACHING_artifact_comparison.md`에 시대 구간별 등장 순서 안내 추가.

## 2026-09-09 — 실시간 협동 MUD QR·수동 코드 입장 보안

`TASK-20260909-02 | QR·수동 코드 입장 보안 구현 | 담당: implementation agent | 상태: DONE`

- QR 입장을 6자리 코드와 분리해 `/join#entry=...`의 256비트 난수키로 바꿨다. Convex에는 SHA-256 해시만 저장하고 학생 화면은 fragment를 읽은 즉시 주소창에서 제거한다.
- QR·수동 코드는 최대 15분, 학생 복구 토큰은 최대 2시간 유효하다. 미리보기·시작·종료 시 입장을 잠그고 예약 정리 함수가 만료 QR 해시와 HMAC 시도 버킷을 제거한다.
- 수동 코드 실패는 서버 비밀키 HMAC 버킷으로만 센다. 브라우저별 5분 5회 뒤 10분, 코드 전체 5분 30회 뒤 5분 차단하며 코드·IP·User-Agent 원문은 저장하지 않는다.
- Convex 개발 배포에서 5회 실패 차단과 원문 비저장을 확인했다. Vercel Preview에서 fragment 제거·공통 오류를 확인했고 Runtime 로그에는 `/join` 경로만 남았다.
- 검증: lint·TypeScript·단위 테스트 12건·Next.js production build 통과, Convex 개발 배포 함수 준비 완료, Vercel Preview 배포 성공.

## 2026-09-09 — 실시간 협동 MUD 가상 21명 회귀 테스트

`TASK-20260909-03 | 가상 21명 Convex 동시성·삭제 회귀 테스트 | 담당: implementation agent | 상태: DONE`

- 운영 함수나 실제 학생 데이터를 추가하지 않고 공식 `convex-test`·Vitest·Edge Runtime 기반 함수 통합 테스트를 구성했다.
- 교사 세션 생성 뒤 같은 QR 입장키로 가상 학생 21명을 병렬 입장시키고, 서로 다른 호 21개와 4·4·4·4·5명 모둠, 모둠 안 역할 중복 없음, 교사 힌트 수신, 학생 단계 진행을 확인한다.
- 연결된 가상 입장 시도 1건을 포함해 세션을 종료한 뒤 `sessions`·`players`·`rooms`·`interventions`·`joinAttempts`가 모두 비는지 검증한다.
- 검증: ESLint, TypeScript, 기존 단위 테스트 12건, Convex 가상 학급 통합 테스트 1건, Next.js production build, `npm audit` 취약점 0건 통과.

## 2026-09-09 — `deep_three_kingdoms` 선택지 길이 편향 수정

`TASK-20260909-04 | deep_three_kingdoms 선택지 길이 편향 2건 수정 | 담당: content agent(Codex) | 상태: DONE`

- 1단계 황산벌과 5단계 기벌포의 짧은 오답에 각 장면에서 이미 확인한 역사 단서를 인용해, 정답 문장만 10자 이상 길어 보이던 편향을 제거했다.
- 분기·정답·다음 단계·시뮬레이터·역사 서술은 바꾸지 않았고 `data/mud/deep_joseon.json`·`data/mud/deep_modern.json`은 수정하지 않았다.
- 검증: `scripts/12_audit_choice_bias.py`에서 `deep_three_kingdoms:1/5`가 10자 이상 편향 목록에서 제거됐고, `scripts/03~09`의 MUD 무결성·계약·런타임·정적 자산·시간·카탈로그·탭 내성·출처 검사를 통과했다.

## 2026-09-09 — Deep-dive 역할 경계와 설계 결정 확정

`TASK-20260909-05 | Deep-dive PRD·DECISIONS 결정 정리 | 담당: audit agent(Codex) | 상태: DONE`

- `deep_three_kingdoms.json`은 Codex, `deep_joseon.json`·`deep_modern.json`은 Claude가 담당하는 소유권 경계를 확인했다.
- D-024로 Deep-dive 필수 활동을 "활동 자체에 역사적 판단이 있는 경우"로 제한하고, 단순 클릭 통행료형 게이팅은 금지했다.
- D-025로 교차 단원 Deep-dive는 대표 단원으로 축약하지 않고 `unitId: null`과 단원별 `spansUnits`를 사용하도록 방향을 정했다. 스키마·검증기 구현은 첫 후보 승인 뒤 별도 작업으로 남겼다.
- D-026으로 Deep-dive를 시대 순서의 긴 반복이 아니라 비교·관통·분류·사료 비판 같은 역사적 사고 질문 중심 탐구로 정의했다.

## 2026-09-09 — 모둠 미리보기 기능 `main` 통합 준비

`TASK-20260909-01 | 모둠 미리보기 기능 main 통합 | 담당: integration agent | 상태: DONE`

- `feat/cooperative-group-preview`가 `main`보다 정확히 한 커밋 앞서 있고 원격 브랜치와 일치함을 확인했다.
- 실시간 앱 `npm run lint`·`npm run typecheck`·단위 테스트 6건·프로덕션 빌드를 통과했다. 빌드는 샌드박스의 `spawn EPERM` 제한 때문에 승인된 외부 실행으로 재검증했다.
- 저장소의 JSON·MUD 무결성·계약·카탈로그·출처·정적 자산·시뮬레이터 런타임 검사와 협동 MUD·핵심 JavaScript 문법 검사를 모두 통과했다.
- `project_context.md`에서 7차시 실제 수업 완료, Convex/Auth0/Vercel 가상 통합 완료, 다음 코드 작업인 QR·수동 코드 입장 보안 상태를 최신화했다.

## 2026-09-09 — 유물 비교·추론 프로토타입 6번째 페어(김홍도 vs 신윤복 풍속화)

`TASK-20260909-CMP8 | 유물 비교 페어 확장(김홍도 vs 신윤복) | Claude Sonnet 5 | 상태: DONE`

- Codex는 협동 MUD 실시간 서버(Convex) 트랙, 이 세션은 유물 비교 트랙으로 역할을 재확인한 뒤 6번째 페어를 추가했다.
- 김홍도 「논갈이」(단원 풍속도첩, 본관6504-1)와 신윤복필 「여속도첩」(덕수1103)을 museum.go.kr에서 확인했다. 둘 다 국립중앙박물관 소장, 공공누리 출처표시만 적용(변경금지 없음). 신윤복의 대표작 《혜원전신첩》(국보135호)은 간송미술관 소장이라 이번 페어에서 제외하고, 국립중앙박물관 소장품인 「여속도첩」을 짝으로 골랐다.
- 앞선 다섯 페어(기술 변화·가치관 변화·사회 격변·정치 체제 차이·기록 방식)와 다르게, "같은 조선 후기 화가여도 무엇을 눈여겨봤는지가 다른 이유"(김홍도=서민 남성의 노동, 신윤복=여성의 일상)라는 여섯 번째 해석 유형을 추가했다.
- `unlockThreshold: 11`로 설정해 조선 후기 서민 문화 MUD(art_11 보상) 이후에만 등장하도록 시대 구간을 맞췄다.
- 구현 중 claim 문장의 빈칸 뒤에 조사가 공백을 사이에 두고 어색하게 붙는 문제(`노동 을(를) 그렸고`)를 발견해, "모습을 그렸고" 형태로 middle/suffix를 다듬어 자연스러운 문장이 되도록 고쳤다.
- `js/artifactComparison.js`는 변경하지 않았다 — 데이터만 추가.
- 문서: `BACKLOG.md`에 확장 5 기록 추가, `TEACHING_artifact_comparison.md`를 6페어 기준으로 갱신.
- 검증: 브라우저 콘솔에서 `getEligibleComparison()`을 유물 개수 10/11로 호출해 임계값 경계를 확인했고(10 → 이전 페어, 11 → 신규 페어), 실제 결과 화면까지 진행해 근접/차이 두 톤 분기와 문장 수정 결과를 확인했다. `01_validate_game_data.py`·`06_validate_static_assets.py` 통과.
- 참고: 같은 작업 폴더에서 다른 세션이 `.worktrees/`를 만들어 격리 작업을 시작한 것을 확인했다 — 공유 폴더 충돌을 줄이는 올바른 방향이다.

## 2026-09-09 — 6번째 페어 콘텐츠 적절성 재검토

`TASK-20260909-CMP9 | 유물 비교 6번째 페어 콘텐츠 안전성 수정 | Claude Sonnet 5 | 상태: DONE`

- 사용자가 6번째 페어(김홍도 vs 신윤복) 추가 직후 "이 그림이 초등학생 수준인가?"라고 확인 질문을 던졌다. 다시 조사한 결과, 처음 고른 신윤복 「여속도첩」은 국립중앙박물관 소장 화첩 6면 중 **3면이 기녀(妓女)를 그린 것**이라고 박물관이 직접 소개하고 있었다. 처음 작성한 데이터는 화첩 전체를 뭉뚱그려 "몸단장·나들이 등 여성의 일상"으로만 서술해, 실제로 어느 면을 가리키는지 특정하지 않은 채 부정확하게 대표시킨 상태였다.
- 사용자와 다시 확인한 결과: 김홍도·신윤복이라는 "당대를 대표하는 인물화가" 구도는 유지하되, 안전이 확인된 특정 그림으로 좁히기로 했다. 국립중앙박물관이 "기녀가 아닌 일반 부녀자를 그린 것"으로 공식 소개하는 3면 중 「저잣길」(생선 함지박을 인 여성 상인이 채소 망태기를 낀 채 대화하는 장면)을 특정해 다시 작성했다.
- 관찰 선택지·근거 카드·claim 빈칸 문구를 모두 "여성의 일상"(모호함)에서 "시장에서 장사하는 여성"(구체적)으로 고쳤다. 새로운 근거 카드(`ev_rarity`, "그 화가가 평소에 잘 그리지 않던 소재인지")도 추가해, 신윤복이 평소 화려한 인물을 주로 그렸다는 사실을 숨기지 않으면서도 이 그림이 그중 드문 예외임을 정직하게 설명하도록 했다.
- **링크 안전성도 별도로 점검**: 결과 화면의 "실물 보기" 링크가 국립중앙박물관 소장품 페이지로 가면 화첩의 다른(기녀) 면 이미지도 함께 노출될 수 있어, 신윤복 쪽 링크를 한국민족문화대백과사전의 신윤복 소개 글로 바꿨다. WebFetch로 그 페이지의 유일한 이미지가 「미인도」(전통 복식을 입은 여성 초상화, 성적 요소 없음)임을 확인한 뒤에 반영했다.
- claim 템플릿의 조사 결합 문제("노동 을(를)")도 이번 수정에서 근본적으로 없앴다 — 선택지 문구 자체에 조사를 포함시켜(`"농사짓는 남성의 모습을"` 등) 템플릿의 강제 공백과 무관하게 항상 자연스러운 문장이 되도록 설계를 바꿨다. 이 패턴은 이후 페어에도 재사용할 수 있다.
- 검증: 브라우저 콘솔에서 결과 화면까지 재실행해 문장·톤 분기·실물 보기 링크(김홍도→museum.go.kr, 신윤복→encykorea.aks.ac.kr) 3개를 모두 확인했다. `01_validate_game_data.py`·`06_validate_static_assets.py` 통과.
- 교훈: 유물 하나를 "화첩·화첩" 단위로 뭉뚱그려 인용하면, 그 안에 섞인 다른 내용까지 암묵적으로 끌어들일 수 있다. 다음 페어부터는 화첩·묶음 소장품을 쓸 때 반드시 어느 낱장·어느 면을 가리키는지 박물관 설명에서 확인하고 명시한다.

## 2026-09-09 — 나머지 5개 유물 비교 페어 콘텐츠 재검토

`TASK-20260909-CMP10 | 유물 비교 5개 페어 콘텐츠 적절성 재검토 | Claude Sonnet 5 | 상태: DONE`

- 신윤복 페어 수정 뒤, 사용자 요청으로 나머지 5개 페어(토기 2종, 도자기 2종, 왕관·관식 2종)도 같은 기준으로 재검토했다. 각 museumUrl을 직접 열어 단일 유물 사진(다각도)뿐인지, 묶음·화첩처럼 다른 콘텐츠가 섞여 있지 않은지 확인했다.
- 결과: 5개 전부 이상 없음. 전부 토기·도자기·금관 같은 단일 물체라 화첩처럼 여러 장면이 섞이는 구조가 아니었다. 서술 문구도 기술·양식·정치체제 설명뿐이라 별도 수정 없음.
- `TEACHING_artifact_comparison.md`에 "새 페어(또는 다른 활동) 제작 전 콘텐츠 점검 체크리스트"를 추가했다 — 화첩·묶음 소장품 여부, 인물·장면 묘사 여부, 바깥 링크가 실제로 보여주는 것, 서술 문구의 폭력·성적 요소 여부를 항목화했다. 이후 다른 활동을 만들 때도 재사용한다.

## 2026-09-09 — 유물 비교 페어 등장 조건 근본 수정(마커 유물 기반)

`TASK-20260909-CMP11 | 유물 비교 페어 등장 로직 근본 수정 | Claude Sonnet 5 | 상태: DONE`

- 사용자가 스크린샷으로 "을사의병·안중근 MUD(근현대) 클리어 직후에 '같은 매병, 다른 시대'(고려·조선 도자기 페어)가 뜬다"고 보고했다.
- 원인 재분석: 2026-09-08에 페어별 `unlockThreshold`를 시대 구간에 맞게 분리하는 수정을 했지만, 그건 어디까지나 "학생이 1단원→2단원→3단원 순서대로, 빠짐없이 플레이한다"는 가정에 의존했다. 이 앱은 포털에서 어떤 차시든 자유롭게 선택해 플레이할 수 있어(비선형 진행 가능), 학생이 근현대 MUD를 먼저 여러 개 클리어하면 `unlockedArtifacts.length`가 임계값을 훌쩍 넘어버려 무관한 시대의 페어가 튀어나올 수 있었다. 개수는 "진도"의 신뢰할 수 있는 대리 지표가 아니었다.
- **근본 수정**: `js/artifactComparison.js`의 `getEligibleComparison()`을 다시 짰다. 이제 각 페어는 `requiredArtifactNames`(그 시대를 다루는 MUD가 실제로 주는 보상 유물의 정확한 이름, id 병기)를 갖고, 학생이 그 유물을 **실제로** 갖고 있어야만(개수와 무관하게) 뜬다.
  - `cmp_pottery_1` → "고창 탁자식 고인돌"(art_3, 청동기 MUD)
  - `cmp_ceramics_1`·`cmp_ceramics_2` → "직지심체요절 영인본과 고려 비색 청자"(art_8, 고려 문화 MUD)
  - `cmp_crown_1`·`cmp_crown_2` → "백제 칠지도" 또는 "신라 북한산 순수비"(art_5/6, 한강 쟁탈전 MUD)
  - `cmp_folk_paintings_1` → "김홍도 서당도 영인본과 하회탈"(art_11, 조선 후기 서민 문화 MUD)
- 각 MUD json의 실제 `rewards[].name` 문자열을 `grep`으로 직접 확인해 정확히 일치시켰다(`encyclopedia.unlockArtifact()`가 id가 아니라 name으로 저장하는 경우가 많아서, name·id 둘 다 매치 대상에 넣었다).
- `unlockThreshold`는 지우지 않고 `requiredArtifactNames`가 없는 콘텐츠를 위한 안전장치(대체 로직)로 남겼다.
- 검증: 브라우저 콘솔에서 (1) 근현대 유물 9개만 있고 마커 없음 → `null`(아무것도 안 뜸), (2) 거기에 고려 문화 MUD 마커를 추가 → 정확히 `cmp_ceramics_1`만 뜸, (3) 각 페어를 해당 마커 하나씩만으로 개별 테스트해 전부 정확히 매치, (4) `crown_1`을 본 뒤 같은 마커로 `crown_2`가 이어지는 것도 확인. 최초 테스트에서 캐시된 구버전 스크립트를 오래 붙잡고 있었던 걸 발견해 `index.html`의 `artifactComparison.js` 캐시 버스터를 `20260909-eratrigger1`로 올렸다 — 이 저장소의 고질적인 캐시 문제를 또 겪었으므로, 다음에도 스크립트를 고친 뒤에는 반드시 `location.reload(true)`나 캐시 버스터 확인부터 한다.
- `01_validate_game_data.py`·`06_validate_static_assets.py` 통과.

## 2026-09-09 — 유물 비교 3개 페어 추가(7~9번째)

`TASK-20260909-CMP12 | 유물 비교 페어 확장(앙부일구·자격루, 구석기·신석기 도구, 청동·철검) | Claude Sonnet 5 | 상태: DONE`

- 근본 수정(마커 기반 트리거) 뒤 새 아이디어 4개(세종 과학기구, 구석기·신석기 도구, 청동·철 금속기술, 홍패·투표함)를 브레인스토밍했고, 사용자가 "2→1→4→3" 순서로 진행을 요청했다.
- **7번째(앙부일구 vs 자격루)**: museum.go.kr에서 확인하다가 중요한 사실을 발견했다 — 세종 때(1430년대) 처음 발명됐지만, 지금 박물관에 남은 실물은 둘 다 그 시절 원본이 아니라 후대에 다시 만든 것이다(앙부일구=1871년 강건 제작본, 자격루=1536년 중종 때 남은 부속). 이 사실을 숨기지 않고 traits의 `era` 필드와 학계 관점 문구에 정직하게 담았다 — "발명은 세종 때, 실물은 후대"라는 점을 근거 카드(`ev_current_relic_age`)로도 만들었다.
- **8번째(주먹도끼 vs 바퀴날도끼)**: 게임 속 art_1(주먹도끼)의 출토지가 "연천 전곡리"인데, 처음 찾은 museum.go.kr 후보(relicId=2486)는 파주 출토품이라 다시 검색해 연천군 출토가 명시된 relicId=31578173(신수51599)로 바꿨다. 이런 사소한 출토지 불일치도 그냥 넘기지 않고 재확인했다.
- **9번째(비파형 동검 vs 고리자루 큰 칼)**: 이전 브레인스토밍에서 "무기 소재라 톤 설계 주의 필요"라고 미리 남겨둔 항목이었다. 두 유물 모두 국립중앙박물관 자체 설명이 "실제 전투보다 신분·권위의 상징으로 무덤에 함께 묻혔다"고 밝히고 있어서, 그 설명을 그대로 근거 삼아 무기 대결이 아니라 금속 기술 발전(청동→철)으로 서술을 한정했다. 우려했던 톤 문제를 실제 자료로 해소한 사례다.
- **10번째 후보(고려 홍패 vs 5·10 총선거 투표함)는 보류**했다. 현존하는 고려 시대 홍패 실물 6건 전부가 국립중앙박물관이 아니라 개인·문중 소장(보물 지정)이었고, 국립중앙박물관 소장 홍패는 조선 시대(1814년)뿐이었다. 투표함도 소장처를 확인하지 못했다. 두 소장처 모두 확인 안 된 상태로 만들지 않고, 사용자에게 상황을 그대로 보고한 뒤 "3번은 건너뛰고 나머지만 커밋"으로 정리했다.
- 세 페어 모두 `requiredArtifactNames`(2026-09-09 근본 수정에서 만든 방식)를 바로 적용해서 만들었다 — `js/artifactComparison.js`는 이번에도 변경 없음, 데이터만 추가.
- 검증: 브라우저 콘솔에서 각 마커 하나만으로 정확한 페어가 매치되는지, 결과 화면까지 진행해 문장·두 톤 분기를 페어마다 확인했다. `01_validate_game_data.py`·`06_validate_static_assets.py` 통과.
- 문서: `BACKLOG.md`에 확장 6~8 기록, `TEACHING_artifact_comparison.md`를 9페어 기준 표로 갱신(10번째 보류 사실도 명시).

## 2026-09-09 — 확장 역사 활동(타임머신 스토리·퀴즈·미니게임) Tailwind 미적용 스타일 깨짐 수정

`TASK-20260909-06 | 확장 역사 활동 스타일 깨짐 수정 | Claude Sonnet 5 | 상태: DONE`

- 사용자가 스크린샷으로 "타임머신 스토리" 카드 목록이 배경·테두리·버튼 스타일 없이 밋밋한 텍스트로만 보인다고 보고했다.
- 원인: `js/storyEngine.js`가 `bg-stone-800`, `rounded-2xl`, `grid-cols-3` 등 Tailwind 유틸리티 클래스로 마크업을 그리는데, `index.html`에는 Tailwind가 로드된 적이 없었다. 같은 문제가 `js/quizGame.js`(골든벨 퀴즈)와 `js/miniGames.js`(유물 카드 짝맞추기·연표 순서 맞추기의 실제 게임 화면)에도 있었다.
- 수정: 세 파일 모두 사이트 기존 커스텀 팔레트로 다시 작성했다 — 스토리·미니게임은 확장 역사 활동 박스의 다크 톤(`#F7E7CE`/`#1F1B19`/`#5A4E46`/`#B7791F`), 퀴즈는 모달의 라이트 톤(`--card-bg`/`--accent-red`/`--accent-teal` CSS 변수)에 맞췄다. MUD 엔진이 쓰던 `.btn`/`.btn.secondary`/`.choice-marker` 클래스를 그대로 재사용해 선택지 버튼 스타일을 통일했다.
- 유물 카드 3D 뒤집기는 Tailwind 임의 유틸(`rotate-y-180` 등, 플러그인 없이는 원래도 작동 안 함)을 대체하는 순정 CSS(`.card-flip-wrap`/`.card-flip-inner`/`.card-flip-face`)를 `css/style.css`에 추가했다.
- 검증: 로컬 정적 서버에서 각 기능을 JS로 직접 구동하며 computed style/DOM 상태를 확인했다 — 타임머신 스토리 카드 목록·진행 화면(선택지·타자 효과·보상 알림), 골든벨 퀴즈 진행·피드백·결과 화면, 유물 카드 짝맞추기(플립 애니메이션·매칭)·연표 순서 맞추기(선택·스왑·정오답 피드백) 전부 정상 동작 확인.
- 배포 확인: 이 수정은 `feat/cooperative-group-preview` 브랜치에만 있었고 GitHub Pages는 `main`만 배포하므로, 실제 배포 페이지의 `js/storyEngine.js`를 fetch해 옛 버전임을 확인해 사용자에게 보고했다. main 병합은 별도 요청 시 진행하기로 함(→ 이번 병합 커밋에서 반영).

## 2026-09-09 — 원인과 결과 순서 맞추기 세 번째 미니게임 추가

`TASK-20260909-07 | 원인과 결과 순서 맞추기 미니게임 기획·구현 | Claude Sonnet 5 | 상태: DONE`

- 사용자가 "다른 원인 결과 스토리 순서 활동을 기획하자"고 요청. 기존 "역사 연표 순서 맞추기"(연도 표시, 연대 암기로 풀 수 있음)와 차별화 지점을 먼저 확인한 뒤(연도 비노출, 인과 논리로만 채점) 진행했다.
- 기획 문서 `docs/plans/implementation_plan_cause_effect_order_game.md` 작성 — 콘텐츠 후보(3단원 35~45차시 기반 2세트), UI 재사용 설계, 그리고 `docs/audits/historical_language_audit.md`가 경고하는 "단일 원인·과도한 인과 서사" 위험을 어떻게 피할지(카드 문구에 복수 요인 암시, 정답 화면에 "다섯 사건만으로 설명되지 않는다" 고정 안내)를 명시했다. 사용자가 문구 초안과 스테이지 2개 전부 진행을 승인.
- `data/causeEffectChains.json` 신규: 스테이지 2개(을사늑약~임시정부 수립, 8·15 광복~정전 협정), 각 5개 사건, `year` 필드 없음.
- `js/miniGames.js`에 `startCauseEffectGame`/`renderCauseEffectUI`/`handleCauseEffectClick`/`checkCauseEffectOrder` 4개 함수를 연표 게임과 동일한 클릭 교환 UI 패턴으로 추가.
- `js/encyclopedia.js`에 `badge_causality_master`(🔗 인과 관계 탐정) 배지 추가. `index.html` 미니게임 카드에 버튼·컨테이너 추가.
- 검증: 정적 서버에서 스테이지 1을 셔플→클릭 교환→정답 확인→배지 해금→스테이지 2 진입까지 전부 실행해 카드에 연도가 전혀 노출되지 않음과 판정 로직을 확인. `01_validate_game_data.py`는 신규 JSON을 검사 대상에 포함하지 않아 별도로 JSON 유효성만 확인(다음에 검증 스크립트 대상 추가를 고려할 것).

## 2026-09-09 — 유물 탐정 미니게임 신규 추가

`TASK-20260909-CMP13 | 유물 탐정 미니게임 구현 | Claude Sonnet 5 | 상태: DONE`

- 유물 비교 활동과 별개로, "유물 관련 다른 활동 아이디어"를 브레인스토밍(개인 전시 준비실 / 유물 탐정 / 내 유물 연표 3개)한 뒤 사용자가 "지금 학생 수준에 가장 적절하다"고 고른 **유물 탐정: 이게 뭘까?**를 미니게임으로 구현했다.
- 개인 전시 준비실(1번)은 구현 전에 사용자가 "학생 입력이 어디 저장되는지"를 먼저 물어, `localStorage`뿐이라 서버 제출 경로가 필요하다는 걸 미리 답하고 이번엔 보류했다 — 나중에 만들 때는 구글 폼 같은 제출 경로를 처음부터 같이 설계하기로 했다.
- 설계: 고정 콘텐츠가 아니라 **그 학생이 실제로 해금한 유물만** 출제한다(해금 4개 미만이면 "더 모으고 와" 안내). 힌트 3단계(시대→종류→설명 일부)를 순서대로 공개하는 4지선다형. 오답이면 감점 없이 다음 힌트가 자동으로 열리고, 힌트를 다 봐도 틀리면 정답을 알려주고 다음 문제로 넘어간다.
- 기존 미니게임 3종(카드 짝맞추기·연표 순서·인과관계 순서)과 같은 클래스(`MiniGameEngine`)·같은 컨테이너/버튼 패턴을 그대로 따라 `js/miniGames.js`에 추가했다(새 파일 없음).
- 배지 체계도 기존 3개(발굴 전문가·시간의 지배자·인과 관계 탐정)와 나란히 `badge_detective_master`(유물 감식가, 전 문제를 힌트 1개로 맞혔을 때만 획득)를 `js/encyclopedia.js`에 추가했다.
- 검증: 로컬 정적 서버에서 (1) 해금 유물 3개일 때 안내 문구, (2) 6개일 때 5라운드·4지선다 정상 생성, (3) 오답 시 힌트 자동 증가, (4) 정답 시 힌트 단계별 격려 문구 분기, (5) 5라운드 전부 힌트1로 완료 시 배지 획득까지 브라우저 콘솔로 전부 확인했다. 콘솔 에러 없음. `01_validate_game_data.py`·`06_validate_static_assets.py` 통과.
- 아직 안 만든 것: 개인 전시 준비실(1번), 내 유물 연표 만들기(3번) — 다음에 이어갈 후보로 남긴다.

## 2026-09-09 — Claude Sonnet 5 세션 마무리(유물 비교·미니게임 트랙)

이 세션에서 유물 비교 트랙 전체 작업을 정리한다. Codex는 같은 날 협동 MUD 실시간 서버(Convex) 트랙을 별도로 진행했다(`apps/cooperative-live/`, `feat/cooperative-group-preview` 등 별도 브랜치 이력 참조).

**오늘 만든 것 (전부 `main`에 배포·검증 완료)**

- 유물 비교·추론 프로토타입을 6번째 → 9번째 페어까지 확장:
  - 6. 김홍도 「논갈이」 vs 신윤복 「저잣길」(조선 후기 화가의 서로 다른 관심사)
  - 7. 휴대용 앙부일구 vs 보루각 자격루(세종 발명품, 실물은 둘 다 후대 제작본이라는 사실 포함)
  - 8. 주먹도끼(구석기) vs 바퀴날도끼(신석기)(뗀석기→간석기 기술 변화)
  - 9. 비파형 동검(청동) vs 고리자루 큰 칼(철)(무기 대결이 아닌 금속 기술·상징물로 서술)
  - 보류: 고려 홍패 vs 5·10 총선거 투표함(국립중앙박물관 소장 확인 실패로 제외)
- **콘텐츠 안전성 재검토**: 사용자 질문("초등학생 수준인가?")을 계기로 신윤복 페어를 「여속도첩」 전체(기녀 3면 포함)에서 「저잣길」(일반 부녀자로 공식 소개된 면) 하나로 좁혀 다시 작성. 나머지 5개 페어도 같은 기준으로 재검토해 이상 없음 확인. 재사용 가능한 "콘텐츠 점검 체크리스트"를 `TEACHING_artifact_comparison.md`에 추가.
- **버그 근본 수정**: 페어 등장 조건을 "해금 유물 개수 임계값"에서 "그 시대 MUD가 실제로 주는 보상 유물 이름"(`requiredArtifactNames`) 기반으로 바꿔, 학생이 차시를 비선형으로 진행해도 무관한 시대 페어가 뜨지 않게 했다(을사의병·안중근 MUD 직후 도자기 페어가 뜨던 재발 버그를 근본적으로 해소).
- **학생 피드백 채널**: 구글 폼(`window.FEEDBACK_FORM_URL`)을 MUD 공통 완료 화면과 유물 비교 결과 화면에 연결.
- **신규 활동 — 유물 탐정 미니게임**: 유물 비교와 별개로 새 메커닉을 브레인스토밍(개인 전시 준비실/유물 탐정/내 유물 연표)한 뒤, 학생 수준에 가장 적절하다고 판단된 "유물 탐정: 이게 뭘까?"를 구현(그 학생이 실제로 해금한 유물만 출제, 힌트 3단계, 4지선다).

**작업 환경 관련 — 다음 세션이 알아둘 것**

- 이 폴더는 세션 도중 계속 `feat/cooperative-group-preview`(Codex 브랜치)에 있었다. 공유 폴더 브랜치를 임의로 바꾸지 않는다는 원칙에 따라, 유물 비교·미니게임 관련 커밋은 전부 그 브랜치에서 만든 뒤 **`git worktree`로 별도 격리 공간을 만들어 `main`에만 cherry-pick**하는 방식으로 배포했다(Codex의 협동 MUD 관련 커밋은 섞이지 않도록 매번 `git diff origin/main --stat`로 반영 범위를 확인함).
- 배포 후 실제 사이트 확인 시 **브라우저가 옛 스크립트를 캐싱**하는 문제가 반복됐다 — `<script>` 태그의 캐시 버스터 쿼리스트링이 바뀌었는지, 그리고 필요하면 `location.reload(true)`로 강제 새로고침했는지 먼저 확인한다.
- 다음에 이어갈 것: 학생 피드백(구글 폼) 응답 누적 대기, 개인 전시 준비실(제출 경로부터 설계), 내 유물 연표 만들기.

## 2026-09-09 — `deep_joseon:1` 선택지 편향 수정 + Codex 핸드오프 작성

`TASK-20260909-08 | deep_joseon 선택지 편향 수정 + Codex 역할 핸드오프 | Claude Sonnet 5 | 상태: DONE`

- `deep-dive-competition-task`(2026-09-01) 결정의 범위가 일주일째 미확정 상태였던 것을 사용자에게 재확인했다: `deep_three_kingdoms.json`은 Codex 소유 유지, `deep_joseon.json`·`deep_modern.json`은 Claude가 감사·개선 착수로 확정.
- `deep_joseon.json`·`deep_modern.json` 구조 감사: 예상과 달리 `deep_joseon`은 이미 8개 스테이지 전부 `ordered-hotspot` 실제 게이팅이 걸려 있었고, `deep_modern`도 7개 중 4개가 동일 수준·3개(`precise-taegeukgi`×2·`precise-vote`)는 정밀 조작형의 다른 방식 게이팅이라 문제로 보지 않았다.
- `scripts/12_audit_choice_bias.py`가 미해결로 남겨 둔 `deep_joseon:1`(정답이 오답보다 12자 길어 자료 안 읽고도 정답 티남)을 "선택지 단서 인용형 재작성" 기법으로 수정. 재실행으로 목록에서 빠진 것 확인.
- `docs/handoff/codex_deep_three_kingdoms_ownership_handoff.md` 신규 작성: 위 역할 분담, `deep_three_kingdoms:1/5`에 남아있던 같은 종류의 편향(Codex 몫), PRD.md/DECISIONS.md 개정이 필요한 결정 3건(Deep-dive 필수 게이팅 허용 여부·교차 단원 스키마·Deep-dive 정의 개정)을 전달.
- **정정 1건**: 문서 작성 중 `codex-deep-three-kingdoms` 브랜치 삭제를 제안하려다, `git log codex-deep-three-kingdoms ^main`으로 먼저 확인해 그 브랜치에 `deep_three_kingdoms.json`과 무관한 실제 협동 MUD 기능 2건(뒤로가기 네비게이션, 추가 미션 — 아래 TASK-10 참고)이 미반영 상태로 남아있음을 발견하고 삭제 제안을 철회·정정했다.
- `docs/handoff/claude_deep_dive_session_continuation.md` 갱신(별도 워크트리 `claude-deep-prehistoric`) — 역할 분담 확정 사실 반영.
- 검증: `01/03/04/06/08/09_validate_*`, JS 문법 검사 통과.

## 2026-09-09 — Codex 작업 통합(선택지 편향 2건 + PRD/DECISIONS 3건)

`TASK-20260909-09 | Codex 완료 작업 검증·통합 | Claude Sonnet 5 | 상태: DONE`

- 위 핸드오프에 대해 Codex가 `fix/deep-three-choice-bias` 브랜치에서 응답: `deep_three_kingdoms:1/5` 편향 수정(`9ef85ae`)과 D-024(Deep-dive 필수 게이팅은 판단이 있을 때만)·D-025(교차 단원은 `unitId: null` + `spansUnits`)·D-026(사고축 구획 정의) 채택 + PRD.md 개정(`4220cd0`)을 완료했다는 보고를 사용자로부터 전달받았다.
- 두 커밋을 그대로 신뢰하지 않고 `git show`로 직접 diff를 확인한 뒤(내용이 보고와 정확히 일치) cherry-pick으로 가져왔다. 중복 커밋(`7df22e7`, 내 커밋의 재적용본)은 Codex 안내대로 건너뛰었다.
- `scripts/12_audit_choice_bias.py` 재실행: 10자 이상 편향 **0건**(기존 3건 전부 해소).
- 검증: 정적 검증 전체(01/03/04/06/08/09/12) + 시뮬레이터 런타임 + JS 문법 통과.

## 2026-09-09 — `main` 통합(오늘 작업 3건) + 배포 반영

`TASK-20260909-10 | 오늘 작업 3건 main 병합·배포 | Claude Sonnet 5 | 상태: DONE`

세 차례에 걸쳐 격리 워크트리를 만들어 `main`에 병합·검증·배포했다(공유 작업 폴더의 `feat/cooperative-group-preview` 체크아웃은 건드리지 않음).

1. **1차 병합**(`3b076af`): Tailwind 스타일 깨짐 수정 + 원인과 결과 순서 맞추기 미니게임. `walkthrough.md`·`project_context.md` 충돌 2건은 양쪽 로그를 모두 보존해 해결.
2. **2차 병합**(`ee77e94`): `deep_joseon:1` 편향 수정 + Codex의 `deep_three_kingdoms:1/5` 편향 수정·PRD/DECISIONS 개정. `walkthrough.md` 순수 추가형 충돌만 있어 전부 보존.
3. **3차 병합**(`5067620`·`6636734`): `codex-deep-three-kingdoms`에서 발견한 미반영 협동 MUD 기능 2건(뒤로가기 네비게이션, 추가로 해볼 것 선택 미션)을 cherry-pick. `cooperative-mud/gojoseon-law/app.js`·`index.html`에서 실제 코드 충돌 발생 — main이 그 사이 추가한 타이머 페이싱 기능(`CoopPacing`)과 뒤로가기 기능이 `setScreen()`의 같은 지점을 건드려서, 두 기능 호출을 모두 유지하는 방향으로 직접 해결했다. 로컬 서버로 화면 전환 시 뒤로가기 버튼 노출·클릭 동작·추가 미션 토글을 브라우저로 직접 재생해 확인, 콘솔 에러 없음.
4. 매 병합 전 `git rev-list --left-right --count origin/main...main`으로 그 사이 다른 푸시가 없었는지 확인 후 push, 매 push 후 `gh api repos/.../pages/builds/latest`로 빌드 성공을 폴링하고 배포된 파일을 직접 fetch해 반영을 확인했다.
- **문서 처리 점검(2026-09-09 후속)**: 사용자가 "문서 처리 했는지" 확인 질문을 던져 재점검한 결과, `walkthrough.md`에 이 병합 작업 자체와 `deep_joseon:1` 수정 건에 대한 로그가 빠져 있었고, 내 `TASK-20260909-04/05`가 Codex가 독립적으로 쓴 `TASK-20260909-04/05`(QR 보안·가상 21명 테스트가 아니라 실제로는 `deep_three_kingdoms` 편향 수정·Deep-dive 결정)와 번호가 겹쳐 있었다. 이 절(TASK-08~10)로 보강하고, 겹치던 두 항목은 `TASK-20260909-06`·`-07`로 재번호했다(과거 커밋 메시지 텍스트 자체는 고치지 않음 — 로그 문서만 정정).

## 2026-09-09 — GitHub README 현행화

`TASK-20260909-README | README 현행 기능·실행·배포 상태 최신화 | documentation agent(Codex) | DONE`

- `README.md`의 기준일을 2026-09-09로 갱신하고 32개 MUD, 정적 협동 MUD 3편, 유물 비교 9페어, 미니게임 4종을 실제 데이터와 맞췄다.
- 실시간 협동 앱을 “설계 중·미구현”으로 설명하던 내용을 Next.js·Convex·Auth0 기술 Preview 구현 상태로 바로잡았다. 실제 학생 사용은 `P1-COLLAB-PRIVACY` 해제 전까지 금지한다는 운영 경계를 함께 명시했다.
- 정적 포털과 실시간 앱의 로컬 실행·검증 명령, 실시간 앱 상세 README 링크를 추가했다.
- 검증: README 로컬 링크 37개 존재 확인, `scripts/01·03·04·06·08·09`과 시뮬레이터 런타임 검사 통과, 실시간 앱 `npm run check`로 lint·TypeScript·단위 테스트 12건·Convex 통합 테스트 1건·production build 통과.

## 2026-09-10 — 시대 매칭 버그 3차 수정 + 8조법 정정 + 유물↔유적 연결

`TASK-20260910-CMP14 | 유물 비교 시대 매칭 근본 원인 보완 + 유물↔유적 연결 신규 | Claude Sonnet 5 | 상태: DONE`

- 사용자가 스크린샷 2장으로 "두 나라의 왕관(삼국시대) 페어가 3·1 운동·대한민국 정부 수립(근현대) MUD 직후에도 뜬다"고 보고했다. 지난(9-09) 마커 기반 수정은 "그 시대 MUD가 실제로 준 유물을 갖고 있는가"는 보장했지만, "방금 끝낸 MUD가 그 시대인가"는 애초에 확인하지 않았다는 걸 사고실험으로 먼저 검증한 뒤 코드를 고쳤다.
- 데이터를 더 파다가 두 번째 원인도 찾았다: `cmp_metal_tech_1`(비파형동검 페어)이 자기 유물(art_4)이 아니라 `cmp_crown_1`의 마커(칠지도·순수비)를 복붙해 갖고 있어서, 고조선 MUD를 한 번도 안 한 학생에게도 뜰 수 있었다.
- 수정: `js/artifactComparison.js`에 `getEraGroup()`(선사~근현대 5개 묶음)을 추가하고 `getEligibleComparison(currentEraGroup)`이 시대 불일치 페어를 후보에서 아예 제외하도록 바꿨다. `js/mudEngine.js`의 `renderFinalReflection()`이 방금 끝낸 MUD의 실제 보상 유물 era를 `artifacts.json`에서 찾아 `currentEraGroup`을 계산해 넘긴다. `cmp_metal_tech_1`의 마커를 정정했고, `cmp_crown_2`(실물 보상이 따로 없는 페어)는 사용자 확인에 따라 삼국시대 보상 여러 개 중 OR 유지 + eraGroup 보호막을 추가했다.
- 검증: 브라우저 콘솔에서 사고실험 5개 시나리오(스크린샷 재현이 null이 되는지, 정상 삼국시대 MUD 뒤엔 제대로 뜨는지, crown_1 완료 뒤 crown_2로 이어지는지, metal_tech_1이 정확한 조건에서만 뜨는지, currentEraGroup 미확인 시 안전하게 미노출되는지)를 전부 재현·확인했다.
- 이어서 사용자가 "8조법 목판?"이라고 물어, Deep-dive 보상 `art_deep_1`의 "8조법 목판" 표기가 실물 근거 없는 단정이라는 걸 확인(8조법은 『한서』에 3개 조항만 전해지는 문헌 기록, 실물 목판 발굴 사례 없음). `data/artifacts.json`·`data/mud/deep_prehistoric.json`의 이름을 "…전해오는 8조법"으로 정정하고 desc에 사실 관계를 명시했다.
- 마지막으로 "유물만큼 유적도 많고 웹에 좋은 사진이 많다"는 요청에 유물↔유적(발견 장소) 연결 기능을 추가했다. 사진을 직접 삽입하지 않고(저작권 부담) 국가유산포털(heritage.go.kr) 공식 사적 페이지로만 링크하는 방식을 사용자가 선택했다. 6개 유적(서울 암사동, 부여 송국리, 연천 전곡리, 경주 대릉원 일원, 고령 지산동 고분군, 공주 무령왕릉과 왕릉원)을 heritage.go.kr에서 직접 열어 사진·설명이 실제로 맞는지 확인한 뒤 `data/artifactComparisons.json`의 유물 객체에 `site` 필드로 추가했다. 출토지를 모르는 전세품 유물(청자·백자·풍속화·앙부일구·자격루)에는 일부러 넣지 않았다. `js/artifactComparison.js`에 `siteLinksHtml()`을 추가해 결과 화면에 "🏛️ 이 유물이 발견된 유적" 박스로 노출(같은 유적 공유 시 중복 제거).
- 검증: 로컬 정적 서버 + 브라우저 콘솔에서 site 링크 텍스트·href 정확성, site 정보 없는 페어(풍속화)는 빈 문자열을 반환해 정보를 지어내지 않는지 확인. `01_validate_game_data.py` 통과, `js/artifactComparison.js`는 `node -c`로 문법 확인.
- 문서: `BACKLOG.md`에 3건 기록, `TEACHING_artifact_comparison.md`에 "2026-09-10 추가 — 유물↔유적 연결" 절 신설, 캐시버스터 `?v=20260910-eragroup1`(엔진)·`?v=20260910-sitelinks1`(엔진 재갱신).

## 2026-09-10 — 트랙 1-A(관문 설계실) `regular_neolithic` 파일럿 1편

`TASK-20260910-GATE1 | Regular MUD 관문 문법 다양화 1편(신석기) | Claude Sonnet 5 · worktree claude-gate-grammar/feat/mud-gate-grammar | 상태: DONE`

- 지시서 `docs/handoff/claude_track1a_gate_grammar_instruction.md`(Opus 5 기획) §3 1차 범위 3편 중 1편째. `data/mud/regular_neolithic.json`의 필수 단계 4개를 기존 `ordered-hotspot`/`resource-allocation`/`reflection`(모두 "원 3개 누르기" 계열)에서 `inquiry-task`로 전환했다. 배정 문법은 `sequence`(정착지→토기→의생활 순서의 앞뒤 관계 판단), 4단계(종합)는 참조 구현 `regular_goryeo_culture`와 같은 관례로 `claim-evidence`를 사용했다.
- 범위: `simulator` 블록만 수정, `choices`·`narrative`·`glossary`와 실패 분기(`1-1`/`2-1`/`3-1`)는 그대로 유지했다. `js/`·`css/`·`index.html`은 건드리지 않았다(트랙 1-A 소유권 범위).
- D-027(범주 무력화) 회귀를 직접 재현·확인: 근거 3개 중 같은 범주(`technology`) 2개만으로 `minCategories:2` 주장을 제출하면 정상적으로 거부됨을 확인했다. 범주를 `environment`(1개)·`technology`(2개)로 의도적으로 겹치게 배정해 범주 다양성 조건이 실제로 의미 있게 걸리도록 설계했다.
- 검증: 지시서 §8 전체(`node --check` 3종, `01/03/04/06/07/08/09_validate·audit_*`, `05_test_simulator_runtime.js`) 통과, `git diff --check` 클린. 격리 워크트리 로컬 정적 서버(다른 세션과 포트 충돌 확인 후 8792 포트 사용)에서 1~4단계 전부 실제 플레이: 오답 제출 시 다음 버튼 비활성 유지·정답 비노출, 정답 제출 시 탐구 패널 컨트롤 전체 비활성화 + 다음 버튼만 활성화, 최종 보상(`art_2` 암사동 빗살무늬 토기) 정상 지급, 콘솔 error/warning 0건을 모두 확인했다.
- 버그 발견(수정 안 함, 기록만): `js/mudInquiry.js`의 `evaluateSequence()`가 완료 메시지를 JSON의 `completion.successText`가 아니라 고려 문화 2단계 전용 문구로 하드코딩 반환한다. `regular_neolithic` 1~3단계 정답 제출 시 실측으로 확인했다. `js/`는 이 트랙 소유가 아니므로 `BACKLOG.md`에만 기록했다.
- 문서: `BACKLOG.md`에 "트랙 1-A(관문 설계실) `regular_neolithic` 파일럿 완료" 절 신설.

## 2026-09-10 — 트랙 1-A(관문 설계실) `regular_three_kingdoms` 파일럿 2편

`TASK-20260910-GATE2 | Regular MUD 관문 문법 다양화 2편(삼국 한강 쟁탈전) | Claude Sonnet 5 · worktree claude-gate-grammar/feat/mud-gate-grammar | 상태: DONE`

- 지시서 §3 1차 범위 3편 중 2편째. `data/mud/regular_three_kingdoms.json`의 필수 단계 4개를 `map-evidence`(1~3단계, 백제·고구려·신라 각 나라의 핵심 장소·근거·자료 범위 연결)와 `claim-evidence`(4단계, 시대별 근거 종합)로 전환. 기존 3개 hotspot이 각 단계의 `locations`와 정확히 대응해 별도 좌표 재설계 없이 그대로 재사용했다.
- 범주 설계 자체 점검 사례: 4단계 두 주장의 `minCategories`를 처음 2로 잡았다가, 갱신된 `scripts/04_validate_mud_contract.py`가 "근거 3개가 이미 서로 다른 범주라 아무 2개를 골라도 항상 통과하는 죽은 조건"이라고 자동으로 잡아냈다. 1~3단계 각 나라당 지원 근거가 정확히 1개뿐이라 같은 범주 조합 자체가 존재하지 않는 게 원인이었다 — `minCategories`를 1로 낮추고 `minEvidence`+`accepts` 멤버십으로 "서로 다른 나라 근거를 실제로 골라야 한다"는 요구를 대신 걸었다.
- D-027 회귀를 4단계에서 직접 재현: 좁은 주장("5세기 고구려→6세기 신라")을 고른 뒤 그 주장의 `accepts`에 없는 백제 근거를 함께 제출 → 정상 거부됨을 확인, 백제 근거를 빼자 정상 완료(역사가 등급)됐다.
- 검증: 지시서 §8 전체 통과(반복 탭 감사 후보 7개→6개, `regular_three_kingdoms.json:4` 제외됨). `.claude/launch.json`의 `gate-grammar` 설정(포트 8801, 이 워크트리 디렉터리 고정)으로 로컬 서버를 띄워 1~4단계 전부 실제 플레이: 오답 게이트 유지·정답 비노출, 정답 시 컨트롤 전체 비활성화+다음 버튼만 개방, 최종 보상(`art_5` 백제 칠지도, `art_6` 신라 북한산 순수비) 정상 지급, 콘솔 error/warning 0건.
- 문서: `BACKLOG.md`에 "트랙 1-A(관문 설계실) `regular_three_kingdoms` 파일럿 완료" 절 신설.

## 2026-09-11 — 트랙 1-A(관문 설계실) 정정: `regular_neolithic` 편 내부 4관문 재설계

`TASK-20260911-GATE1R | 지시서 재해석 정정에 따른 신석기 편 재설계 | Claude Sonnet 5 · worktree claude-gate-grammar/feat/mud-gate-grammar | 상태: DONE`

- 사용자가 지시서 §3 "같은 문법을 두 번 쓰지 않는다"를 "세 편 사이"가 아니라 "참조 구현(`regular_goryeo_culture`)처럼 한 편 안의 4관문이 서로 다른 사고 문법을 요구해야 한다"는 뜻으로 정정했다. 앞서 커밋한 `regular_neolithic`(1~3단계 전부 `sequence`)이 이 기준을 어겼다.
- 재배정: 1단계(정착지 선택) `sequence`→`map-evidence`, 2단계(토기 제작 순서) `sequence` 유지, 3단계(가락바퀴·뼈바늘) `sequence`→`commit-revise`, 4단계 `claim-evidence` 유지(기존 award id·category 보존). 최종 분포: `map-evidence`→`sequence`→`commit-revise`→`claim-evidence`.
- 검증: `python -c`로 4단계 interaction/task.type을 직접 출력해 4종이 서로 다름을 확인. 지시서 §8 전체 재통과. `preview_start(name: "gate-grammar")`로 포트 8801 서버를 다시 띄우고 1~4단계 전부 재플레이(신규 3단계의 초기 판단→근거 확인→최종 판단 흐름, 4단계 D-027 가드 재확인 포함), 콘솔 error/warning 0건.
- 버그 확인 범위 확장: `evaluateCommitRevise()`도 `evaluateSequence()`와 같은 방식으로 고려 문화 전용 문구를 하드코딩 반환한다. `evaluateMapEvidence`/`evaluateClaimEvidence`는 문구가 MUD-불특정 일반 문장이라 눈에 띄지 않을 뿐, 4개 평가 함수 모두 `completion.successText`를 쓰지 않는 동일 구조임을 확인해 `BACKLOG.md`에 갱신했다.
- 문서: `BACKLOG.md`에 정정 배경과 재설계 결과 절 신설.

## 2026-09-11 — 트랙 1-A(관문 설계실) `regular_three_kingdoms` 편 내부 4관문 재설계

`TASK-20260911-GATE2R | 지시서 재해석 정정에 따른 삼국 한강 쟁탈전 편 재설계 | Claude Sonnet 5 · worktree claude-gate-grammar/feat/mud-gate-grammar | 상태: DONE`

- `regular_neolithic`와 같은 기준으로 `regular_three_kingdoms`(1~3단계 전부 `map-evidence`)를 재설계했다.
- 재배정: 1단계(백제) `map-evidence`→`commit-revise`, 2단계(고구려) `map-evidence` 유지, 3단계(신라) `map-evidence`→`sequence`(영토 확보→교류→기록 3단계 카드). 4단계 `claim-evidence`는 변경 없음, 기존 award id·category(`baekje-han-river-network`/`goguryeo-pyeongyang-policy`/`silla-bukhansan-record` 등) 그대로 보존.
- 검증: `python -c`로 4단계 interaction/task.type 재확인(4종 모두 다름). 지시서 §8 전체 재통과. `preview_start(name: "gate-grammar")`로 1~4단계 전부 재플레이: 신규 1단계(commit-revise) 초기 판단→근거→최종 판단, 신규 3단계(sequence) 카드 순서·의미 질문, 4단계 D-027 가드 재확인 모두 정상 작동, 콘솔 error/warning 0건.
- 신규 3단계에서도 `evaluateSequence()`의 하드코딩 성공 메시지 버그를 재현(기존 기록과 동일 원인, `BACKLOG.md`에 중복 기록하지 않음).
- 이로써 지시서 §3 파일럿 3편 중 2편(신석기/삼국)이 "편 내부 4관문 서로 다른 문법" 기준을 충족했다. 문서: `BACKLOG.md`에 재설계 결과 절 신설.

## 2026-09-11 — 트랙 1-A(관문 설계실) 라운드 2 §1-1: `regular_modern_open` 전환 (파일럿 3편/3 완료)

`TASK-20260911-GATE3 | 4트랙 라운드 2 §1-1 강화도 조약·근대 문물 편 전환 | Claude Sonnet 5 · worktree claude-gate-grammar/feat/mud-gate-grammar | 상태: DONE`

- `docs/handoff/claude_four_track_round2_instruction.md` §1-1. `data/mud/regular_modern_open.json`의 필수 단계 4개를 `ordered-hotspot`에서 `inquiry-task`로 전환. 신석기·삼국과 달리 처음부터 편 내부 4관문 다양화(commit-revise/sequence/map-evidence/claim-evidence)로 설계해 재작업 없이 끝냈다.
- 1단계(강화도 조약) `commit-revise`, 2단계(제중원·우정총국) `sequence`(1884 우정총국→1885 제중원→이용 한계 확인의 실제 연대순), 3단계(전차·전등) `map-evidence`(D-029에 따라 `task.labels` 필수 채움), 4단계 `claim-evidence`.
- **민감 주제 균형 설계(라운드 2 §1-1 요구)**: `docs/plans/implementation_plan_sensitive_history_interactions.md`를 먼저 읽고, 4단계 `limits`에 "불평등 조약이 있었으므로 근대화는 의미 없다"(치욕사관 단정)와 "전차 하나로 주권 문제까지 다 설명된다"(근대화 미화) 양쪽을 모두 `correct:false`로 배치해 어느 한쪽 단정도 게이트를 통과하지 못하게 했다. 실플레이로 편향 응답 제출 시 실제 거부됨을 확인했다.
- 검증: 지시서 §8 전체 재통과(반복 탭 후보 6개 유지). `preview_start(name: "gate-grammar")` 포트 8801, **아이패드 가로 1180×820**에서 1~4단계 전부 재생: 3단계 커스텀 `labels` 문구 렌더링 확인, 4단계 편향 응답 거부 후 균형 응답으로 정상 완료, 콘솔 error/warning 0건.
- **지시서 오류 발견 및 기록**: 라운드 2 §1-1은 "`commit-revise`의 완료 메시지도 이제 `completion.successText`가 표시된다"고 했지만, 실측 결과 `evaluateSequence()`만 D-029로 수정됐고 `evaluateCommitRevise()`는 여전히 고려 문화 전용 문구를 하드코딩 반환한다. 1단계(강화도 조약, `commit-revise`) 정답 완료 시 "제작 과정과 보관 환경을 함께 고려한 판단입니다."가 그대로 노출됨을 재현했다. `js/`는 트랙 1-B 소유라 수정하지 않고 `BACKLOG.md`에 기록만 남겼다.
- §1-2: `docs/plans/mud_perspective_task_design.md` 설계 문서 작성(코드 없음) — `perspective` task 유형 설계안, D-029 적용(라벨·완료 메시지를 데이터로 받는 방법 포함).
- 파일럿 3편(신석기/삼국/근대 개항) 전부 완료. 지시서 §1-3에 따라 여기서 멈춘다.

## 2026-09-10~11 — 4트랙 병렬 작업 기획·통합·배포 (Opus 5 세션)

`TASK-20260911-INT4 | 4트랙 병렬 작업 기획·라운드 1·2 통합·배포 | Claude Opus 5 | 상태: DONE`

### 기획

Claude 쪽 작업을 네 창으로 나누고 파일 소유권·포트를 분리했다. 지시서는 `docs/handoff/`에 있다 — [개요](./docs/handoff/claude_four_track_overview.md), [관문 설계실](./docs/handoff/claude_track1a_gate_grammar_instruction.md), [대조실](./docs/handoff/claude_track2_artifact_site_instruction.md), [연결 공방](./docs/handoff/claude_track3_activity_loop_instruction.md), [조작대](./docs/handoff/claude_track1b_inquiry_console_instruction.md), [라운드 2](./docs/handoff/claude_four_track_round2_instruction.md).

### 통합 단계에서 잡은 결함 (각 창의 트리에서는 보이지 않던 것)

1. **고려 문화 4관문 주장–근거 정합성**(`972dcd4`): Codex가 red team으로 만든 파일럿을 운영 사이트에서 4관문 실플레이한 결과, "국제 교류"를 말하는 주장이 교류 근거 0장으로 통과했다. `minCategories: 2`가 허용 근거 5장의 category가 전부 달라 절대 실패할 수 없는 죽은 조건이었다. `DECISIONS.md` D-027, 계약 검증기에 `check_claim_categories` 추가.
2. **편 전용 문구 누출**(`47a39e6`, `e208e86`): `map-evidence`·`sequence`·`commit-revise`의 화면 문구가 파일럿 1편 기준으로 코드에 박혀 있어, 같은 문법을 재사용한 신석기·삼국·근대 편에 "국제 교류를 뒷받침하는 근거를 고르세요", "인쇄 절차와 금속활자의…"가 그대로 따라왔다. `task.labels`·`taskCompleteMessage()`로 데이터화하고 계약 검증기에 `check_task_labels` 추가. `DECISIONS.md` D-029.
   - 1차 수정에서 `evaluateCommitRevise()`를 빠뜨렸고, 관문 설계실 세션이 `regular_modern_open` 작업 중 실측으로 보고해 `e208e86`에서 전수 대조로 3곳을 마저 고쳤다.
3. **`js/miniGames.js` 캐시버스터 누락**(`9bf6112`): 51줄을 고치고 `index.html` 버전 쿼리를 올리지 않아 재방문 학생이 캐시된 옛 파일을 받을 상태였다.

### 배포

- 라운드 1: `47a39e6` — 네 트랙 병합, 검증 17종, Pages `built`.
- 라운드 2: `699cc53` — 네 트랙 병합, 검증 17종, Pages `built`. 충돌은 `BACKLOG.md`뿐(양쪽 보존).
- 운영 사이트 실측: 파일럿 4편 모두 관문별 문법이 다름, "나의 연표" 해금 5개로 동작, 유물 비교 11페어, 원인결과 4세트, 캐시버스터 4개 파일 갱신, 콘솔 error/warning 0건.

### 사고

- **검증 스크립트 통과 ≠ 의도대로 동작.** D-027·D-028·D-029 모두 같은 계열이다. 자동 검증은 "조건이 있는가"를 보지, "그 조건이 실패할 수 있는가"나 "그 문구가 이 편에 맞는가"를 보지 않는다.
- **자기 트리 검증만으로는 안 잡히는 결함이 있다.** 위 2번은 콘텐츠 창이 `js/`를 안 만지고 런타임 창이 자기 편만 테스트해서 양쪽 다 볼 수 없었다. 라운드 2 지시서 §0-1에 "공용 런타임을 고쳤으면 다른 편을 실제로 열어 확인"을 필수 보고 항목으로 넣었다.
- **잘못된 지시는 실행 세션이 정정할 수 있다.** 조작대는 "진행도 게이지 0% 고정"이 실제로는 노출되지 않음을 실측으로 밝혔고(`#widget-gauge`가 기본 `display:none`), 관문 설계실은 지시서의 틀린 서술을 잡아 보고했다. 둘 다 지시를 그대로 따르지 않은 것이 옳았다.

### 정리

4트랙 브랜치·worktree와 통합 브랜치를 삭제했다(미반영 커밋 0·미커밋 0 확인 후). Codex 소유 worktree 4개와 미반영 커밋이 남은 브랜치 3개(`codex-deep-three-kingdoms` 18, `claude-deep-prehistoric` 7, `fix/deep-three-choice-bias` 3)는 손대지 않았다.

## 2026-09-14 — Codex red team 감사 반영 (Opus 5)

`TASK-20260914-02 | Codex red team 감사 결과 검증·수정·브랜치 정리 | Claude Opus 5 | 상태: DONE`

- 감사 문서: [`claude_four_track_red_team_audit.md`](./docs/audits/claude_four_track_red_team_audit.md). 핵심 주장을 코드·데이터로 직접 재현해 확인한 뒤 수정했다. 판정표는 감사 문서 §6.
- **RT-01 나의 연표 역전**: `js/miniGames.js`의 정렬 기준을 유물 hint 차시 번호에서 `era`별 기간으로 바꾸고, 기간이 겹치지 않는 유물끼리만 순서를 판정한다(`isChronologicallyValidOrder`). 36개 유물 모두 기간표에 있고, 감사의 역전 사례 A(세종↔개항기)·B(3·1 운동↔이산가족)·C(선사 심화·백제↔고려)가 모두 오답으로 바뀌었다. 겹치는 조선 시대 유물(남한산성↔판옥선)은 양쪽 순서를 모두 인정한다.
- **RT-02·03 주장 요소 누락**: 파일럿 4편 마지막 관문 주장 8개 모두에 요소별 근거 요구를 넣었다. `requiredCategories`에 any-of 배열, `requiredEvidenceIds`를 추가하고 계약 검증기가 둘 다 검사한다. 근대 개항 편은 `requireLimit: true`로 한계 선택을 필수화했다. 통과 조합 밀도: 고려 19/40→11/40, 신석기 4/8→2/8, 삼국 5/8→2/8, 근대 5/8→4/24.
- **RT-05 근대 2관문**: 연도 없는 해석 카드 "이용 기회의 차이 확인"을 빼고 "강화도 조약 체결(1876)"을 첫 카드로 넣었다. 이용 기회는 의미 질문으로 옮겼다. 순서를 미리 알려 주던 instruction 문구도 고쳤다.
- **RT-06 유적 사실관계**: 암사동 "움집터 100여 개"를 국가유산 설명 원문("20여 기의 집터", "신석기시대 최대의 마을단위 유적")으로 정정했다. 고창의 출토품·크기-계급 서술은 연결된 출처로 확인되지 않아 "짐작·단서" 범위로 낮췄다.
- **RT-04 전수 대입**: 부분 수용. 오답 제한·감점 없는 재시도는 초등 대상 의도된 설계라 유지하고, 마지막 관문 밀도만 낮췄다.
- **브랜치 정리**: Codex 권고대로 Codex 브랜치 9개와 worktree 4개, 원격의 Claude 4트랙 브랜치 4개를 삭제했다. `codex-deep-three-kingdoms`는 main에 병합할 수 없는 낡은 base라 `archive/codex-deep-three-kingdoms` 태그(`a4d1f75`)를 남기고 지웠다. `fix/deep-three-choice-bias`는 main과 실질 차이 0을 확인했다.
- 검증: 자동 14종 통과, 감사 재현 사례 전부 차단 확인, 검증기 오류 규칙 3종 동작 확인, 브라우저 실플레이(아래 배포 항목).

## 2026-09-14 — 조작대(inquiry-console) 라운드 3: inquiry-task 조합 감사 도구 + D-030 회귀 테스트

`docs/handoff/claude_four_track_round3_instruction.md` §4 주 작업.

- **`scripts/13_audit_inquiry_combinatorics.js` 신설**: red team이 손으로 계산한 "관문별 통과 조합 밀도"(RT-04)를 반복 가능한 도구로 만들었다. `js/mudEngine.js`·`mudInquiry.js`·`mudSimulators.js`를 DOM 스텁 위에서 그대로 불러와, 각 편의 inquiry-task 관문마다 "제출 가능한 상태"를 실제 UI 활성화 조건대로 전수 열거(`commit-revise`는 initialChoice×finalChoice, `sequence`는 카드 순열×의미선택지, `map-evidence`는 장소×근거×한계, `claim-evidence`는 주장×근거 부분집합×(requireLimit일 때만) 한계)해 `MudInquiry.evaluateTask()`에 그대로 넣고 `complete` 비율을 낸다. 결과는 `docs/audits/inquiry_combinatorics_audit.md`에 표로 남긴다.
- 2026-09-14 D-030 수정 후 마지막 관문 기대 수치(고려 11/40, 신석기 2/8, 삼국 2/8, 근대 4/24 — 위 "Codex red team 감사 반영" 절의 수치와 일치)로 스크립트 자체가 자기 검증한다. 불일치 시 종료 코드 1로 실패한다.
- 경고 규칙 2종을 함께 낸다: 마지막 관문 통과 밀도 50% 초과, claim-evidence의 주장이 여러 범주 근거를 받는데 `requiredCategories`·`requiredEvidenceIds`가 둘 다 없는 경우. 현재 4편은 D-030 수정이 이미 반영돼 경고 0건이다.
- **`scripts/05_test_simulator_runtime.js`에 D-030 회귀 8건 추가**: `requiredCategories`(문자열/배열 any-of), `requiredEvidenceIds`, `requireLimit`을 막아야 하는 조합과 통과해야 하는 조합을 짝으로 넣었다 — 신석기(의생활 근거 누락), 삼국(신라 근거 누락), 근대(조약 근거 0장·한계 미선택), 고려(보관+교류만으로 D-027류 재발 여부) 4편 모두 포함.
- 검증: `node scripts/13_audit_inquiry_combinatorics.js`·`node scripts/05_test_simulator_runtime.js` 모두 통과, 기존 §8 자동 검증(01·03·04·06·09) 재실행 이상 없음. 화면을 건드리지 않아 브라우저 확인은 생략했다.
- P2 두 건(단계 진행형 전환, 캔버스 옵션 B)은 지시대로 구현하지 않았다 — §0-5에서 사용자 판단 대기 항목으로 명시됐다.

## 2026-09-14 — 「관문 설계실」(gate-grammar) Round 3 §1-1·§1-2: 16관문 의미 검토 + 수업 관찰 준비

`TASK-20260914-GATE-R3 | 파일럿 4편 16관문 의미 검토표 + 수업 관찰 체크리스트 | Claude Sonnet 5 · worktree claude-gate-grammar/feat/mud-gate-grammar | 상태: DONE`

- 지시서 `docs/handoff/claude_four_track_round3_instruction.md` §1. 새 편 전환·`js/` 수정 없이 문서 검토와 1건의 데이터 수정만 진행했다.
- `docs/audits/inquiry_pilot_semantic_review.md`: 파일럿 4편(고려 문화·신석기·삼국·근대 개항) 16관문 전부를 문법별 기준(commit-revise 요소별 근거/sequence 절차·연대/map-evidence 세 정답 일치/claim-evidence 요구-문장 대응)으로 사람이 직접 읽고 표로 남겼다.
- 지시서가 지목한 근대 개항 1관문의 공백("개항의 계기"를 받치는 자료 없음, RT-02)을 처리: 강화도 조약 제5조 원문(국사편찬위원회 우리역사넷 `kc_i400800`, "부산 이외에 두 항구를 20개월 이내에 개항하여 통상을 허가한다")을 대조해 새 근거 카드 `port-opening-clause`를 추가하고 `requiredEvidenceIds`에 포함시켰다. 조 번호 표기(조/관)는 두 번째 출처로 교차 확인하지 못해 학생 화면 문구에서 뺐다(`BACKLOG.md` §0-3 표에 보류로 기록).
- 나머지 15관문은 2026-09-14 `32f6d82`(D-030)가 이미 해소한 상태였음을 재확인만 하고 데이터는 바꾸지 않았다.
- `docs/plans/classroom_observation_inquiry_pilot.md`: Codex 첫 인계(2026-09-10) §6 P0가 요구했던 수업 관찰 체크리스트를 처음으로 작성. 공통 지표·편별 관찰 포인트·교사용 1장 기록 양식 포함. `EXPERIMENTS.md`에 `EXP-008`을 관찰-전 상태(결과·다음 결정 비움)로 추가.
- 검증: `node --check` 3종, `01/03/04/06/07/08/09_validate·audit_*`, `05_test_simulator_runtime.js` 전부 통과(source 중복 URL 오류 1건 발견·즉시 정정). `preview_start(name: "gate-grammar")`(포트 8801)에서 근대 개항 1관문 실플레이 — 치외법권·해안 측량권만 열고는 제출 버튼이 비활성 상태로 남고, 항구 개방 자료까지 연 뒤에야 제출이 활성화되며 정상 완료됨을 확인. 콘솔 error/warning 0건.
- 커밋 전 `git status --short`로 바뀐 파일이 의도한 데이터 1개 + 문서 3개뿐인지 확인했다(지난 라운드 작업 위치 사고 재발 방지, §0-4).

## 2026-09-14 — 4트랙 라운드 3 통합·배포 (Opus 5)

`TASK-20260914-03 | 4트랙 라운드 3 통합·교차 검증·배포 | Claude Opus 5 | 상태: DONE`

- 지시서: [`claude_four_track_round3_instruction.md`](./docs/handoff/claude_four_track_round3_instruction.md). 넓히지 않고 관찰 준비와 의미·사실 검증.
- 관문 설계실(`8173011`): 파일럿 4편 16관문 의미 검토표(`docs/audits/inquiry_pilot_semantic_review.md`), 수업 관찰 체크리스트(`docs/plans/classroom_observation_inquiry_pilot.md`), EXPERIMENTS 항목. 근대 1관문에 "항구 개방 조항" 근거를 추가해 "개항의 계기" 공백을 메웠다.
- 대조실(`56f8013`, main 직접): 기존 9개 페어 사실 검증, 연대 표기 정밀화, 유적 링크 `checkedAt`·`hasPhoto` 필드와 검증.
- 연결 공방(`90f460f`): 나의 연표 판정 인수 검토, 원인과 결과·기본 연표 40개 카드 검증, 고려 세트 벽란도 순서 정정. `art_12` era 라벨은 보류(BACKLOG 참조).
- 조작대(`c331259`): `scripts/13_audit_inquiry_combinatorics.js`(관문별 통과 조합 밀도 전수 열거, 기대값 자기 검증), `scripts/05`에 D-030 회귀 테스트.
- 통합: 충돌은 `walkthrough.md`·`BACKLOG.md`(양쪽 보존)뿐. 검증 15종 통과. 브라우저 교차 확인: 근대 1관문은 근거 3장을 모두 열어야 최종 판단이 열림, 고려 원인결과 새 순서 정답 판정, 대조실 연대 표기와 유적 필드 반영.

## 2026-09-14 — Codex T1 라운드 3 red team 감사 완료(보고 전용)

`TASK-20260914-T1 | 라운드 3 결과 5항목 교차 감사 | audit agent(Codex) | 상태: DONE`

- 사용자 요청: `t1 진행`. 기준 `main / 9e27a67`, 시작 시 미커밋 변경 없음.
- 결과: [round3_red_team_audit.md](./docs/audits/round3_red_team_audit.md), [재현 명령](./docs/audits/round3_red_team_reproduction.md). 16관문과 연대 정정 4페어 6곳, 미변경 페어의 범위를 분리해 기록했다.
- 13번은 실제 판정 호출+고정 기대값 비교이지만 선택적 한계 차원을 누락했다. 확장한 의미 상태는 고려 22/160·신석기 4/32·삼국 4/32·근대 4/24. 근거 순서까지 904상태에서 순서에 따른 status/quality 차이 0건.
- 정상 경로를 차단하는 변이 4종이 모두 05를 통과했고, 선택적 정답 한계 차단은 13도 통과했다. 실제 UI 콜백으로 첫 판단 덮어쓰기와 한계 선택/해제를 재현했다.
- 의미 검토의 반례: 신석기 토기 제작 완성·무늬 해석, 삼국 장소 오답 설명·교류와 순수비의 강제 순서, 정답 미노출 감사 기록. 수정 없이 근거·심각도·판단 선택지를 보고했다.
- 공식 원문: 가야 금동관의 인용 대상 오인, 청자 매병의 기관 내 편년 설명 차이 확인. 개항 내용은 제4·5관 요약으로 타당; 금관 연대는 무덤 축조 시기의 간접 추정으로 구분. 앙부일구 1871년은 직접 근거가 있어 임의 정정하지 않았다.
- 검증: `node scripts/05_test_simulator_runtime.js` PASS, `python scripts/04_validate_mud_contract.py` PASS(32 MUD), 13 원본 VM 실행 PASS(보고서 쓰기만 스텁), 변이·DOM 콜백 재현 완료. 실기기 브라우저 검증은 수행하지 않았다.
- 감사·인계 문서만 변경. 코드·데이터·기존 생성 감사 문서·Claude worktree/브랜치는 변경하지 않음. 커밋·푸시 없음. T2~T4는 미착수이며 지시서 끝 완료 기록으로 인계했다.
- 최종 확인: `git diff --check` PASS(LF→CRLF 안내만 있음), 브랜치 `main` 유지, `git diff --name-only -- js data scripts .worktrees` 출력 없음. 감사·인계 문서 6개만 변경했다.
## 2026-09-15 — T2 공식 출처 대조·명백한 데이터 정정

`TASK-20260915-T2 | 벽란도 순서 및 원문 미대조 유물 페어 공식 출처 대조 | audit agent(Codex) | 상태: DONE`

- 우리역사넷 원문으로 벽란도 국제 교류가 몽골 침입(1231)보다 앞선 현행 순서의 근거를 교체했다.
- 국립중앙박물관 원문 대조로 `artifactComparisons.json`의 「저잣길」 채색(화려한 색채→담청 중심), 바퀴날도끼(신석기→청동기 및 그에 기대던 질문·해설), 고리자루 큰 칼의 출토지 단정(유사 유물 사례로 한정)을 정정했다.
- 해석·표현 문제는 보고만 했으며, 상세 근거는 [`source_crosscheck_followup_20260914.md`](./docs/audits/source_crosscheck_followup_20260914.md)에 남겼다.
- 검증: `01/03/04/05/06/13` 및 `git diff --check` 전부 통과.

## 2026-09-15 — T3 판옥선 시대 표시 조사(보고 전용)

`TASK-20260915-T3 | art_12 판옥선 시대 라벨 및 연표 영향 조사 | audit agent(Codex) | 상태: DONE`

- 2022 개정 국가 성취기준, 프로젝트가 사용하는 5-2 교사용 지도서 차시 배열, 국사편찬위원회 『신편 한국사』의 시대 구분을 대조했다. 국가 기준은 임진왜란을 전기·후기로 직접 지정하지 않고, 교사용 지도서는 임진왜란을 명시적 조선 후기 소단원 직전에, 『신편 한국사』는 조선 중기에 둔다.
- `art_12`를 `조선 전기`로 라벨만 바꾸면 명량 해전 1597년이 기존 `[1392, 1591]` 범위에서 벗어나며, 세종 유물과 범위가 겹쳐 순서 판정도 사라짐을 확인했다. 전기 범위를 1598년까지 넓혀도 같은 범위 안의 순서는 강제되지 않는다.
- 데이터와 코드는 수정하지 않았다. 현행을 임시 유지하고, 별도 계획에서 화면 표시와 연표 계산 범위를 분리하는 방안을 권고했다.
- 상세: [`source_crosscheck_followup_20260914.md`](./docs/audits/source_crosscheck_followup_20260914.md#t3-art_12-판옥선-시대-표시와-연표-영향-조사)

## 2026-09-15 — T4 `claude-deep-prehistoric` 브랜치 분석(보고 전용)

`TASK-20260915-T4 | claude-deep-prehistoric 브랜치 미반영 작업 분석 | audit agent(Codex) | 상태: DONE`

- branch의 7커밋 중 2개가 main에 patch-equivalent이고 5개가 고유함을 확인했다.
- 콘텐츠 `6fee7fc`는 직접 cherry-pick 시 `stage 4`에서 충돌했다. 현재 `hotspot-choice`와 빈 `choices`를 유지하면 7단계 14문구의 단서 인용 개선만 남으며, 분리된 임시 worktree에서 현행 전체 검사에 통과했다.
- 과거 재설계 계획의 핵심은 이미 `D-024~D-026`과 PRD에 반영됐고 역할 분담도 현행 인계 문서에 있다. 계획·세션 노트를 그대로 복원하면 승인·진행 상태가 후퇴하지만, main의 일부 문서가 이 미존재 파일을 가리키므로 링크 정리는 필요하다.
- 직접 cherry-pick 대신 문구 수동 이식, 문서 링크/아카이브 정리, 이후 archive 태그와 브랜치 정리를 순서대로 승인받는 방안을 권고했다.
- 상세: [`claude_deep_prehistoric_branch_review.md`](./docs/audits/claude_deep_prehistoric_branch_review.md)

## 2026-09-15 — Codex 라운드 3 red team(T1) 반영 1차 (Opus 5)

`TASK-20260915-01 | round3_red_team_audit.md R3-01~07·10 반영 | Claude Opus 5 | 상태: DONE(브랜치), main 병합은 Codex T2 커밋 뒤`

- 감사: [`round3_red_team_audit.md`](./docs/audits/round3_red_team_audit.md). 지적을 코드·데이터로 확인한 뒤 반영했다. 판정 정정 기록은 [`inquiry_pilot_semantic_review.md`](./docs/audits/inquiry_pilot_semantic_review.md) 끝 "2026-09-15 정정" 절.
- **R3-01**: `scripts/13`이 선택 사항 한계를 미선택 한 값으로만 세던 것을 미선택·정답·오답 모두 세도록 고침. 기대값 고려 22/160, 신석기 4/32, 삼국 4/32, 근대 4/24.
- **R3-02**: `scripts/05`에 과잉 차단 회귀 추가 — 모든 주장의 정상 조합, any-of 대체 가지(고려 인쇄+교류, 근대 조약+도시), 선택 한계의 정답(역사가 등급)·오답(revise) 경로.
- **R3-03**: 첫 판단을 바꾸면 처음 선택이 덮어써져 실제로 생각을 고친 학생이 "수정 안 함"으로 기록되던 문제. 화면 동작은 그대로 두고 `firstInitialChoice`로 처음 선택만 보존해 `revised`·`firstChoice`·등급 판정에 쓴다(`js/mudInquiry.js`, 캐시버스터 `20260915-r3fix1`). 옛 상태 호환 테스트 포함.
- **R3-04 신석기 2관문**: 과제 범위를 "모양을 만들고 무늬를 새기는 단계"로 좁히고 굽기를 카드 설명에 명시. 공식 원문으로 확인되지 않은 "빗살무늬가 굽는 동안 갈라짐을 줄인다" 서술을 삭제하고 의미 질문을 뾰족한 바닥의 쓰임으로 한정. 순서를 알려 주던 instruction 수정. 같은 과장 문구가 있던 게이지 단계 핫스팟 설명도 함께 고침.
- **R3-05 삼국 2관문**: 질문을 "427년에 수도를 옮긴 장소"로 명료화하고, 한강 유역 오답을 "결과라서 아님"이 아니라 자료-장소 불일치로 설명. 신석기 1관문의 "가장 먼저 확인", "강가를 찾은 뒤에야 농경지" 같은 입증되지 않은 시간적 필연성도 자료 범위로 낮춤.
- **R3-06 삼국 3관문**: 선후가 입증되지 않은 "당항성 교류" 카드를 날짜로 확인되는 "진흥왕의 북한산 순행(555)"으로 교체. 553 한강 하류 확보 → 555 순행 → 순수비 건립은 건립 연대 학설(561~568년 또는 그 뒤) 어느 쪽으로도 성립. 순서를 드러내던 instruction·의미 질문 수정, 출처 2건(한국민족문화대백과 순수비, 우리역사넷 평양 천도) 추가.
- **R3-07·10**: 의미 검토표의 "정답 미노출", 개항 조항 "제5관 원문" 기록을 정정. 요약 판정이 "15관문 조치 없음"에서 "5관문 의미 수정 필요"로 바뀜.
- **보류**: R3-08(가야 금동관 연대 출처 대상 오인)·R3-09(청자 매병 편년 인용)는 `data/artifactComparisons.json` 수정인데 Codex가 같은 파일로 T2를 진행 중이라 커밋 뒤 처리. R3-11(앙부일구 1871년)은 오류 아님, 기록만.
- 검증: 자동 14종 통과(05 신규 회귀 포함, 13 새 기대값 일치). 변경 관문은 실제 판정 함수로 확인 — 신석기2·삼국2·삼국3 정답 경로 complete, 삼국3 비석을 순행 앞에 두면 revise, 삭제 대상 서술 잔존 0. 브라우저 확인은 main 체크아웃 설정을 건드리지 않으려고 이번에 하지 않았고, 병합 전 통합 단계에서 한다.

## 2026-09-15 — 「관문 설계실」(gate-grammar) Round 4 §1-1: 1단원 비파일럿 9편 사실 대조

`TASK-20260915-GATE-R4 | 1단원 비파일럿 9편 수치·연도·인물·인과 대조 | Claude Sonnet 5 · worktree claude-gate-grammar/feat/mud-gate-grammar | 상태: DONE`

- 지시서 `docs/handoff/claude_four_track_round4_instruction.md` §1. 기준 브랜치 `fix/round3-red-team`(`b4dca60`) 위에서 작업, main 체크아웃은 건드리지 않았고 이 브랜치에만 커밋했다(원격 push 없음, §0-4).
- 대상 9편(`regular_paleolithic`/`regular_bronze_age`/`regular_gojoseon`/`regular_three_kingdoms_life`/`regular_silla`/`regular_balhae`/`regular_goryeo_founding`/`regular_goryeo_society`/`regular_goryeo_war`)의 `narrative`·`choices`·`glossary`·`location`·`badge`·시뮬레이터 텍스트에서 수치·연도·인물·인과 단정을 뽑아 `WebFetch`로 국사편찬위원회 우리역사넷 원문과 직접 대조했다. 결과 표: `docs/audits/unit1_regular_fact_check.md`.
- **정정 2건**(원문 직접 열람으로 확정): (1) `regular_goryeo_war` 3단계의 "귀주대첩 이후 200년간 평화·다시는 넘보지 못했고" — 원문은 그런 구체적 기간·절대적 서술을 담고 있지 않아 원문 문구("큰 갈등이나 물리적 충돌 없이 오랜 시간 안정적인 평화")에 맞춤. (2) `regular_gojoseon` 1단계의 "《삼국유사》에는... 기원전 2333년" — 원문이 "삼국유사와 동국통감의 기록에 따르면"이라 두 문헌을 함께 인용하므로 병기로 정정.
- **완화 1건**(원문 미확인): `regular_bronze_age`의 "전 세계 고인돌의 절반 이상" — 국가유산포털 등 여러 페이지를 열었으나 비율을 명시한 원문을 찾지 못해 구체적 수치 단정을 뺐다.
- **구조 오류 1건**(사실 오류 아님, 문자열만 정정): `regular_three_kingdoms_life`의 `roadmap` 2관문 라벨 "삼국의 불교 수용"이 실제 stage 내용(고분 벽화 인물 크기로 신분 표현)과 처음부터 달랐다 — "2. 벽화 속 신분 표현"으로 정정. 게임 구조는 그대로 유지.
- 나머지 5편은 검토 후에도 반증을 찾지 못해 데이터를 바꾸지 않았고, 어디까지 이번 세션에서 실제로 재열람했는지와 어디는 기존 2026-08-26~27 검증에 의존했는지를 감사 문서에 정직하게 구분해 표시했다.
- 검증: `01/03/04/06/08/09_validate_*`, `05_test_simulator_runtime.js`, `11_audit_artifacts.py`, `13_audit_inquiry_combinatorics.js` 전부 통과, `git diff --check` 클린. 모든 변경이 텍스트 필드뿐이라 게임 구조에 영향이 없음을 `git diff`로 직접 확인했고, 별도 브라우저 플레이는 생략했다. `13_audit_inquiry_combinatorics.js` 실행이 부수적으로 갱신한 `docs/audits/inquiry_combinatorics_audit.md`(날짜만 변경, 내 소유 아님)는 커밋 전 `git checkout --`으로 되돌렸다.
- 커밋 직전 `git status --short`로 바뀐 파일이 의도한 데이터 4개 + 문서 1개뿐인지 확인했다.

## 2026-09-15 — 조작대(inquiry-console) 라운드 4: inquiry-task 의미 lint 스크립트 + 첫 판단 보존 브라우저 확인

`docs/handoff/claude_four_track_round4_instruction.md` §4 주 작업.

- **`scripts/14_lint_inquiry_semantics.js` 신설**: 라운드 3 red team이 사람이 읽어서 찾은 결함(R3-04, R3-07, D-030 3) 중 기계가 신호로 잡을 수 있는 5종을 검토 목록(실패 아님)으로 낸다 — 순서 노출(구조적 화살표-사슬 신호 + 카드 라벨 어휘 신호), 날짜 없는 순서 카드, map-evidence 오답 피드백의 정답 장소 라벨 노출, 완료·단정어(`완성`·`가장 먼저`·`반드시`·`뒤에야`·`마지막 단계`), inquiry-task 편의 개별 출처 공백. 결과는 `docs/audits/inquiry_semantics_lint.md`.
- **자기 검증 결과, 지시서와 다른 사실을 발견**: 지시서는 "기획 세션이 방금 고친 삼국 3관문은 순서 노출 경고가 사라져야 한다"고 했으나, 실제로는 남아 있다. `b4dca60`는 `simulator.instruction`의 "영토 확보→교류→기록 순서로"만 지웠고, `meaningQuestion.options[order-random].feedback`의 같은 패턴("영토 확보→순행→기록 사이에는 앞뒤 관계가 있습니다")은 손대지 않았다. `data/mud/regular_three_kingdoms.json`은 이번 라운드 조작대 소유가 아니라(관문 설계실) 고치지 않고 보고서 상단에 눈에 띄게 남겼다.
- 검증 명령 목록에 `node scripts/14_lint_inquiry_semantics.js` 추가.
- **§4-2 첫 판단 보존 브라우저 확인**: 아래 항목에 별도 기록.

## 2026-09-15 — 기획 세션: 라운드 4 통합·배포 (`5e59d03`)

- 병합 순서:
  1. `origin/main`(Codex T2~T4 `e4229c9`)
  2. `feat/mud-gate-grammar`
  3. `feat/artifact-site-comparison`
  4. `feat/extended-activity-loop`
  5. `feat/inquiry-ui-console`
- 충돌은 walkthrough/BACKLOG의 덧붙이기 충돌뿐이었고, 양쪽 내용을 모두 남겼다.
- lint(`scripts/14`)가 R3 수정의 누락을 찾아 정정했다.
  - 삼국 3관문 `order-random` 피드백에 "확보→순행→기록"으로 순서가 노출돼 있었다. 연도 비교를 안내하는 문장으로 바꿨다.
  - 근대 3관문 `electric-light` 피드백에 정답 장소 이름 "전차 노선"이 들어 있었다. 기록의 조건을 안내하는 문장으로 바꿨다.
  - 재실행 결과 순서 노출·정답 장소 노출 경고는 0건이다.
- 검증: 04 PASS, 05 PASS(D-030·R3-02·R3-03 회귀 포함), 11 PASS(36종 0건), 13·14 보고서를 재생성했다.
- 브라우저 교차 확인은 하지 않았다. 라운드 5 §4로 넘겼다.
- `origin/main`에 fast-forward push했다. main 체크아웃에는 Codex의 커밋 전 작업이 있어 건드리지 않았다. Codex는 커밋 전에 `git pull --rebase`가 필요하다.
- 사용자 판단 대기에 1건 추가: 첨성대 해설 문구. 연결 공방이 우리역사넷 표현("온전히 남은 것 중 세계에서 가장 오래된")으로 바꿨는데, 백과사전의 흔한 표현은 "동양 최고"다.

## 2026-09-15 — T2~T4 통합 및 T5-1 구현 계획 작성

- T2 공식 출처 정정, T3 판옥선 시대 표시 조사, T4 `claude-deep-prehistoric` 브랜치 감사를 전체 검증 후 `e4229c9`로 커밋하고 `origin/main`에 push했다.
- `TASK-20260915-T5-PLAN | 고조선 정적 협동 MUD 최초 판단 순서 교정 계획 | planning agent(Codex) | 상태: DONE`
- `EXP-006`, 고조선 전용 `app.js`, 공용 `episode.js`, 7·8차시 화면 흐름을 대조해 [`implementation_plan_gojoseon_initial_judgment_order.md`](./docs/plans/implementation_plan_gojoseon_initial_judgment_order.md)를 작성했다.
- 계획은 고조선 편을 공용 엔진으로 전면 이관하지 않고 `역할 → 최초 판단 → 공유 → 추가 증거` 순서만 최소 이식한다. 기존 총 시간 예산, 법 만들기, 3·4·5인/5·10분형 분기를 유지하고 뒤로가기 재진입 상태를 명시적으로 검증한다.
- 상태: 계획 작성 완료, 사용자 구현 승인 대기. 협동 MUD 코드와 실시간 앱은 수정하지 않았다.

## 2026-09-15 — T5-1 고조선 협동 MUD 최초 판단 순서 교정

`TASK-20260915-T5-1 | 고조선 정적 협동 MUD 최초 판단 순서 교정 | implementation agent(Codex) | 상태: DONE`

- 고조선 8조법의 화면 흐름을 `역할 정보 → 최초 판단 → 정보 공유 → 추가 증거 → 재판단`으로 바꿨다. 역할 화면에서는 `shareText`를 보이지 않게 했고, 최초 판단 뒤에만 역할별 공유 문장과 공유 확인 버튼을 보여 준다.
- 최초 판단을 다시 고르면 `shared=false`, 재판단과 판단 변화 기록을 초기화한다. 추가 증거에서 공유 화면으로 돌아간 뒤에는 `다시 추가 증거 보기`로 재진입할 수 있어 뒤로가기에 막히지 않는다.
- 5분형(310초)·10분형(600초)의 총 시간 예산, 3·4·5인 역할 분기, 법 만들기·역사 자료 비교·로컬 저장 구조는 유지했다. 진행 표시는 8단계로 맞췄다.
- 교사용 허브·안내·프로젝트 문서를 세 활동 공통의 최초 판단 순서로 정리했다. 판단 변화의 교육 효과는 실제 수업에서 별도로 재관찰해야 한다.
- 검증: `node --check cooperative-mud/gojoseon-law/app.js`, `scenario.js`, `cooperative-mud/pacing.js`; `python scripts/06_validate_static_assets.py`; `git diff --check` 통과. 로컬 브라우저에서 3인·5분형의 역할→공유 재진입, 5인·10분형의 완료 요약까지를 확인했다.

## 2026-09-15 — 「관문 설계실」(gate-grammar) Round 5 §1: 2단원 비파일럿 8편 사실 대조

`TASK-20260915-GATE-R5 | 2단원 비파일럿 8편 수치·연도·인물·인과 대조 | Claude Sonnet 5 · worktree claude-gate-grammar/feat/mud-gate-grammar | 상태: DONE`

- 지시서 `docs/handoff/claude_four_track_round5_instruction.md` §1. `origin/main` 기준, main 체크아웃 미접촉, 이 브랜치에만 커밋(push 없음).
- 대상 8편(`regular_joseon_founding`/`regular_sejong`/`regular_joseon_status`/`regular_joseon_silhak`/`regular_joseon_economy`/`regular_joseon_folk`/`regular_joseon_diplomacy`/`regular_myeongnyang`) 전부를 읽고 수치·연도·인물·인과 단정과 로드맵 라벨-콘텐츠 정합성을 확인했다. 결과: `docs/audits/unit2_regular_fact_check.md`.
- **확인된 사실 오류 없음 — 데이터를 바꾸지 않았다.** 지시서가 예시로 든 "측우기를 장영실이 만들었다" 서술을 `regular_sejong` 3관문에서 직접 확인했지만, 실제로는 측우기 발명자를 특정하지 않고 앙부일구와 명확히 구분해 서술하고 있었다. 8편 모두 로드맵 라벨도 실제 관문 내용과 일치했다(라운드 4에서 발견된 것과 같은 불일치 없음).
- `regular_joseon_silhak`의 거중기·수원 화성 "10년 예정→2년 9개월" 수치는 여러 URL 시도에도 원문을 확보하지 못해 확인 불가로 남기고 유지했다(반증도 없음).
- `data/artifacts.json`의 `art_10`(홍패) 관련해 대조실이 남긴 참고 사항을 `regular_goryeo_society.json`에서 직접 확인 — 이미 "고려·조선 시대" 병기, 제도의 고려 기원만 말하고 실물 연대는 단정하지 않아 조치 불필요로 결론.
- 검증: `04_validate_mud_contract.py`, `05_test_simulator_runtime.js`, `11_audit_artifacts.py`, `13_audit_inquiry_combinatorics.js`, `14_lint_inquiry_semantics.js` 전부 통과(데이터 불변이라 회귀 없음). `13`·`14`가 부수적으로 갱신한 보고서 2개(내 소유 아님)는 `git checkout --`으로 되돌렸다.
- 커밋 직전 `git status --short`로 새 문서 1개만 추가됐는지 확인했다.

## 2026-09-15 — 조작대(inquiry-console) 라운드 5: lint 오탐 줄이기 + 라운드 4 통합본 브라우저 교차 확인

`docs/handoff/claude_four_track_round5_instruction.md` §4.

### 4-1 lint 오탐 줄이기

- **"날짜 없는 카드" 분류**: 카드 전부가 날짜 없이 구성되면 "공정 순서"(직지 인쇄 절차, 빗살무늬 토기 제작)로 보고 개별 카드 경고 대신 편당 요약 1줄로 묶었다. 일부만 날짜가 있으면 "사건 순서"로 보고 날짜 없는 카드만 계속 개별 경고한다(삼국 3관문 북한산 순수비 카드처럼 진짜 검토할 공백을 놓치지 않기 위함). 경고 8건 → 3건(공정 순서 요약 2건 + 사건 순서 진짜 공백 1건).
- **데이터에 이 구분을 나타내는 필드가 없어(§4-1), `task.sequenceKind: "event"|"process"` 필드를 스키마 제안으로만 보고서에 남겼다. 구현하지 않았다** — 데이터 스키마 변경은 관문 설계실·기획 세션 승인이 먼저 필요하다.
- **"완료·단정어" 사람이 읽고 분류**: 8건(+라운드 4 이후 재검사분 포함) 전부 실제 역사 단정이 아니었다 — "판단 확인" 버튼을 누르라는 화면 지시문 6건, "완성된 N"(이미 만들어진 사물) 서술어 2건, "옷/도구가 완성된다"는 제작 조건 설명 2건. 전부 `ABSOLUTE_ALLOWLIST`로 코드에 옮겨 다른 편에 같은 문구가 나와도 재검토 시간을 쓰지 않게 했다. 총 경고 20건 → 3건(허용됨 12건은 보고서에 접이식으로 남김).

### 4-2 라운드 4 통합본 브라우저 교차 확인 (`inquiry-console`, 8804)

7개 항목 모두 실제로 화면에 열어 콘솔 오류 0건으로 확인했다.

| 항목 | 확인 방법 | 결과 |
|---|---|---|
| 고려 전쟁 3관문 infoText | `renderStage('3')` 후 `widget-info` 내용 확인 | "귀주대첩 이후 고려와 거란은 국교를 회복해..." 정상 표시 |
| 고조선 1관문 | `openMUD` 후 서사 텍스트 확인 | "《삼국유사》와 《동국통감》의 기록에 따르면..." 정상 표시 |
| 삼국 3관문 오답 피드백 | 정답 순서 입력 + 오답 의미선택지(order-random) 클릭 후 제출 | "세 카드에는 연도가 있습니다..."로 정상 표시. 라운드 4에서 보고한 "영토 확보→순행→기록" 노출은 사라짐(lint 자기 검증도 PASS) |
| 근대 3관문 오답 피드백 | 오답 장소(전등) 선택 후 제출 | "전등도 중요한 변화이지만..."으로 정상 표시, 정답 라벨("전차 노선") 노출 없음 |
| 도감 art_10·art_14 | `encyclopedia.unlockArtifact()`로 해금 후 렌더 | "고려 과거 합격증 '홍패'", "홍대용의 둥근 지구의" 카드 정상 표시 |
| 퀴즈 quiz_7 | `quizGame.currentList=[quiz_7]; showQuestion()` | "세종실록에 따르면 왕세자(훗날의 문종)가 처음 발명을 주도했고..."로 정정된 해설 정상 표시 |
| 금동대향로 스토리(story_samguk) | `storyEngine.startStory()` 후 장면 1→5 순차 이동 | 6개 장면(분기 포함) 모두 정상 렌더, 콘솔 오류 없음 |

콘솔 error/warning 0건(전 구간). 1180×820은 화면 레이아웃을 건드리지 않아 별도 확인하지 않았다.

## 2026-09-15 — 기획 세션: 라운드 5 통합·배포(`ae5bfbc`)와 사용자 결정

- 병합한 네 브랜치와 결과:
  - `feat/mud-gate-grammar`: 2단원 8편, 새 오류 없음
  - `feat/artifact-site-comparison`: R3-08·09 정정, 심화 보상 desc 2건
  - `feat/extended-activity-loop`: ce15 인과 표현 완화
  - `feat/inquiry-ui-console`: lint 오탐 축소, 라운드 4 통합본 브라우저 교차 확인 7곳, 콘솔 오류 0
- 충돌은 BACKLOG/walkthrough의 덧붙이기 충돌뿐이었다.
- 검증: 01·03·04·05·06·08·09·11·13·14 모두 PASS. 기존 경고 1건(원본 데이터상 정답이 첫 보기 → 화면 섞기 유지 필요)은 그대로다.
- Codex가 main을 `ae5bfbc`까지 fast-forward한 것을 확인했다.
- 사용자 결정:
  - 첨성대 문구 유지(D-033)
  - `sequenceKind` 채택(D-031)
  - 3단원 수정 금지 해제(D-032)
  - 수업 관찰 먼저(D-033)
- 관찰 기록지를 새로 만들었다: `docs/plans/classroom_observation_session_sheet.md`. 차시 전체 기록 A, 추적 학생 B(기존 파일럿 기록지 확장), 라운드 3~5에서 고친 곳을 교실에서 확인하는 표 C를 담았다.

## 2026-09-16 — 「관문 설계실」(gate-grammar) Round 6 §1: 조선 후기 역할별 자료 카드 (10월 28일 공개수업 C1)

`TASK-20260916-GATE-R6 | 조선 후기 협동 활동 4/3/5인 역할 카드 초안 | Claude Sonnet 5 · worktree claude-gate-grammar/feat/mud-gate-grammar | 상태: DONE`

- 지시서 `docs/handoff/claude_four_track_round6_instruction.md` §0·§1. `data/mud/`·`apps/**`·`js/*.js` 미접촉, main 체크아웃 미접촉, 이 브랜치에만 커밋(push 없음).
- 산출물: `docs/handoff/claude_joseon_late_role_cards.md`. 4인 역할(농민·보부상·장시 주민·수공업자·기록관)의 `privateInfo`(역할 간 내용 비중복)·`sharePrompt`·`interest`, 3인 변형(겸임 없음 + 공통 자료 보충), 5인 변형(다섯째 역할 `공인` — 고유 근거 보유, 서기 아님), `commonPrompt`, 정답 없는 `firstJudgment`, `synthesis` 옵션(근거 8개, `minEvidence:2`·`minDistinctRoleSources:2`), 질문 틀 3종, 교사 개입 문구, sticky-wall 형식을 계획서 §4 `CooperativeScenario` 타입에 맞춰 초안으로 작성했다.
- 지시서의 핵심 제약("자료 한 장만 봐도 답이 나오면 안 됨", "역할끼리 이해관계가 부딪혀야 함")을 직접 점검해 문서 §2에 4가지 갈등축(농민↔보부상 가격, 보부상↔수공업자 경쟁, 농민 본인↔이웃 광작 격차, 공인↔소상인 물량 선점)을 명시했다.
- 사실 문장마다 `[출처 후보]` 표기로 이미 검증된 것과 대조실이 확인할 것을 구분했다 — 원문 대조는 직접 하지 않고 대조실에 넘겼다.
- 검증: `04/05/11/13/14` 전부 통과(데이터 불변, 회귀 없음). `scripts/15`는 아직 없어(조작대 신설 예정) 건너뜀. 부수 갱신된 보고서 2개(내 소유 아님)는 되돌렸다.
- 커밋 직전 `git status --short`로 새 문서 1개만 추가됐는지 확인했다.
- 대조실·연결 공방·조작대에 넘길 요청을 문서 §9에 남겼다(남의 파일은 고치지 않음).

## 2026-09-16 — 조작대(inquiry-console) 라운드 6: 협동 패킷 검증 스크립트 + D-031 sequenceKind 구현

`docs/handoff/claude_four_track_round6_instruction.md` §4.

### 4-1 `scripts/15_validate_cooperative_packet.js` 신설

- 계획서(`implementation_plan_joseon_late_cooperative_live_vertical_slice.md` §4)의 `CooperativeScenario` 계약 관문을 검사: `roleIds`·`evidenceIds` 참조 무결성, 3·4·5인 변형 모두 참여자마다 고유 자료 보유, 3인 변형의 역할 겸임 금지, `minEvidence`/`minDistinctRoleSources` ≥ 2, `limitOptions`에 "알 수 없는 것" 항목 존재, 과잉 단정어("모든 사람"·"전국"·"즉시"·"누구나"·"완전히") 검사(경고).
- 대상 파일(`data/cooperative/*.json`)이 아직 없어 **건너뛰고 통과**(연결 공방과 병렬 작업). 임시 fixture로 6개 위반 유형이 실제로 걸리는지 직접 확인한 뒤 삭제.

### 4-2 D-031 `sequenceKind` 구현

- `data/mud/regular_goryeo_culture.json` 2관문, `regular_neolithic.json` 2관문 → `sequenceKind: "process"`. `regular_three_kingdoms.json` 3관문, `regular_modern_open.json` 2관문 → `sequenceKind: "event"`.
- **부수 발견·수정**: `event`로 표시한 삼국 3관문의 `erect-bukhansan-monument` 카드에 연도 표지가 없어 D-031 규칙 2("event는 모든 카드에 연도 필요")를 어겼다. 기존 데이터에 이미 "건립 연대는 학자마다 다르다"는 주석이 있어 임의로 연도를 지어내지 않고, 우리역사넷(국사편찬위원회) 원문을 직접 열어 확인 — "비문의 지명 '남천주'를 근거로 568년 10월 이후로 추정"한다고 명시함을 확인하고, 라벨에 "(568년경)"을 추가·`detail`에 555~568년 학설 대립을 함께 적었다. `sources`에 확인 날짜(2026-09-16)와 근거를 새 항목으로 추가(기존 항목은 수정하지 않음).
- `scripts/04_validate_mud_contract.py`: `sequenceKind`가 `event`/`process` 중 하나인지, `event`면 카드마다 연도 표지(YEAR_PATTERN, `scripts/14`와 동일 정규식)가 있는지 계약으로 강제.
- `scripts/14_lint_inquiry_semantics.js`: 추정 대신 `task.sequenceKind`를 그대로 읽어 분류. 필드가 없는 미래 편을 위해 라운드 5의 추정 규칙을 fallback으로 유지. 파일럿 4편 모두 필드가 채워져 경고 0건(허용됨 12건은 그대로 보존).
- 검증: `04`·`05`·`11`·`13`·`14`·`15` 전부 통과. `inquiry-console`(8804)에서 삼국 3관문 카드 라벨·설명이 화면에 정상 표시되는지, 정답 순서 제출이 여전히 통과하는지 확인. 콘솔 오류 0건.
- 확인 불가 항목 없음(우리역사넷 원문을 직접 열어 확인함).
## 2026-09-16 — 고려 초기 실시간 협동 활동 T0~T5 구현

`TASK-20260916-01 | apps/cooperative-live 및 고려 초기 활동 | implementation/content agent | 상태: DONE`

- 별도 worktree와 `feat/cooperative-live-early-goryeo` 브랜치에서 기존 고조선 기술 회귀를 유지한 채 1단원 13차시 “새 고려의 첫 회의”를 두 번째 서버 검증형 시나리오로 추가했다.
- 공개 시나리오 메타데이터와 서버 전용 역할 본문·근거·출처를 분리했다. 학생 조회는 자기 역할의 비공개 자료만 반환하고, 다른 역할은 전원 말하기 완료 뒤 공동 초안에 필요한 근거 요약만 반환한다.
- 개인 최초 판단의 선택·까닭은 현재 탭 `sessionStorage`에만 저장한다. 서버에는 완료 시각만 기록한다.
- 전원 공유 관문, 정책 2개·서로 다른 역할 근거 2개·연결 설명·한계의 서버 검증, 단조 증가 revision, 수정 시 확인 무효화, 같은 revision 전원 확인을 구현했다.
- 교사 pause/resume, 단계 이동, 도움 요청·해제, 모둠별 완료·확인 n/N, 새 테이블을 포함한 종료·TTL 삭제를 구현했다.
- 3·4·5인 역할 배치, 고려 4명 전체 흐름, 기존 고조선 21명 병렬 흐름을 자동 검증했다. production client bundle에서 고려 비공개 역할 본문과 출처 URL이 포함되지 않은 것도 확인했다.
- 교사용 1장 체크리스트와 장애 시 교사 읽기·화이트보드 대체 절차를 `apps/cooperative-live/TEACHING_EARLY_GORYEO.md`에 기록했다.
- 검증: `npm run check` 전체 통과 — lint, TypeScript, 단위 테스트 17건, Convex 통합 테스트 2건, production build. `git diff --check` 통과, production client bundle의 고려 역할 비공개 문장과 우리역사넷 URL 미검출.
- 미실행: 4~8대 성인·교사 기기 리허설, 학교 Wi-Fi·iPad QR·실제 WebSocket 재접속, 실제 학생 시험. 이 항목들은 완료로 간주하지 않고 `EXP-009`에 결과를 기록한다.
- 기준 커밋 `39df640` 읽기 전용 감사에서 확인 화면의 초안 내용 누락과 동시 편집 revision 경쟁 결함을 발견했다. 자기 역할 자료의 판단·말하기 단계 유지, 공유 n/N, 서버의 정책·역할 근거 정확히 2개 검증까지 함께 보정하고 revision 1 확인→revision 2 수정→0/N 초기화→구 revision 거부 회귀를 추가했다. 상세는 `docs/audits/early_goryeo_cooperative_live_review.md`.
- 수정 커밋 `c21e74b` 최종 재감사는 병합 차단 결함 없음으로 판정했다. 비차단 지적에 따라 21명 통합 테스트가 고조선을 명시하도록 고치고, 여러 학생이 대기실에서 함께 쓰는 QR을 “일회 입장키”라고 부르던 README 표현을 정정해 `bd67371`에 기록했다. 기술 Preview·성인 리허설은 Go, 실제 학생 Production은 No-go다.

## 2026-09-16 — 고려 초기 실시간 활동 main 통합·push 전 검증

- 기능 브랜치를 최신 `origin/main` 위로 재배치하고 `main`에 fast-forward 통합했다. 작업 중 원격에 추가된 `16d28bf`도 merge commit `9a01416`으로 합쳐 조선 후기 라운드 7 문서를 보존했다.
- main에 남아 있던 중단된 대안 `apps/cooperative-live/shared/scenario.ts`는 삭제하지 않고 `preserve interrupted early-goryeo alternative before integration 2026-09-16` 이름의 stash로 보존했다. 최종 구현은 비공개 역할 본문을 서버 전용 파일로 분리하므로 이 대안은 적용하지 않는다.
- main의 의존성을 `npm ci`로 lockfile과 맞춘 뒤 `npm run check`를 재실행했다. lint, TypeScript, 단위 테스트 17건, Convex 통합 테스트 2건, production build가 모두 통과했다.
- 최신 main의 `python scripts/04_validate_mud_contract.py`와 `node scripts/15_validate_cooperative_packet.js`도 통과했다. 조선 후기 패킷의 “전국” 표현 경고 3건은 기존 라운드 7 검토 항목이며 이번 고려 구현의 실패가 아니다.
- 판정은 기술 Preview·성인 4~8대 리허설 Go, 실제 학생 Production No-go를 유지한다.


## 2026-09-16 — 「관문 설계실」(gate-grammar) Round 7 §1: 조선 후기 역할 카드 — 공인→품팔이 교체 (1단계)

`TASK-20260916-GATE-R7 | 조선 후기 5인 변형 공인 카드를 품팔이 카드로 교체 | Claude Sonnet 5 · worktree claude-gate-grammar/feat/mud-gate-grammar | 상태: DONE`

- 지시서 `docs/handoff/claude_four_track_round7_instruction.md` §0·§1(D-034). 라운드 6에서 네 창이 동시 작업해 역할 카드 문서와 패킷 초안의 역할 id·5인 역할이 서로 어긋난 것을 맞추는 1단계 작업이다. `docs/handoff/claude_joseon_late_role_cards.md` 한 파일만 고쳤다.
- D-034 확정값에 따라 5인 변형의 `gongin`(공인, 보부상과 관점이 겹침)을 `laborer`(품팔이)로 교체했다. 광작 확대로 땅을 빌리지 못하게 된 "일부" 농민이 품삯으로 생활한다는 내용으로, "모든 농민이 땅을 잃었다"는 과잉 단정을 피하려고 privateInfo에 범위를 두 번 명시했다(지시서가 특히 주의를 준 부분).
- 농민↔품팔이 이해관계 충돌(같은 광작이 두 처지에 다르게 나타남)을 §2·§3에 추가했다. `evidenceOptions`의 `ev-gongin`→`ev-laborer`, §9의 5인 구성·§10 확인 불가 목록도 확정값에 맞춰 정리했다.
- 문서 첫머리에 "§4~§8은 연결 공방 참고 자료, 확정본은 패킷 JSON"이라는 소유 경계 안내를 추가했다(D-034가 판단·해석 옵션은 연결 공방 소유로 정리했기 때문).
- 검증: `04`·`05`·`13`·`14`·`15`(조작대가 이번 라운드에 신설한 `scripts/15_validate_cooperative_packet.js`) 전부 통과. `15`가 연결 공방 소유 `joseon_late_packet.draft.json`에 낸 "전국" 과잉 단정 경고 3건은 남의 파일이라 손대지 않았다. 부수 갱신된 조작대 소유 보고서 2개는 `git checkout --`으로 되돌렸다.
- 커밋 직전 `git status --short`로 대상 문서 1개만 바뀌었는지 확인했다.
- 2단계(대조실 수정 요청 반영)는 기획 세션이 따로 지시할 때까지 착수하지 않는다.


## 2026-09-16 — 조작대(inquiry-console) 라운드 7: scripts/15 네 가지 보완

`docs/handoff/claude_four_track_round7_instruction.md` §4.

- **부정문 오탐 제거**: 문장 단위로 나눠 "수(는) 없", "것은 아니", "않았/아니었" 같은 부정·한계 패턴이 같은 문장에 있으면 과잉 단정어 경고를 내지 않는다. `limitOptions.limit_scope`("...전국 모든 지역에서 똑같이 일어났는지는 이 자료만으로 알 수 없다")에서 재현 확인 — "전국"이 있어도 더 이상 경고하지 않음. 처음 정규식(`할\s*수\s*없`)이 "알 수 없다"류를 놓쳐 직접 재현 테스트로 잡아 고침.
- **D-034 확정값·역할 카드 문서 교차 검사 추가**: `CONFIRMED_ROLES` 상수(역할 id·evidenceId·참여 변형)를 두고 패킷의 `roles[].id`/`evidenceId`, `groupVariants.3`(craftsman·laborer 금지 + 공통 자료 `jangsi-craftsman-summary` 필수), `groupVariants.5`(laborer 필수)를 대조한다. 역할 카드 문서(`claude_joseon_late_role_cards.md`)의 헤딩 백틱 id·`evidenceId` 줄을 정규식으로 뽑아 패킷과도 대조한다. 다르면 실패로 낸다.
- **타입에 없는 최상위 필드 경고**: `CooperativeScenario`(계획서 §4) 필드 목록에 없는 최상위 키가 있으면 경고(`_`로 시작하는 키는 초안 메모로 보고 넘어감). 현재 `questionFrames`가 걸림 — 지시서 §3이 연결 공방에게 이미 제거를 요청한 필드와 일치.
- **자리표시자 개수 보고 + `--final` 옵션**: `[PLACEHOLDER`를 문자열 검색으로 세어 보고. 기본은 경고, `--final` 플래그를 주면 실패로 전환.
- **검증**: 빈 규칙 확인용 fixture(완전 위반본 6종, D-034 완전 합치본)를 임시로 만들어 각 검사가 정확히 걸리고/통과하는지 직접 확인한 뒤 삭제, 실제 파일은 복원(`git status --short`로 조작대 소유 파일만 바뀐 것 확인).
- **현재 상태(예상된 실패)**: 내 워크트리의 `data/cooperative/joseon_late_packet.draft.json`·`claude_joseon_late_role_cards.md`는 아직 라운드 6 산출물(`nongmin`/`artisan`/`rokgwan`/`pumpali`/`gongin`)이라 `scripts/15`가 실패한다 — 지시서 §4 1단계 주의대로 **예상된 것**이며, 통합본(연결 공방·관문 설계실의 라운드 7 §1·§3 반영 후)에서 다시 확인해야 한다.
- `04`·`05`·`13`·`14`는 모두 통과.


## 2026-09-16 — 「관문 설계실」(gate-grammar) Round 7 §1: 조선 후기 역할 카드 — 대조실 판정 반영 확정 (2단계)

`TASK-20260916-GATE-R7-2 | 대조실 문장별 판정을 역할 카드에 반영해 확정 | Claude Sonnet 5 · worktree claude-gate-grammar/feat/mud-gate-grammar | 상태: DONE`

- 지시서 `docs/handoff/claude_four_track_round7_stage2_instruction.md` §1. `docs/handoff/claude_joseon_late_role_cards.md` 한 파일만 고쳤다.
- 대조실이 `docs/audits/joseon_late_source_review.md`의 `ROLE_CARD_REVIEW`에 남긴 문장별 판정을 전부 반영했다.
  - **범위 초과 정정**: 농민 #2 "혼자 힘으로" → "예전보다 적은 일손으로"(우리역사넷 「광작과 광작농」 — 광작농도 농번기엔 고용 노동을 함께 썼다는 서술).
  - **확인 불가 → 카드에서 빼고 새 §11 "보류 문장" 표에 원문·판정·되살릴 조건을 보존**: 농민 이모작 문장, 보부상 가격 변동 문장, 수공업자 "보부상이 다른 지역에서 가져온 물건과 경쟁" 원문. 개념 자체(가격 변동, 지역 간 경쟁)는 삭제하지 않고 `interest` 필드나 일반화된 문장으로 남겨 각 카드의 이해관계 충돌 설계는 그대로 유지했다.
  - **원문 용어 교체**: 기록관 #2 "품을 팔러 떠돌아다니는 사람" → "유리민"(우리역사넷 「인구의 증감」, 정조 12년(1788) 호적 미등재 약 30% 추정 근거 확인).
  - 기록관 #3(장시 기록의 고을별 차이·미조사 범위)은 대조실이 "확인 불가"로 남겼으나, 역할이 스스로 밝히는 업무 범위 한계 서술이라 새로운 역사적 단정이 아니라고 판단해 그대로 두고 그 판단 근거를 문서에 명시했다.
- **품팔이(`laborer`) 카드 전면 재작성**: 대조실이 정리한 우리역사넷 「임노동자」·「광작과 광작농」 근거로 privateInfo를 새로 썼다. "몰락 농민 중 일부"로 범위를 두 번(2번 굵게, 5번 재확인) 못박았고, "모든 품팔이가 도시로 갔다"는 단일 경로 대신 농촌 잔류(고공)·도성 이동(세곡 하역) 두 경로를 원문 그대로 함께 제시했다.
- **글자 수 점검**: 조작대의 `scripts/15_validate_cooperative_packet.js` `--cards` 옵션이 이번 라운드에는 아직 올라오지 않아(`grep -n "\-\-cards"`로 두 차례 확인) 수작업으로 확인했다. `privateInfo` 중 60자를 넘긴 2곳(농민 #4, 기록관 #2)을 40자 안팎으로 압축했고, 나머지는 40~55자 구간이라 그대로 두었다.
- 문서 첫머리 상태를 "확정(2단계)"으로 바꾸고, §9(대조실 요청)·§10(확인 불가 목록)을 갱신했다.
- 검증: `04`·`05`·`13`·`14`·`15` 전부 통과. `15`가 낸 `joseon_late_packet.draft.json`의 자리표시자 15개 경고는 연결 공방의 2-나(§4) 몫이라 1단계 설계대로 비차단 취급했다. 부수 갱신된 조작대 소유 보고서 2개는 `git checkout --`으로 되돌렸다.
- 커밋 직전 `git status --short`로 대상 문서 1개만 바뀌었는지 확인했다.
- **여전히 확인 불가로 §11에 남은 항목**: 이모작의 조선 후기 시점·지역 범위, 장시 가격 변동의 구체 사료, 수공업자-보부상 간 특정 경쟁 인과. 대조실이 근거를 확인하면 다음 라운드에 복원 여부를 판단한다.
- 2-나(연결 공방이 확정 문장을 패킷 JSON에 옮기는 단계)는 기획 세션이 따로 지시할 때까지 착수하지 않는다.


## 2026-09-17 — 「관문 설계실」(gate-grammar) Round 8 §1: 역할 본문 확정본 v2(앱 계약)

`TASK-20260917-GATE-R8 | 역할 카드 v2 확정 — 추론 낮춤·대표 근거 1개·문장/메모 분리 | Claude Sonnet 5 · worktree claude-gate-grammar/feat/mud-gate-grammar | 상태: DONE`

- 지시서 `docs/handoff/claude_four_track_round8_instruction.md`(D-035) §0·§1. 조선 후기 콘텐츠를 실제 앱 계약(`apps/cooperative-live/convex/scenarios.ts`, 읽기만 함)에 맞춰 옮기는 1단계 작업이다. `docs/handoff/claude_joseon_late_role_cards.md` 한 파일만 고쳤다.
- **C1-04(직접 근거 없는 추론 문장 낮추기)**: 보부상의 값 변동 이문 문장(interest), 수공업자의 안 팔리는 달 살림 문장(privateInfo #4), 품팔이의 농번기 밖 벌이 불안정 문장(privateInfo #4) 세 곳을 "~할까 걱정입니다", "~일 것입니다" 같은 추론·걱정 표현으로 고쳐, 사실로 단정하는 서술이 남지 않게 했다.
- **역할당 대표 근거 1개**: 기존에 각 역할에 붙어 있던 단일 `evidenceId`(`farmer-ipbeop`/`bobusang-currency`/`jangsi-craftsman`/`record-keeper-scope`/`laborer-wage`)를 그대로 대표 근거로 확정하고, 각각 30자 안팎 label과 선택 이유를 한 줄씩 추가했다. 5개 id가 서로 겹치지 않음을 확인했고, 기록관은 지시서 요구대로 "자료의 한계"(유리민이 호적에서 빠짐)를 가리키는 근거를 대표로 삼았다.
- **학생 문장·편집 메모 분리(C1-05)**: 모든 역할 카드에서 줄 끝에 붙어 있던 `[출처: …]`류 대괄호 메모를 목록 항목 아래 들여쓴 "- 메모: …" 줄로 옮겼다. 조작대의 `--cards` 문장 분리 오탐(마침표가 아닌 곳에서 잘리던 문제)의 원인이었던 학생 문장 내 섞인 메모를 제거한 것이다.
- 문서 첫머리 상태를 "확정(v2, 앱 계약)"으로 바꾸고 라운드 8 v2 정정 안내를 추가했다.
- **검증**: 조작대가 이번 라운드에 올린 `node scripts/15_validate_cooperative_packet.js --cards`를 처음 돌려, 기록관 privateInfo #4가 71자(60자 초과)라는 경고를 받고 53자로 압축한 뒤 재실행해 경고 0건을 확인했다. `04`·`05`·`13`·`14`도 전부 통과. `--runtime <파일>` 옵션은 조작대의 §3.4 작업이 아직 이 워크트리에 올라오지 않아 실행하지 못했다(대기).
- `scripts/15`의 기본 모드(옵션 없음)는 여전히 `data/cooperative/joseon_late_packet.draft.json`(연결 공방 소유, 아직 라운드 6 산출물)을 기준으로 실패를 낸다 — 2-나에서 연결 공방이 앱 계약 JSON을 새로 조립하면 해소될 예상된 실패이며 이번 라운드 관문 설계실의 몫이 아니다.
- 부수 갱신된 조작대 소유 보고서 2개(`inquiry_combinatorics_audit.md`, `inquiry_semantics_lint.md`)는 `git checkout --`으로 되돌렸다.
- `git status --short`로 대상 문서 1개만 바뀐 것을 커밋 직전 확인했다.
- 2단계(연결 공방이 `docs/handoff/claude_joseon_late_runtime_scenario.json`을 조립하는 단계)는 기획 세션이 따로 지시할 때까지 착수하지 않는다.

## 2026-09-16 — 조작대(inquiry-console) 라운드 7 2단계: scripts/15 글 길이·가독성 검사 + --cards 옵션

`docs/handoff/claude_four_track_round7_stage2_instruction.md` §3.

- **길이·가독성 검사 추가**(`checkReadability`): `privateInfo`는 문장 수 3~5·문장당 60자 초과 경고(권장 40자 안팎), `sharePrompt`·`interest`·`firstJudgment.prompt`는 80자 초과 경고, `evidenceOptions[].shortLabel`은 30자 초과 경고, `limitOptions[].text`는 60자 초과 경고. `[PLACEHOLDER`가 남은 문자열은 아직 실제 문안이 아니므로 검사에서 제외한다.
- **기준값은 제안값**이라고 스크립트 주석과 이 기록에 명시한다 — 수업 리허설(10/12~16)에서 실측하며 조정한다. 경고만 내고 실패로 막지 않는다(`--final`도 이 검사에는 영향 없음, 자리표시자 개수만 실패로 전환).
- **`--cards` 옵션 신설**: 패킷 JSON 없이 역할 카드 문서(`claude_joseon_late_role_cards.md`)의 `privateInfo` 번호 목록만 같은 기준으로 검사한다. 관문 설계실이 2단계 §1에서 바로 쓸 수 있다.
- **버그 수정**: 문서 파싱 정규식이 `$` 앵커를 썼는데 파일이 CRLF(`\r\n`)라 모든 줄 끝에 `\r`이 남아 있어 어떤 줄도 매칭되지 않았다(`--cards`가 항상 "문장 0개"만 보고). `split('\n')`을 `split(/\r?\n/)`로 바꿔 고쳤고, 실제 파일로 재현·수정 확인.
- 실행 결과: 현재 패킷은 `evidenceOptions` 7곳과 `firstJudgment.prompt`가 30자/80자를 넘어 경고. 역할 카드 문서는 `farmer`·`craftsman`·`recordkeeper`·`laborer`의 문장 하나씩(총 5건)이 60자를 넘어 경고 — 관문 설계실이 참고할 수 있게 그대로 남겨 둠(내가 직접 고치지 않음, 남의 파일).
- 검증: `04`·`05`·`13`·`14` 통과. `15`(정상/`--final`/`--cards` 세 모드) 모두 의도대로 동작 확인. `git status --short`로 조작대 소유 파일만 바뀐 것 확인.

## 2026-09-17 — 기획 세션: 라운드 7 완료, 조선 후기 패킷(C1) 초안 확정

- 1단계: D-034 확정값으로 역할 카드와 패킷 id를 맞췄다. 공인은 품팔이로 교체했고, 질문 틀은 교사 자료로 옮겼으며, `scripts/15`에 교차 검사를 추가했다.
- 2-가: 대조실 판정을 반영해 역할 카드를 확정했다. 확인 불가 6건 중 3건을 해소했고, `scripts/15`에 글 길이 검사와 `--cards` 옵션을 넣었다.
- 2-나: 확정 문장을 패킷 JSON에 옮겼다(`e6feb9b`). 편집 메모와 출처 코드는 제외했다. `--final` PASS, 자리표시자 0곳.
- 남은 경고 14건은 제안값 기준의 길이 경고와 부정문 오탐 1건이다(`recordkeeper.interest`의 "모든 사람"). 리허설 뒤 조정한다.
- 교훈: 라운드 6은 id를 먼저 고정하지 않고 동시에 작업해 결과가 갈라졌다. 라운드 7은 2단계 순서로 풀었다(D-034).
- Codex 읽기 전용 감사 요청: `docs/handoff/codex_joseon_late_packet_audit_request_20260917.md`.

## 2026-09-17 — 조작대(inquiry-console) 라운드 8: scripts/15 문장 분리 수정 + --runtime 앱 계약 검사

`docs/handoff/claude_four_track_round8_instruction.md` §3.

### C1-05 문장 나누기 오탐 수정 + 회귀 fixture

- `splitSentences`가 어중의 "다"·"요" 뒤 공백에서도 문장을 잘라, "내다 팔아"·"다 조사하지는" 같은 문구에서 문장이 잘못 갈라졌다. 마침표·물음표·느낌표·줄바꿈에서만 나누도록 정규식을 좁혔다(`(?<=[.!?])\s+|\n+`).
- 이 버그가 실제로 낸 오탐: 기록관 `interest`("모든 사람과 모든 고을의 사정을 다 알 수 없다는 것도...")에서 "모든 사람"(과잉 단정어)과 그것을 부정하는 "알 수 없다"가 서로 다른 조각으로 갈라져 부정문 인식이 무력화됐다.
- `runRegressionFixtures()`를 추가해 `node:assert/strict`로 위 세 문구(내다 팔아·다 조사·기록관 interest)를 매 실행마다 자동 확인한다. 깨지면 스크립트가 즉시 실패한다.
- `--cards`: 항목 끝 메모가 중첩 백틱(예: 인용 URL 슬러그)을 포함해 "첫 백틱부터 줄 끝까지 자르기"로 바꿨다 — 짝을 맞춰 지우는 이전 방식은 중첩된 안쪽 텍스트를 남겼다.

### `--runtime <파일>` 신설

- `apps/cooperative-live/convex/scenarios.ts`(`CooperativeScenario`)·`shared/scenario.ts`(`PublicScenario`)를 **읽기만** 하고 그 모양을 옮겼다. 필수 필드 존재, `groupSizes`의 역할 참조, 역할당 `evidence`≥1·`firstChoices`≥2, `sharedPrompt.policies`≥3(모둠이 2개를 고름)·`limitations`≥1·`connections`≥1, `commonEvidenceByGroupSize`의 3/4/5 키, `sources`≥1(`claim`/`url`/`accessedAt`)을 검사한다.
- **앱 계약에 없는 최상위·역할·하위 필드는 실패로 낸다**(C1-01 "버려지는 학생용 필드 0개") — 패킷 초안의 `sharePrompt`(역할별 개별 문구)나 `targetOptions`가 그대로 남아 있으면 걸린다. 과잉 단정어·길이 검사도 패킷과 동일하게 적용.
- 대상 파일이 없으면(2단계 조립 전) 건너뛰고 통과한다.
- **검증**: 고려 초기(`earlyGoryeoPublic`) 실제 데이터를 본떠 만든 정상 fixture는 통과, 5가지 위반(계약에 없는 필드 2곳·존재하지 않는 role 참조·policies 부족·sources 누락)을 주입한 fixture는 정확히 5건 모두 실패로 잡히는 것을 확인 후 fixture 삭제.
- **보고**: `--runtime`은 콘텐츠 모양만 본다. 실제 등록 가능 여부는 Codex의 `validateScenarioRegistry()`와 앱 테스트가 판정한다(스크립트 출력에도 매번 이 문구를 남긴다).
- 기존 `--final`(패킷 초안 모델)은 대상 파일이 없으면 이미 건너뛰던 동작을 그대로 유지 — 2단계에서 초안이 삭제된 뒤에도 별도 수정 없이 건너뛴다.
- 검증: `04`·`05`·`13`·`14`·`15`(정상/`--cards`/`--runtime` 세 모드) 전부 통과. `git status --short`로 조작대 소유 파일만 바뀐 것 확인.


## 2026-09-18 — 「관문 설계실」(gate-grammar) Round 9 §1: 정규 편 연대표 `data/lesson_track.json`

`TASK-20260918-GATE-R9 | 정규 MUD를 단원 넘어 잇는 연대표 데이터 작성 | Claude Sonnet 5 · worktree claude-gate-grammar/feat/mud-gate-grammar | 상태: DONE(확인 불가 1건 있음)`

- 지시서 `docs/handoff/claude_four_track_round9_instruction.md`(D-036) §0·§1. 완료 화면에서 항상 전체 진도표로만 나가고 앞뒤 차시로 이어지는 길이 없던 문제(교사 관찰 EXP-008)를 해결하려고 단원 경계를 넘어 이어지는 연대표 띠 데이터를 만드는 1단계 작업이다. `data/lesson_track.json`(신규) 한 파일만 만들었다.
- **32 vs 28 불일치**: 지시서·D-036 모두 "정규 32편"을 전제했지만 `data/mud/_index.json`의 `tier: "regular"` 항목을 직접 세어 보니 28개였다(전체 MUD 32개 = 정규 28 + 심화 4, `08_validate_mud_catalog.py`가 보고하는 "32 MUD"는 이 전체 수다). 4편을 지어내지 않고 실제 28편으로 만들었고, `note` 필드와 완료 보고에 확인 불가 항목으로 남겼다 — 기획 세션이 32라는 숫자의 근거를 다시 확인해야 한다.
- **2단원 7차시 동률 처리**: `regular_myeongnyang`(명량대첩, 1597년)과 `regular_joseon_diplomacy`(병자호란, 1636년)가 둘 다 7차시다. 연대표는 실제 역사 순서를 보여주는 것이 목적이라는 D-036 배경에 따라, 교과서 차시 번호가 같더라도 실제 사건 발생 연도(1597 < 1636) 순으로 명량 편을 앞에 두었다. 판단 근거를 각 항목의 `orderNote`에 남겼다.
- **eraLabel 배치**: 지시서가 준 13개 라벨을 그대로 쓰되, 3단원 2차시 `regular_independence_army`(의병 항쟁·안중근, 1909년 이전 사건)는 1910년 이후를 가리키는 "일제강점기"가 아니라 앞 편과 같은 "개항기"로 묶었다 — 연도상으로도 맞고 교과서 흐름과도 어긋나지 않는다. 2단원 7차시(왜란·호란 두 편)는 지시서 라벨 목록에 별도 항목이 없어 "조선 전기"로 묶고 8차시(조선 후기 경제)부터 "조선 후기"를 시작했다.
- **eraRange 초안**: 전부 `eraRangeDraft: true`로 표시했다. 구석기는 라운드 5에서 전곡리를 "수십만 년"으로 낮춘 기존 판단과 어긋나지 않도록 지시서 예시 문구("약 70만 년 전")를 그대로 쓰지 않고 "수십만 년 전~약 1만 년 전"으로 적었다. 나머지는 통상 범위로만 채웠으며 대조실 검증(§2)을 기다린다.
- **shortTitle**: 번호식 이름("1차시"·"MUD 3") 없이 각 편의 핵심 소재로 6자 안팎 이름을 지었다(주먹도끼, 고인돌 축조, 병자호란, 8·15 광복 등).
- **자체 검증**: node 스크립트로 (1) `order` 1~28 연속, (2) `_index.json`의 regular 28개와 정확히 1:1 대응(빠짐·중복·유령 id 0건), (3) `unitId`가 순서상 비내림차순임을 직접 확인했다.
- **검증(스크립트)**: `01`·`04`·`05`·`06`·`08` 전부 통과. `16_validate_lesson_track.js`는 조작대가 이번 라운드에 아직 올리지 않아(병렬 작업, 지시서 §4.1에 "대상 파일이 없으면 건너뛴다"는 설계가 이미 있음) 실행하지 못했다.
- `git status --short`로 신규 파일 1개(`data/lesson_track.json`)만 있는 것을 커밋 직전 확인했다. `apps/**`·`cooperative-mud/**`는 읽지도 않았다.
- **대조실에 넘길 것**: 13개 `eraLabel`의 `eraRange` 검증과, 내가 임의로 정한 라벨 배치(의병·안중근→개항기, 왜란·호란 두 편→조선 전기) 재검토.
- **기획 세션에 넘길 것**: "정규 32편" 전제와 실제 28편의 불일치 확인.

## 2026-09-18 — 조작대(inquiry-console) 라운드 9: 정규 연대표 띠 구현(D-036)

`docs/handoff/claude_four_track_round9_instruction.md` §4.

### 지시서와 다른 사실 발견

- 지시서·D-036은 "정규 32편"이라고 반복해서 적지만, `data/mud/_index.json`을 직접 세면 `tier: "regular"`는 **28편**이고 나머지 4편은 `tier: "deep-dive"`다(32는 전체 MUD 수 — 아마 라운드 앞부분에서 "32 MUD 전체"와 "정규 편 수"를 혼동한 것으로 보임). `scripts/16_validate_lesson_track.js`는 "32"를 하드코딩하지 않고 `_index.json`에서 실제 regular 편 수를 세어 그 값과 대조하도록 만들어, 지시서 숫자가 틀려도 스크립트가 잘못된 기준을 강요하지 않는다 — 대신 실제 개수가 지시서와 다르면 경고로 알린다. 관문 설계실이 `data/lesson_track.json`을 만들 때 이 경고를 참고할 수 있다.

### `scripts/16_validate_lesson_track.js` 신설(먼저 만듦)

- 형식(필수 필드, order/unitId/lessonNumbers 정수, eraRangeDraft boolean), regular 편과 entries의 1:1 대응(빠짐·중복·유령 id·심화 편 혼입), order 1~N 연속, unitId·lessonNumbers 오름차순(같은 차시 두 편은 허용, 역순만 오류), shortTitle의 번호식 이름("N차시" 등) 금지를 검사한다.
- 대상 파일이 없으면 건너뛰고 통과. 임시로 실제 28편 fixture를 만들어 정상 통과와 5가지 위반(심화 편 혼입·order 불연속·번호식 이름·필수 필드 누락·mudId 중복)이 각각 정확히 잡히는 것을 확인한 뒤 삭제.

### `js/lessonTrack.js` 신설

- `data/lesson_track.json`을 읽어 완료/현재/미완료 세 상태(✓/●/○ — 색만으로 구분하지 않음)를 가로 띠로 그린다. 완료 여부는 **새 저장 키를 만들지 않고** 기존 `window.encyclopedia`(localStorage `history_explorer_save_v1`)의 `unlockedArtifacts`와 각 편 `data/mud/<id>.json`의 `rewards`를 대조해서 판단한다(보상 유물이 도감에 있으면 완료로 봄).
- 띠 항목을 누르면 `MudEngine.openMUD()`로 그 편으로 바로 이동한다.

### `js/mudEngine.js` 완료 화면에 앞뒤 이동 추가

- `renderFinalReflection`에 이전/다음 차시 버튼과, 접어 둔(`<details>`) 전체 진도 띠를 넣었다. 기존 "전체 탐구 진도표로 돌아가기" 버튼은 그대로 두고 그 위에 배치했다. 연결 공방의 화면 사양서(§3)가 아직 없어 기본 동작(완료 화면 배치, 접힘 기본값 닫힘)으로 먼저 구현 — 사양서가 나오면 맞춘다.

### 브라우저 확인 (`inquiry-console`, 8804)

임시 fixture(실제 28편)로 실제 플레이 경로를 확인했다.

- **1180×820**: 완료 화면에 "다음 차시 →" 버튼과 "▶ 전체 진도 띠 보기" 정상 표시. 클릭해 펼치면 28칸 띠가 가로 스크롤 컨테이너(`scrollWidth 2280px` vs `clientWidth 394px`)로 넘치지 않고 담김.
- **390×844**: 이전/다음 버튼 높이 71px·너비 132px, 띠 항목 52×78px — 최소 터치 목표 44px를 모두 넘음. 페이지 자체의 가로 스크롤은 발생하지 않음(`document.documentElement.scrollWidth <= window.innerWidth` 확인). 스크린샷으로 완료(✓, 초록)/현재(●, 빨강 테두리)/미완료(○) 세 상태가 색+기호로 함께 구분되는 것을 육안 확인.
- 두 크기 모두 콘솔 error/warning 0건.
- 띠 항목 클릭 시 `MudEngine.openMUD()`로 실제 이동하는 것, `window.encyclopedia.data.unlockedArtifacts`를 읽어 완료 상태가 정확히 반영되는 것(`regular_paleolithic` 완료 후 `isCompleted: true` 확인)을 스크립트로 확인.

### 검증

`01`·`04`·`05`·`06`·`08`·`16` 전부 통과. `git status --short`로 조작대 소유 파일(`js/lessonTrack.js`, `js/mudEngine.js`, `css/style.css`, `index.html`, `scripts/16_validate_lesson_track.js`)만 바뀐 것 확인.

## 2026-09-18 — Codex: 조선 후기 실시간 협동 MUD 앱 등록

`TASK-20260918-CODEX-JOSEON-LATE | 확정 런타임 JSON의 실제 앱 등록·3·4·5인 회귀 | implementation agent(Codex) | 상태: DONE`

- D-035의 파일 소유권에 따라 Claude가 확정한 `docs/handoff/claude_joseon_late_runtime_scenario.json`은 수정하지 않고, `apps/cooperative-live`의 공개·서버 레지스트리에 `joseon-late-market` version 1을 등록했다.
- 공개 레지스트리에는 제목·차시·권장 시간·역할 id/icon/name·3/4/5인 배치만 넣었다. 역할별 `privateInfo`·`interest`·`evidence`, 공동 질문, 자료 한계, 연결 문장, 개입 문구, 출처 8건은 서버 레지스트리에만 넣었다.
- 3인 모둠은 농민·보부상·기록관을 배치하고, 빠진 수공업자 관점을 `common-craftsman` 읽기 전용 공통 자료로 보충한다. 4인과 5인 모둠에는 공통 자료를 추가하지 않는다.
- 공개·서버 레지스트리 단위 테스트에 조선 후기 3·4·5인 고유 역할 배치와 3인 공통 자료 검사를 추가했다.
- Convex 가상 학급 테스트에 조선 후기 3·4·5인 각각의 실제 세션 생성→입장→편성→역할 확인→최초 판단 완료→자료 공유→공동 초안 단계→종료 삭제 흐름을 추가했다. 역할 단계에서는 자기 역할 외의 비공개 본문이 응답에 없는지, 공동 초안 단계에서 모둠 크기에 맞는 공통 자료가 나타나는지 확인한다.
- production 화면에서 공개 첫 화면에 고조선·고려 초기·조선 후기 세 활동이 함께 표시되는 것을 확인했다. 로컬 가상 교사 화면은 기존 설계대로 고려 초기 데모를 고정 표시하므로, 실제 시나리오 선택 UI는 Convex/Auth0 연결 Preview에서 후속 확인한다.
- production client bundle을 역할별 비공개 본문 고유 문구 5개로 검색해 0건임을 확인했다. 공개 제목·역할명·배치만 client bundle에 포함된다.
- 검증:
  - `node scripts/15_validate_cooperative_packet.js --runtime docs/handoff/claude_joseon_late_runtime_scenario.json` PASS(Claude가 기록한 interest 길이 제안 경고 3건만 유지)
  - `npm run check` PASS: ESLint, TypeScript, 공개 레지스트리·입장 보안 단위 테스트 19개, Convex 기존 테스트, Next.js production build
  - 추가 후 `npm run test:convex` PASS: 3개 테스트(조선 후기 3·4·5인 서버 회귀 포함)
  - `git diff --check` PASS
- 이번 범위에서 하지 않은 것: 콘텐츠 문안 수정, Preview/Production 배포, 실제 계정·학생 접속, 물리 기기 리허설, Claude 라운드 9 연대표 작업 수정.

## 2026-09-18 — 「관문 설계실」(gate-grammar) Round 9 2단계 §1: 연대 표기 확정

`TASK-20260918-GATE-R9-2 | 대조실 검증값 반영, eraRangeDraft 제거 | Claude Sonnet 5 · worktree claude-gate-grammar/feat/mud-gate-grammar | 상태: DONE`

- 지시서 `docs/handoff/claude_four_track_round9_instruction.md` "라운드 9 — 2단계" §1. 1단계에서 초안으로 남겨 둔 `data/lesson_track.json`의 `eraRange`를 대조실 `docs/audits/lesson_track_era_review.md` 제안표대로 확정하고 `eraRangeDraft` 표시를 뗐다. 이 파일 하나만 고쳤다.
- **연대 값 정정**(지시서가 명시한 항목): 통일신라는 이미 "676~935"로 맞아 있던 값을 유지(936은 고려의 후삼국 통일 완성이지 신라 멸망 연도가 아님), 구석기는 "수십만 년 전~약 1만 년 전"에서 끝 연도를 빼 "수십만 년 전~"으로(라운드 5·6의 "70만 년 전은 원문에 없다" 판단과 일치), 고조선은 "기원전 2333년(전한다)~기원전 108년"으로 전승 기록임을 드러냈다, 개항기는 "1876년~1910년"에서 끝 연도를 빼 "1876년~"으로, 청동기는 이미 시작만("기원전 2000년경~") 쓰고 있어 그대로 두었다.
- **나머지 표도 함께 반영**: 신석기 표기를 "기원전 8000년경~"으로 통일, 조선 전기(founding·sejong·status)는 "1392년~1592년경", 조선 후기(economy·silhak·folk)는 "1592년경~1876년경"으로 대조실 구분을 그대로 썼다.
- **설계 판단(지시서에 없어 직접 결정)**: 조선 전기의 새 표준 범위(~1592년경)를 명량해전(1597)·병자호란(1636~1637) 두 편에 그대로 적용하면 각 편의 실제 사건이 자기 `eraRange` 밖에 있게 되는 모순이 생긴다. `eraLabel`은 교과서 흐름(조선 후기 경제 편 앞)을 지키려고 그대로 "조선 전기"를 쓰되, `eraRange`만 편별 실제 연도(1597년 / 1636~1637년)로 바꾸고 `eraRangeNote`에 이유를 적었다. 1단계에서 정한 `orderNote`(동률 tie-break 근거)는 그대로 유지했다.
- **발해–통일신라 228년 겹침**: 대조실이 "연대를 억지로 쪼개지 말고 UI 안내로 해결"하라고 제안한 대로 `eraRange`는 그대로 두고, 발해 항목에 `eraOverlapNote` 필드를 새로 추가해 겹침 사실과 연결 공방 UI 사양서로의 위임을 데이터에도 남겼다.
- **자체 검증**: order 1~28 연속, 28편 1:1 대응, `eraRangeDraft` 잔존 0건을 node 스크립트로 재확인했다.
- **검증(스크립트)**: 조작대가 이번 1단계에서 만든 `scripts/16_validate_lesson_track.js`를 처음 돌려 PASS를 확인했다 — 다만 "_index.json 기준 28편"과 지시서 "32편"이 다르다는 경고는 계속 뜬다(2-§4에서 조작대가 문구를 다듬기로 했으므로 내가 손대지 않았다). `01`·`04`·`05`·`06`·`08`도 전부 통과.
- `git status --short`로 커밋 직전 대상 파일 1개(`data/lesson_track.json`)만 바뀐 것을 확인했다.
- **연결 공방에 넘길 것**: 발해 `eraOverlapNote`("통일신라와 같은 시대") 안내 문구를 화면에 반영해 달라 — 대조실 §2-1과 같은 요청이다.

## 2026-09-18 — 기획 세션: 라운드 9(정규 편 연대표 띠) 1단계·2단계 일부 통합

- 1단계: `data/lesson_track.json`(정규 28편), 시대 연대 검증표, 화면 사양서, `js/lessonTrack.js`·완료 화면 앞뒤 이동·`scripts/16`을 통합했다.
- **기획 세션의 착오 정정**: 지시서가 "정규 32편"이라고 썼으나 실제 정규 편은 28편이다(32는 심화 4편 포함). `scripts/16`이 개수를 세어 대조하도록 만들어져 경고로 잡혔다(D-036 정정).
- 2단계 일부: 연대 표기를 검증값으로 확정했다(`eraRangeDraft` 0건).
  - 통일신라 끝을 **935년**으로 정정했다(936년은 고려의 후삼국 통일 연도. 참고한 외부 사이트 표기가 틀렸다).
  - 구석기는 "수십만 년 전~"으로 두어 라운드 5 판단과 맞췄다.
  - 고조선은 "기원전 2333년(전한다)"로 전승임을 드러냈다.
  - 개항기·청동기는 끝 연도를 확인하지 못해 시작만 적었다.
  - 발해(698~926)와 통일신라(676~935)는 실제로 겹치므로 연대를 조정하지 않고, 띠가 연표가 아니라 차시 순서임을 문구로 알리기로 했다.
- 검증: 01·04·05·16 통과. 조작대의 브라우저 확인은 1단계 임시 데이터 기준이며, **실제 28편 데이터로의 재확인은 남아 있다**(라운드 9 2-§4).
- 남은 일: 조작대 2단계(사양 반영, 실제 데이터 브라우저 확인, `scripts/16` 경고 문구 정리).

## 2026-09-18 — Codex 작업 기록 및 다른 PC 이전 안전성 확인

`TASK-20260918-CODEX-HANDOFF-SAFETY | Codex 문서·조선 후기 작업의 원격 보존 확인 | integration agent(Codex) | 상태: DONE`

- 역사 여정 연결 관점 3종(개별 MUD의 여정 연결, 유물을 증거로 활용, 개인 증거에서 공동 해석으로 확장)과 교사 제작 역사 웹앱 사례 메모를 `INBOX.md` 및 `docs/handoff/claude_history_journey_connection_followup_20260918.md`에 기록한 커밋 `e07c831`이 `origin/main`에 포함됐음을 확인했다.
- 조선 후기 실시간 협동 MUD 앱 등록 커밋 `39f4fd8`과 감사 커밋 `45404d7`도 `origin/main`의 조상임을 확인했다. 따라서 별도 Codex worktree 폴더가 새 PC에 복사되지 않아도 원격 `main`에서 내용과 이력을 복구할 수 있다.
- `audit/joseon-late-packet-20260917`이라는 별도 원격 브랜치는 없지만 감사 커밋 자체는 원격 `main`에 포함되어 유실 위험이 없다. 새 PC에서는 필요할 때 해당 커밋이나 `origin/main`을 기준으로 worktree를 다시 만든다.
- 원격 갱신 후 로컬 `main`과 `origin/main`이 모두 `1ef85a6`을 가리키며 작업 트리가 깨끗한 상태임을 확인했다.
- 확인 명령: `git fetch origin --prune`, `git status --short --branch`, `git merge-base --is-ancestor <commit> origin/main`, `git ls-tree -r --name-only origin/main -- <path>`.

## 2026-09-21 — 기획 세션: 라운드 9 2단계 마무리(연대표 띠 사양 반영, 직접 확인)

조작대 몫으로 남아 있던 2-§4를 기획 세션이 직접 구현했다. 사양서(`docs/plans/implementation_plan_lesson_track_ui.md` §8 대조표)에서 **어긋남**으로 표시된 항목을 모두 반영했다.

- **위치**: 완료 화면의 접힌 `<details>` 안에 있던 전체 띠를 없애고, **포털(진도표) 화면 커리큘럼 카드 위에 항상 펼쳐진 띠**로 옮겼다. 단원 탭과 무관하게 28편 전체를 보여 준다.
- **레이아웃**: 한 줄 28칸을 **단원별 3줄**(12/9/7)로 나눴다. 1180×820에서 가로 스크롤 없이 들어가고 칸은 60×74px(터치 목표 44px 이상)다. 480px 이하에서만 단원 줄별로 가로 스크롤한다.
- **상태 구분 3채널**: 완료 `✓`/실선, 지금 `📍`/**점선 테두리**, 아직 아이콘 없음. 칸 아래에 "완료"·"지금" 글자 라벨을 항상 보여 준다. **'아직' 칸의 opacity 흐림을 없앴다**(저시력 학생 가독성).
- **문구**: 제목을 `🧭 나의 역사 탐구 순서`로 바꾸고, **"이 띠는 배운 순서예요. 실제 연대순은 아니에요 — 통일신라와 발해는 같은 시대에 함께 있었어요"** 캡션을 상시 노출했다. 통일신라·발해 칸의 설명에는 서로를 가리키는 문구를 넣었다.
- **완료 화면**: 버튼 문구를 `◀ 이전 차시로`/`다음 차시로 ▶`로 맞추고, 없는 방향은 자리표시자 없이 렌더링하지 않는다(첫 편은 다음만, 마지막 편은 이전만).
- `scripts/16`의 "32편" 경고 문구를 정리했다. 개수는 계속 데이터에서 센다.

**직접 확인(inquiry-console 8804, 실제 `data/lesson_track.json` 28편)**

| 확인 | 결과 |
|---|---|
| 1180×820 자동 렌더 | 28칸·3줄, 페이지 가로 스크롤 없음, 콘솔 오류 0 |
| 390×844 | 단원 줄별 가로 스크롤(예: 1단원 clientWidth 358 / scrollWidth 886), 페이지 가로 스크롤 없음 |
| 상태 표시 | 현재 칸 `border-style: dashed`, 아직 칸 `opacity: 1` |
| 통일신라·발해 칸 설명 | "(676년~935년, 발해와 같은 시기가 있었어요)" / "(698년~926년, 통일신라와 같은 시기가 있었어요)" |
| 완료 화면 앞뒤 | 첫 편은 다음만, 중간은 둘 다, 마지막 편은 이전만 |

**중간에 발견한 결함**: `app.js`에 포털 렌더 호출을 넣고도 캐시버스터를 올리지 않아 브라우저가 옛 `app.js`를 써서 띠가 그려지지 않았다. 검사 스크립트는 통과하고 있었다 — 캐시버스터 누락은 자동 검사로 잡히지 않는다는 것을 다시 확인했다(`60c4254`).

- 검증: 01·04·05·06·08·09·11·13·14·16 통과.
- 남은 일: `.claude/launch.json`에 통합 worktree용 포트(8805)를 추가하려다 되돌렸다 — 메인 체크아웃의 설정을 앱이 읽기 때문에 이 파일은 Codex와 함께 쓰는 main에서만 의미가 있다. 필요하면 별도로 정리한다.

## 2026-09-21 — Codex 요청 A 재검증 및 연대표 띠 읽기 전용 감사

`TASK-20260921-CODEX-LESSON-TRACK-AUDIT | 조선 후기 등록 재검증·연대표 띠 읽기 전용 감사 | integration/audit agent(Codex) | 상태: DONE`

- `main`을 `git pull --ff-only`로 `2982c77`까지 fast-forward한 뒤 인계 문서를 확인했다.
- 요청 A는 이미 `fbe3e82`에 병합돼 있었다. 런타임 콘텐츠 계약 검사와 `apps/cooperative-live`의 전체 `npm run check`를 다시 실행해 공개·서버 레지스트리, 3·4·5인 역할 배치, 3인 공통 자료, 클라이언트 비공개 본문 미노출, lint·typecheck·unit/Convex tests·production build를 모두 통과했다. 콘텐츠 원문의 `interest` 길이 제안 경고 3건은 기존 예상값이며 문안을 수정하지 않았다.
- 요청 B는 코드 변경 없이 읽기 전용으로 수행해 [`lesson_track_red_team_audit.md`](./docs/audits/lesson_track_red_team_audit.md)에 10개 발견 사항과 수용 기준을 기록했다. 높은 심각도는 보상 유물 기반 완료 오판 가능성과 `<button role="listitem">`의 버튼 의미 손실이다.
- Claude가 작업 중인 3단원 데이터 파일은 열거나 수정하지 않았다.
- 검증: `node scripts/15_validate_cooperative_packet.js --runtime docs/handoff/claude_joseon_late_runtime_scenario.json`, `npm run check`(`apps/cooperative-live`), `node scripts/16_validate_lesson_track.js` 통과.

## 2026-09-21 — 기획 세션: 연대표 띠 red team(LT-01~10) 1차 대응

Codex 읽기 전용 감사(`docs/audits/lesson_track_red_team_audit.md`, 기준 `2982c77`)에 대응했다. **코드로 고칠 수 있는 5건을 처리하고, 데이터·검증기 5건은 라운드 11로 넘긴다.**

| ID | 내용 | 처리 |
|---|---|---|
| LT-01 | 보상 유물 보유를 편 완료로 간주 | **수용·수정.** `encyclopedia.data.completedMuds`(mudId 배열)를 새로 두고 완료 시 `markMudCompleted()`로 기록한다. 판정은 이 기록을 먼저 보고, 기존 저장본을 위해 보상 유물 보유를 보조 근거로만 쓴다. 기존 저장은 `loadData()`에서 빈 배열로 채워 호환한다. |
| LT-02 | 완료한 편을 다시 열면 '지금'이 '완료'를 덮음 | **수용·수정.** `current-done` 상태를 추가해 "지금 · 완료"로 함께 보여 준다. |
| LT-04 | `<button role="listitem">`이 버튼 역할을 가림 | **수용·수정.** 목록 의미는 감싸는 `<span role="listitem">`이 맡고 버튼은 네이티브 역할을 유지한다. 현재 칸에는 `aria-current="step"`을 준다. |
| LT-05 | 키보드 포커스 표시가 브라우저 기본뿐 | **수용·수정.** `.lesson-track-item`·`.lesson-track-nav-btn`에 `:focus-visible` 3px 외곽선. |
| LT-06 | '아직' 상태가 스크린리더에 안 읽힘 | **수용·수정.** 시각 라벨은 비워 두고 `aria-label`에는 "아직"을 넣는다. |
| LT-03·LT-10 | 검증기가 메타데이터 어긋남·연대 회귀를 못 잡음 | **수용, 라운드 11.** `scripts/16`에서 `unitId`·`lessonNumbers` exact 비교, 파일 존재·내부 mudId 확인, 연대 표기 회귀 fixture. |
| LT-08 | `조선 전기` 라벨과 1597·1636년 표기가 모순 | **수용, 라운드 11.** 명량·병자호란 편의 시대 라벨을 다시 정한다(대조실·관문 설계실). |
| LT-09 | 열린 범위(`1876년~`)가 현재까지로 읽힘 | **수용, 라운드 11.** 표기 규칙을 통일한다. |
| LT-07 | 비동기 렌더·완료 갱신을 스크린리더에 공지 안 함 | **기록만.** 과도한 낭독 위험이 있어 수업 관찰 뒤 판단한다. |

**직접 확인(integration 8805)**

| 확인 | 결과 |
|---|---|
| 보상 유물 없이 `completedMuds`만으로 완료 표시 | 구석기·신석기 `is-done`, 다음 편이 `is-current`(`aria-current="step"`) |
| 완료한 편 재방문 | `is-current-done`, 화면·aria 모두 "지금 · 완료" |
| 버튼 역할 | `BUTTON` 태그 유지, `role` 속성 없음, 감싼 `<span role="listitem">` |
| '아직' 낭독 | aria-label "… — 아직" |
| 키보드 포커스 | Tab 이동 시 `:focus-visible` 3px 파란 외곽선 적용 |

- `.claude/launch.json`에 통합 worktree 확인용 `integration`(8805)을 추가했다. 메인 체크아웃의 설정 파일이라 Codex와 공유한다.
- 검증: 05·06·16 통과.


## 2026-09-21 — 「관문 설계실」(gate-grammar) Round 10 §1: 3단원 정규 7편 사실 대조

`TASK-20260921-GATE-R10 | D-032 해제 뒤 첫 3단원 사실 대조, 확인된 오류만 수정 | Claude Sonnet 5 · worktree claude-gate-grammar/feat/mud-gate-grammar | 상태: DONE`

- 지시서 `docs/handoff/claude_four_track_round10_instruction.md` §0·§0-2·§1. 1·2단원과 확장 활동 대조는 이미 끝났고, 3단원(일제강점기·광복·6·25)만 라운드 3~8 동안 "수정 금지"로 묶여 있다가 D-032로 이번에 처음 열렸다. 정규 7편(`regular_independence_army`·`regular_japanese_rule_1`·`regular_independence`·`regular_japanese_rule_2`·`regular_gwangbok`·`regular_korean_war`·`regular_post_war`) 전부를 문장 단위로 대조했다.
- §0-2가 요구하는 3단원 특유의 기준(일반화 금지·인물 평가 새로 하지 않기·숫자 엄격·감정 자극 지양·남북 양쪽 균형)을 체크리스트 삼아 각 편의 `narrative`·`choices`·`glossary`·시뮬레이터 `feedback`/`infoText`를 훑었다. IF 단계(오답 선택지의 "배신", "독재자 지명" 같은 극단적 텍스트)는 지시서대로 가상 상황이라 대조 대상에서 뺐다.
- **WebFetch로 원문을 직접 연 4건**: 아우내 장터 참여 인원("약 3천여 명", 우리역사넷 「유관순」), 기미독립선언서 저자 논쟁(공약 3장 작성자는 학설 대립, 우리역사넷 「3·1 독립 선언서」), 국호 '대한민국' 제안자(신석우, 우리역사넷 — 기존 서술 정확해 수정 없음), 대한광복회 결성자·김좌진 가입 시점(1915년 박상진 주도 결성, 김좌진은 1916년 이후 가입). 나머지 항목은 검색 결과 종합(공식 도메인만 채택)으로 확인하고 `docs/audits/unit3_regular_fact_check.md` 표에 원문 열람/검색 확인을 구분해 적었다.
- **확인된 오류 9건**을 5개 파일에서 고쳤다.
  - `regular_independence.json`: 아우내 장터 "3천 명"(확정 수치처럼 쓴 추정치)→"약 3천여 명"(원문 표현). 기미독립선언서 glossary의 "한용운 등이 검토"는 학계 논쟁 사안이라 삭제하고 "최남선이 기초"만 남김.
  - `regular_japanese_rule_1.json`: roadmap 라벨과 stage 3 배지에 "독립의군부"가 있었지만 정작 그 관문의 glossary·narrative는 대한광복회·송죽회만 다뤄 실제 내용과 어긋났다(라운드 5 삼국생활 편과 같은 유형의 로드맵-내용 불일치) — 두 곳 모두 "대한광복회와 송죽회"로 정정. 대한광복회 glossary도 "박상진, 김좌진 등이 결성"에서 "박상진이 주도해 결성하고 김좌진 등이 참여"로 고쳐, 김좌진이 1915년 결성 당시가 아니라 1916년 이후 가입했다는 원문과 맞췄다.
  - `regular_japanese_rule_2.json`: 창씨개명 "1939년"은 조선민사령 개정 공포 연도일 뿐 실제 시행은 1940년 2월부터라 "1939년 공포해 1940년부터"로 구분. 조선어학회 infoText의 "숭고한 희생 덕분에 자유롭게 쓰고 있다"는 단정적 인과("덕분에")라 "노력은 ... 중요한 밑거름이 되었다"로 완화.
  - `regular_post_war.json`: "교육열" 정의의 "한국인의 유별난 열정"·"경제 성장의 원동력이 됨"은 국민 전체를 하나의 특성으로 묶는 일반화이자 단일 원인 단정이라 §0-2 1에 따라 완화. "유네스코와 전 세계가 ... 깊이 감동했습니다"는 뒷받침할 공식 출처를 찾지 못해 확인 불가로 보고 삭제(자료 근거 없는 감정 서술, §0-2 4). "천막 교실에서 배운 아이들이 ... 산업화와 민주화를 이끌었습니다"도 과잉 인과라 "다양한 분야에서 일하며 이후 사회 변화에 참여한 사람들도 있었다"로 낮춤.
  - `regular_gwangbok.json`: 사실 오류는 없었지만 `sources`가 "6·25 전쟁" 링크만 있고 광복·정부 수립을 직접 다루는 공식 출처가 없어 보강.
- **이미 잘 되어 있어 수정하지 않은 것**: `regular_independence_army`(확정 수치 없음), `regular_japanese_rule_1`의 토지 조사 사업 서술(결과를 특정 비율로 단정하지 않고 이미 "자료를 연결해 살펴봅시다"로 열어 둠 — 지시서가 특히 우려한 인과 단정 항목), `regular_japanese_rule_2`의 강제 동원 규모(구체 수치 전혀 없음), `regular_korean_war`(사상자 수 없이 남북 어느 한쪽에 치우치지 않음 — §0-2를 가장 모범적으로 지킨 편).
- **확인 불가로 남김**: `regular_gwangbok`의 조선건국준비위원회 인물(여운형·안재홍) — 통상 알려진 사실이나 이번에 연 페이지에는 이름이 없었음. `regular_post_war`의 배지 문구 "천만 이산가족" — 대중적 표현이나 공식 수치 출처를 이번엔 확인 못함(본문 서술이 아니라 그대로 둠).
- 5개 파일 각각에 이번 확인 내용을 반영한 `sources` 항목을 추가해 `claimScope`와 확인 방식을 남겼다.
- 검증: `01`·`04`·`05`·`11`·`16` 전부 통과. `scripts/17_lint_sensitive_wording.js`는 조작대가 이번 라운드에 아직 올리지 않아 실행하지 못했다.
- `git status --short`로 대상 6개 파일(데이터 5개 + `docs/audits/unit3_regular_fact_check.md` 신규)만 바뀐 것을 커밋 직전 확인했다. `apps/**`는 읽지 않았다.
- 판단을 기획 세션에 넘길 항목은 없다.

## 2026-09-21 — 조작대(inquiry-console) 라운드 10: 3단원 민감 주제 표현 lint

`docs/handoff/claude_four_track_round10_instruction.md` §4.

### `scripts/17_lint_sensitive_wording.js` 신설

- D-032 §0-2가 조심하라고 정한 5개 신호(전칭 표현, 확정 수치, 인과 단정, 감정 자극어, 인물 평가어)를 3단원 데이터에서만 모은다. `scripts/14`와 같은 철학 — 실패가 아니라 검토 목록이며, 허용 목록(`ALLOWLIST`)으로 사람이 확인한 문구를 다음 실행부터 조용하게 만든다(현재는 비어 있음, 실제 검토가 나오면 채운다).
- 대상은 `_index.json`에서 `tier: "regular" && unitId: 3`으로 **동적으로** 뽑은 7편(regular_gwangbok·independence·korean_war·independence_army·japanese_rule_1·japanese_rule_2·post_war) — 라운드 9에서 "32"를 하드코딩하지 않은 것과 같은 이유로, 여기서도 단원 소속을 데이터에서 직접 센다. 그 외 `data/quizzes.json`(`unit: "일제강점기 및 근현대"`), `data/artifacts.json`(지정 9종), `stories*.json`·`causeEffectChains.json`(단원 필드가 없어 제목·chasi 키워드로 3단원을 추정 — 사람이 재확인해야 함을 보고서에 명시).
- **자기 회귀 테스트에서 실제 버그 발견**: "아니다"의 존댓말 활용형 "아닙니다"는 "니"+"ㅂ"이 "닙"으로 합쳐져 문자열 "아니"를 포함하지 않는다 — `NEGATION_PATTERN`의 "것은 아니" 조건만으로는 "것은 아닙니다"(교육용 문장에서 더 흔한 존댓말체)를 부정문으로 인식하지 못했다. `runSelfTests()`가 이 사례를 넣자마자 즉시 실패해 잡았고, "아닙니다"를 별도 패턴으로 추가해 고쳤다. `scripts/15`의 C1-05(문장 분리 오탐)와 같은 계열의 함정이라 같은 자기 검증 방식을 그대로 적용한 것이 유효했다.
- 3단원 7편 실행 결과: 신호 5건(인과 단정 4건 — "때문에"·"덕분에"가 정책·희생과 결과를 잇는 문장, 인물 평가어 1건 — "매국"), 허용됨 0건. 원문 대조는 관문 설계실 몫이라 이 스크립트는 고치지 않고 목록만 낸다.
- 1·2단원 확대는 **제안만** 하고 구현하지 않았다(§4 주의) — 이미 검증된 문장이 무더기로 걸리는 것을 막기 위해서다. 절차 제안을 보고서 끝에 남겼다.

### 검증

`01`·`04`·`05`·`11`·`16`·`17` 전부 통과. `16`은 이번 라운드부터 관문 설계실이 만든 실제 `data/lesson_track.json`(28편, `eraRange` 확정값 반영)이 들어와 처음으로 **실제 파일 기준 PASS**를 확인했다 — 지난 라운드에 내가 보고한 "32→28" 정정이 그대로 반영된 것도 `note` 필드에서 확인. `git status --short`로 조작대 소유 파일(`scripts/17_lint_sensitive_wording.js`, `docs/audits/sensitive_wording_lint.md`)만 새로 생긴 것 확인 — 다른 파일은 건드리지 않음.

### 확인 불가·판단을 넘길 항목

- `causeEffectChains.json`·`stories*.json`은 단원 구분 필드가 없어 제목 키워드로 3단원 여부를 추정했다. 정확한 소속은 연결 공방이 확인해야 한다.
- 5건의 신호가 실제 과잉 단정인지는 원문 대조가 필요하다 — 관문 설계실(`unit3_regular_fact_check.md`)에 넘긴다.

## 2026-09-21 — 기획 세션: 라운드 10(3단원 사실 대조) 통합

D-032로 수정 금지가 풀린 뒤 첫 라운드다. 1·2단원과 확장 활동에 이어 **3단원까지 사실 대조를 마쳤다.**

- **관문 설계실**(정규 7편): 확인된 오류 9건 수정.
  - 대한광복회 결성 주체를 원문대로 "박상진이 주도하고 김좌진 등이 참여"로 고쳤다.
  - 창씨개명은 "1939년 공포, 1940년부터 시행"으로 고쳤다.
  - 무단 통치 편 3관문 라벨이 실제 내용(대한광복회·송죽회)과 달라 맞췄다.
  - 아우내 장터 "3천 명"을 "약 3천여 명"으로 낮췄다.
  - 전후 편의 "한국인의 유별난 교육열", "유네스코와 전 세계가 감동", "천막 교실 아이들이 산업화·민주화를 이끌었다" 같은 **원문에 없는 일반화·감정 서술**을 원문 범위로 낮췄다.
  - "천만 이산가족" 등 2건은 확인 불가로 남겼다.
- **대조실**(유물 9종): **hint 차시 번호 오류 2건**을 고쳤다(3·1 운동 "38~39차시" → 4~5차시, 6·25 "44~46차시" → 10~12차시. 옛 48차시 통합 번호가 남아 있었다). 투표함·비밀 결사 기록 등은 실물이 아닌 상징 표현임을 밝혔다.
- **연결 공방**(퀴즈·스토리·원인결과): 퀴즈 해설의 "숭고한 정신"을 사실 서술로 바꿨다. 3단원 스토리가 **아예 없다**는 사실을 기록했다(향후 콘텐츠 판단 사항).
- **조작대**: `scripts/17_lint_sensitive_wording.js` 신설. 3단원만 대상이고, 전칭·확정 수치·인과 단정·감정 자극어·인물 평가어를 검토 목록으로 모은다.

**통합 뒤 lint 4건에 대한 기획 세션 판단**
- 인과 3건("전쟁과 분단 때문에 만나지 못한 가족", "탄압 때문에 비밀리에" 2건)은 교과서 수준 서술이라 **허용 목록**에 사유와 함께 넣었다.
- **"동지를 배신한 밀고는 민족 독립운동을 파괴하는 매국 행위입니다"**(무단 통치 편 3관문 오답 피드백)를 고쳤다. 학생이 고른 선택지에 "매국"이라는 평가어를 붙이는 것은 5학년에게 과하고, D-032 3(인물 평가를 공식 출처 표현 이상으로 하지 않음)의 취지에도 어긋난다. **결과(동지가 위험에 빠짐)와 이유(비밀 결사는 신뢰로 유지됨)** 를 설명하는 문장으로 바꿨다.
- 재실행 결과 검토 신호 **0건**(허용 3건 별도).
- 검증: 01·04·05·06·08·09·11·16·17 통과.


## 2026-09-21 — 「관문 설계실」(gate-grammar) Round 11 §1: D-037 연대 표기 시대 단위 통일

`TASK-20260921-GATE-R11 | 명량·병자호란을 조선 후기로, 같은 시대명은 같은 연도로 통일 | Claude Sonnet 5 · worktree claude-gate-grammar/feat/mud-gate-grammar | 상태: DONE`

- 지시서 `docs/handoff/claude_four_track_round11_instruction.md`(D-037, Codex 감사 `lesson_track_red_team_audit.md` LT-08·LT-09 대응) §0·§1. `data/lesson_track.json` 한 파일만 고쳤다.
- 배경: 라운드 9 2단계에서 명량·병자호란을 "조선 전기" 칸에 두되 편별 실제 연도(1597년, 1636~1637년)를 넣어 화면에 "조선 전기(1392~1592년경)"와 편별 연도가 동시에 보이는 모순이 있었다. D-037은 이걸 "eraLabel은 시대 범위, eraRange는 그 시대 전체 범위"로 규칙을 단순화하고, 사건 연도는 MUD 본문이 다루게 했다.
- **명량·병자호란 → 조선 후기**: `eraLabel`을 "조선 후기"로, `eraRange`를 다른 조선 후기 칸과 같은 "1592년경~1876년경"으로 맞췄다. 자기모순 회피용으로 넣었던 `eraRangeNote`는 지웠고, 순서 결정과는 무관한 `orderNote`(7차시 동률 tie-break 근거)는 남겼다.
- **같은 eraLabel은 같은 eraRange**(D-037 1): "광복·대한민국" 3편이 "1945년~"/"1950년~1953년"/"1953년~"으로 갈려 있던 것을 "1945년~현재"로 통일했다.
- **끝이 열린 표기**(D-037 3): "광복·대한민국"은 현재 진행 중인 시대라 "현재"를 명시("1945년~현재"), "개항기"는 끝 연도(1910년)가 아직 원문으로 확인되지 않아 지시서 기본값대로 "1876년 이후"로 썼다.
- **직접 판단해 확장한 것**: 구석기·신석기·청동기가 기존에 "수십만 년 전~"처럼 bare "~"로 끝나는 표기를 쓰고 있었다. 지시서 §1은 광복·개항기 두 사례만 명시했지만, D-037 3의 원칙("끝을 확인하지 못한 시대는 '○○년 이후'로 쓴다")은 특정 시대에 한정되지 않는 일반 규칙이고, 조작대가 이번 라운드에 추가하는 LT-10 검사("~"로 끝나는 열린 표기는 "~현재" 외엔 전부 실패)가 그대로 이 세 칸에도 적용될 것이므로 함께 "○○ 이후"로 통일했다. `note` 필드에 이 확장 이유를 남겼다.
- **자체 검증**: node 스크립트로 (1) 같은 eraLabel의 eraRange가 모두 일치, (2) "~현재"가 아닌 열린 표기가 하나도 남지 않음, (3) order 1~28 연속, 세 가지를 재확인했다.
- **검증(스크립트)**: `node scripts/16_validate_lesson_track.js`(기존 형식 검사) PASS. `01`·`04`·`05` 전부 통과. 조작대의 LT-10(eraLabel 일관성·열린 표기 검사)은 이 워크트리에 아직 올라오지 않아 실행하지 못했다 — 올라오면 재확인한다.
- `git status --short`로 커밋 직전 대상 파일 1개만 바뀐 것을 확인했다. `apps/**`는 읽지 않았다.
- **이어받아 다음 라운드로 넘기는 것**: 대조실이 이전 라운드(도감 9종 재검토) 중 발견한 `data/mud/regular_independence.json`·`regular_korean_war.json`의 `header.tag` 차시 번호 오류(3단원인데 "38~39차시"/"44~46차시"로 남아 있음, 실제는 4~5/10~12차시)를 이번엔 손대지 않았다 — 지시서가 `data/lesson_track.json`만 고치라고 명시했기 때문이다. 다음 라운드 지시서에 반영을 요청한다.
- **대조실에 넘길 것**: 개항기 끝 연도(1910년, 국권 피탈)를 원문으로 확인해 달라(§2-1) — 확인되면 다음 라운드에 "1876년~1910년"으로 바꿀 수 있다.

## 2026-09-21 — 조작대(inquiry-console) 라운드 11: `scripts/16` 검증기 보강(LT-03·LT-10, D-037)

`docs/handoff/claude_four_track_round11_instruction.md` §4.

### LT-03 — `lesson_track.json` ↔ `_index.json` ↔ 실제 mud 파일 3중 대조

- entries의 `unitId`·`lessonNumbers`가 `_index.json`의 같은 mudId 항목과 **정확히** 같은지 비교한다(라운드 9~10까지는 mudId 존재 여부만 봤지 메타데이터 값 자체는 대조하지 않았다).
- `_index.json`의 `file`이 가리키는 `data/mud/*.json`이 실제로 존재하는지, 그 파일 안의 `mudId`가 인덱스와 같은지 확인한다.
- `data/mud/regular_*.json` 디스크 목록과 `_index.json`의 regular 목록을 교차 대조해 **등록 누락**(디스크엔 있는데 인덱스에 없는 파일)을 잡는다.
- 검증 로직을 `validateMudCrossReference(entries, mudIndex, readMudFileFn, listMudFilesFn)` 형태로 분리해, 실제 `fs` 대신 가짜 파일 목록/내용을 주입해 자기 테스트할 수 있게 했다.

### LT-10 — 연대 회귀 검사(D-037)

- **같은 `eraLabel`이면 같은 `eraRange`**여야 한다 — 다르면 실패. (관문 설계실이 D-037로 통일하기 전 상태인 내 worktree의 `data/lesson_track.json`에는 "조선 전기"(3가지 범위)·"광복·대한민국"(3가지 범위)가 섞여 있어 실제로 이 검사가 잡아낸다.)
- `eraRange`가 `~`로만 끝나는 **열린 표기를 금지**한다. `~현재`만 예외로 허용한다(D-037 3). 구석기·신석기·청동기·개항기·광복 계열의 현재 열린 표기들이 이 규칙에 걸린다 — 관문 설계실이 라운드 11에서 개항기·광복·대한민국만 명시적으로 닫도록 지시받았으므로, 나머지(구석기·신석기·청동기)는 이번 라운드엔 열린 채로 남을 수 있고 그러면 계속 실패로 남는다. **이건 다음 라운드에 넘길 사항**으로 아래에 적는다.
- `docs/audits/lesson_track_era_review.md`(대조실, 라운드 9)의 "제안 표기" 열과 라운드 11 지시서 §1-3(개항기 "1876년 이후", 광복·대한민국 "1945년~현재")을 그대로 옮겨 `EXPECTED_ERA_RANGES` 표로 스크립트 안에 두었다. 값이 실제 데이터와 다르면 **실패가 아니라 경고**로 낸다(의도된 정정일 수 있어서) — 경고 메시지에 "EXPECTED_ERA_RANGES와 검증 문서를 함께 갱신하라"고 안내한다.

### `--ci` 옵션

- 기본 실행은 지금처럼 `lesson_track.json` 부재를 건너뛰고 통과한다(관문 설계실과 병렬 작업 중 파일이 아직 없을 수 있음).
- `--ci`를 주면 부재를 **실패**로 처리한다. 통합 파이프라인에서 파일이 반드시 있어야 함을 강제하기 위함이다.
- 두 경로 모두 직접 파일을 지웠다 복원하며 확인했다(`git status --short`로 원본이 그대로 복원됐음을 확인).

### 일부러 틀린 fixture로 실패 확인 (지시서 §4-3 요구)

`runSelfTests()`에 8건의 회귀 fixture를 추가해 `main()` 실행 전에 항상 돈다:
1. 정상 fixture 2건(LT-03·LT-10) — 오류 0건이어야 함(통과 확인).
2. **메타데이터 불일치**: lesson_track의 unitId·lessonNumbers를 `_index.json`과 다르게 준 fixture → 두 항목 모두 오류로 잡힘.
3. **파일 부재·mudId 불일치**: `_index.json`이 가리키는 파일이 없는 경우, 파일은 있지만 안의 mudId가 다른 경우 → 둘 다 오류로 잡힘.
4. **등록 누락**: 디스크엔 있는데 `_index.json` regular 목록에 없는 파일 → 오류로 잡힘.
5. **같은 eraLabel, 다른 eraRange** → 오류로 잡힘.
6. **열린 범위(`~`로 끝남)** → 오류로 잡히고, `~현재`는 예외로 통과함을 함께 확인.
7. **기대값 표와 다름** → 실패가 아니라 경고로만 나는 것을 확인(오류 0건, 경고 1건 이상).

추가로 실제 `data/lesson_track.json` 파일을 임시로 손상시켜(entries[0].unitId를 99로 바꿔 저장) `main()`의 실제 파일 읽기 경로에서도 LT-03이 잡아내는지 직접 확인했고, 확인 뒤 원본으로 정확히 복원했다(`git status --short`로 diff 없음 확인).

### 실제 실행 결과 — 내 worktree는 통합 전이라 실패해도 되는 상태

지시서 §4 끝 문장대로("자기 worktree에서 D-037 검사가 실패해도 된다. 통합본 기준으로 통과하면 된다") 현재 `data/lesson_track.json`(관문 설계실 작업 전 버전)으로 돌리면 **FAIL 9건**이 난다:
- "조선 전기" eraLabel에 3가지 다른 eraRange(1392~1592년경 / 1597년 / 1636~1637년)가 섞여 있음 — 관문 설계실이 라운드 11 §1-1에서 명량·외교 두 편을 통일하면 사라질 것.
- "광복·대한민국" eraLabel에 3가지 다른 eraRange(1945년~ / 1950~1953년 / 1953년~)가 섞여 있음 — 관문 설계실이 §1-2에서 통일하면 사라질 것.
- 구석기·신석기·청동기·개항기(2편)·광복(2편)의 열린 표기(`~`) 7건 — 개항기·광복은 §1-3 적용 후 "1876년 이후"·"1945년~현재"로 닫혀 사라지지만, **구석기·신석기·청동기의 열린 표기는 이번 라운드 지시서에 닫으라는 지시가 없어 통합 후에도 계속 실패로 남을 수 있다.**

LT-03(메타데이터·파일 대조)은 현재 데이터에서는 오류 0건 — 구조적으로는 문제없다.

### 확인 불가·다음 라운드에 넘길 것

- **구석기·신석기·청동기의 열린 연대 표기**(`eraRange`가 "~"로 끝남)는 LT-10의 일반 규칙("~"는 실패, "~현재"만 예외)에 걸리지만 라운드 11 지시서는 이 세 시대를 언급하지 않는다. 이대로면 통합 후에도 `scripts/16`이 계속 실패한다. **기획 세션이 판단할 사항**: (a) 이 세 시대도 끝 연도를 정해 닫거나, (b) "~"로 끝나되 다음 시대로 자연스럽게 이어지는 경우는 예외로 허용하도록 LT-10 규칙 자체를 조정하거나, (c) 이번 라운드는 §1이 지정한 두 시대만 닫는 게 맞으므로 LT-10을 "지정된 eraLabel에만" 적용하도록 좁힌다. 세 선택지 중 어느 쪽인지 조작대가 임의로 정하지 않고 다음 라운드 지시서로 넘긴다.
- `EXPECTED_ERA_RANGES` 표는 대조실의 라운드 9 문서 기준이라, 대조실이 라운드 11 §2에서 개항기 끝 연도(1910년 여부)를 새로 확인하면 표와 `lesson_track_era_review.md`를 함께 갱신해야 한다(스크립트 경고 메시지에도 이 안내를 넣었다).

### 검증

`01`·`04`·`05` 통과. `16`(기본, `--ci` 모두)은 위에 적은 대로 현재 worktree 데이터 기준 FAIL 9건 — 지시서가 허용한 상태다. 자기 테스트(fixture 8건)는 모두 통과. `git status --short`로 `scripts/16_validate_lesson_track.js` 외 다른 파일은 건드리지 않았음을 확인.

## 2026-09-21 — 기획 세션: 라운드 11 통합 (연대표 감사 잔여 LT-03·08·09·10, 수업 관찰 준비)

- **관문 설계실**: D-037 적용. 명량·병자호란 편을 "조선 후기"로 옮겼다(LT-08 모순 해소). 같은 시대명이면 같은 연대가 되게 맞췄다. 끝을 확인하지 못한 선사 시대는 "○○ 이후", 광복·대한민국은 "1945년~현재"로 썼다(LT-09).
- **대조실**
  - 개항기: "개항기"를 1876~1910으로 **이름 붙여 정의한 원문은 확인 불가**다. 다만 두 경계 연도(1876년 시작, 1910년 국권 피탈로 일제강점기 시작)는 각각 원문으로 확인했다.
  - 명량·병자호란: 연대 기준으로는 조선 후기, 교과서 단원 조직 기준으로는 조선 전기에 남는다. 대조실은 **연대 기준이 더 일관된다**고 보면서 판단을 기획 세션에 넘겼다.
- **기획 세션 판단**
  1. **D-037을 유지한다.** 명량·병자호란은 조선 후기다. 띠의 연대는 시대 범위를 보여 주고, 같은 시대명이 서로 다른 연도를 보이면 학생에게 모순으로 읽힌다(Codex LT-08이 지적한 바로 그 문제).
  2. **개항기를 `1876년~1910년`으로 확정한다.** 두 경계가 각각 원문으로 확인됐고, 다음 칸(일제강점기 1910년~)과 끊김 없이 이어진다. "이후"로 두면 LT-09의 모호함이 남는다.
  3. `scripts/16`의 기대값 표를 실제 확정값에 맞췄다. 선사 시대의 "이후" 표기가 기대값과 달라 경고가 나던 것을 해소했다.
- **조작대**: `scripts/16`에 LT-03(`_index.json`·실제 JSON·띠 3자 대조, 파일 목록 대조, `--ci`)과 LT-10(같은 시대 같은 범위, 열린 범위 금지, 기대값 표)을 넣었다. 자기 테스트 fixture 8건이 모두 의도대로 실패·통과한다.
- **연결 공방**: 수업 관찰 기록지를 갱신했다.
  - 표 C에 연대표 띠·완료 화면 버튼·3단원 수정 문구 관찰을 추가했다.
  - B에 최소 검증 후보 3항목(12~14)을 추가했다.
  - EXP-008 잔여 확인 사항을 기존 칸에 연결하고, 중·하 수준 학생 1명 포함을 **필수**로 올렸다.
  - **B가 약 50줄로 A4 한 장을 넘는다**고 스스로 확인해, 뺄 항목(7번 편별 포인트 축약, 2·6번 병합)을 표시했다.
- **Codex 중복 작업 기록**: Codex도 같은 LT-03·LT-10을 `feat/codex-lesson-track-validator`(`3fcccc4`)에 따로 구현했다. 통합 데이터에서 두 구현 모두 통과했다. 이미 병합된 조작대 구현(기대값 표와 자기 테스트 포함)을 기준으로 두고, **Codex 브랜치는 병합하지 않고 참고로 남긴다.** 중복의 원인은 기획 세션이 라운드 11 착수를 Codex에게 알리지 않은 것이다. 이후 Claude 소유 파일의 구현 착수는 Codex 전달 문서에 명시한다.
- 검증: 01·04·05·06·08·16(`--ci`)·17 통과.
