...
루트 `activity_duration_audit.md`(`scripts/07_audit_activity_duration.py`)가 8종(위 7종 + `regular_paleolithic`)을 "반복 탭 조기 종료 위험"으로 표시하던 것은 **07번 스크립트 쪽의 오탐**이었다. `regular_paleolithic`은 `paleo-*` 전용 렌더러가 중복 입력을 자체적으로 거부해 09번이 이미 안전하다고 판정한 대상인데, 07번이 그 제외 목록을 갖고 있지 않았다.

### 2026-09-07 판정 기준 통일 (Claude)

두 스크립트가 각자 판정을 들고 있어 규칙이 **네 군데** 갈려 있었다. 출력 차이는 `regular_paleolithic` 하나뿐이었지만 언제든 다시 어긋날 수 있는 구조였다.
...
판정을 [`scripts/_tap_resistance_rule.py`](../../scripts/_tap_resistance_rule.py) 하나로 뽑고 두 스크립트가 함께 쓰도록 했다. 통일 기준은 **제외 조건의 합집합**이다 — 다섯 조건은 각각 "같은 곳을 반복해서 눌러도 통과되지 않는다"는 서로 다른 근거이므로, 하나라도 해당하면 후보가 아니다.

결과: 07번 8종 → **7종**, 09번 7종 유지, 두 스크립트 판정 **완전 일치**. 이제 규칙이 한 곳에만 있어 구조적으로 다시 갈릴 수 없다.
