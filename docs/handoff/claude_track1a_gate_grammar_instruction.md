...
런타임 코드는 「조작대」(트랙 1-B) 소유다. 이 창은 **기존 런타임이 이미 지원하는 문법만** 사용한다. 새 문법이 필요하면 구현하지 말고 §4의 설계 문서로만 남긴다.
...
2. 현재 `inquiry-task`의 화면은 세로로 긴 폼(1./2./3. 섹션)이라 아이패드 가로에서 제출 버튼이 보이지 않는다. 이 UI로 27편을 확장하면 나중에 27편을 다시 손대야 한다. UI 개선은 「조작대」(트랙 1-B)가 병행한다. 설계실이 만드는 3편은 현재 UI 위에서 동작하므로 조작대를 기다리지 않는다.
...
`perspective` 문법 설계안을 `docs/plans/mud_perspective_task_design.md`로 **문서만** 작성한다. 코드는 「조작대」가 구현한다.
