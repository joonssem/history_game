...
남는 조건: 제출 여부와 모둠 산출물은 수업 중 잠시 서버에 존재한다. Convex는 미국 호스팅이므로 **학교·교육청의 외부 서비스 및 국외 서버 사용 지침을 착수 전에 확인한다**(`BACKLOG.md` P2-COLLAB-05). 국외 서버가 불가하다는 답이 나오면 리전을 고를 수 있는 백엔드로 [D-019](../../DECISIONS.md)를 다시 연다. 이 §2-3 설계는 백엔드와 무관하게 그대로 쓸 수 있다.
...
**보관 기간은 0일이다**(§2-3). `수업 종료`가 곧 삭제이며, 교사가 남기고 싶은 관찰은 앱 밖에 옮겨 적는다. `deleteAfter`는 종료 버튼을 누르지 못한 세션의 안전망으로만 쓰고 짧게 잡는다. 삭제 작업은 세션 하위 rooms/players/actions/events와 입장·복구 자격을 함께 제거하고 세션 전용 익명 인증 잔여 데이터도 정리한다. 인증·호스팅 제공자의 운영 로그·백업 보관은 별도 확인한다. 후속 연구용 내보내기·학생 장기 추적·다른 교사 초대는 MVP 밖이다.
